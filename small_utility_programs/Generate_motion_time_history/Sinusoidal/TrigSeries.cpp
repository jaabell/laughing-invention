      ...
      period(T), shift(phi), cFactor(theFactor)
{
    if (period == 0.0)
    {
        std::cerr << "TrigSeries::TrigSeries -- input period is zero, setting period to PI\n";
        period = 2 * asin(1.0);
    }
}
...
TrigSeries::getFactor(double pseudoTime)
{
    ...
    double returnedfactor;

    if (pseudoTime >= tStart && pseudoTime <= tFinish)
    {
        returnedfactor = cFactor * sin(twopi * (pseudoTime - tStart) / period + shift);
...
TrigSeries::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    if (result < 0)
    {
        std::cerr << "TrigSeries::sendSelf() - channel failed to send data\n";
        return result;
    }
                     ...
                     FEM_ObjectBroker& theBroker)
{
    ...
    if (result < 0)
    {
        std::cerr << "TrigSeries::sendSelf() - channel failed to receive data\n";
        cFactor = 1.0;
        tStart = 0.0;
        tFinish = 0.0;
        period = 1.0;
