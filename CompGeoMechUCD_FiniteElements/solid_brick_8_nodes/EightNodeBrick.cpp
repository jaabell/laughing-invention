...
/////////////////////////////////////////////////////////////////////////////////////
// Adapted by Sumeet, for EightNodeBrick to use the LTensor library for
...
DTensor2 EightNodeBrick::gp_coords(Num_TotalGaussPts, 3, 0.0);
DTensor2 EightNodeBrick::gp_weight(Num_TotalGaussPts, 3, 0.0);
Matrix EightNodeBrick::gauss_points(Num_TotalGaussPts, 3);
vector<float> EightNodeBrick::Gauss_Output_Vector(Num_TotalGaussPts * 18);
...
double EightNodeBrick::SurfaceLoadValues_in_function;
      ...
      M( 24, 24), P( 24 )
{
    ...
    if (material_array == 0)
    {
        cerr << " EightNodeBrick::EightNodeBrick -- failed allocate material model pointer\n";
        exit(-1);
    }
...
//#############################################################################
//***************************************************************
const DTensor2 &EightNodeBrick::H_3D( double r1, double r2, double r3 ) const
{
...
//#############################################################################
//***************************************************************
const DTensor1 &EightNodeBrick::interp_poli_at( double r1, double r2, double r3 ) const
{
...
const DTensor2 &EightNodeBrick::dh_drst_at( double r1, double r2, double r3 ) const
{
...
////#############################################################################
const DTensor2 &EightNodeBrick::Jacobian_3D( const DTensor2 &dh ) const
{
...
//#############################################################################
const DTensor2  &EightNodeBrick::Jacobian_3Dinv( const DTensor2 &dh ) const
{
    const DTensor2 &N_C = Nodal_Coordinates();
...
////#############################################################################
const DTensor2 &EightNodeBrick::Nodal_Coordinates( void ) const
{
...
////#############################################################################
const DTensor2 &EightNodeBrick::incr_disp( void ) const
{
...
////#############################################################################
const DTensor2 &EightNodeBrick::total_disp( void ) const
{
...
const DTensor2 &EightNodeBrick::nodal_forces( void ) const
{
    double r  = 0.0;
    double w_r = 0.0;
    double s  = 0.0;
...
const Matrix &EightNodeBrick::getInitialStiff ()
{
    ...
    if ( Ki == 0 )
    {
        cerr << "FATAL EightNodeBrick::getInitialStiff() -";
...
const Vector &EightNodeBrick::getSurfaceForce( double loadFactor, const Vector &data )
{
    ...
    int success = 0;
    int surface_nodes_order[4] = {0, 0, 0, 0};
    for ( int i = 0; i < 3 ; i++)
    {
        for ( int j = 0; j < 3; j++)
        {
...
int EightNodeBrick::addLoad( ElementalLoad *theLoad, double loadFactor )
{
    ...
    {
        cerr << "EightNodeBrick::addLoad() - 8NodeBrickLT " << this->getTag() << ",load type " << type << "unknown\n";
        return -1;
    }
...
int EightNodeBrick::sendSelf ( int commitTag, Channel &theChannel )
{
    ...
    {
        cerr << "WARNING EightNodeBrick::sendSelf() - " << this->getTag() << " failed to send ID idData\n";
        return -1;
    }
    ...
    if (idData( 2 ) == 1)
    {
        ...
        {
            cerr << "WARNING EightNodeBrick::sendSelf() - " << this->getTag() << " failed to send its K\n";
            return -1;
        }
    }
    ...
    {
        cerr << "WARNING EightNodeBrick::sendSelf() - " << this->getTag() << " failed to send Vector floatData\n";
        return -1;
    }
    ...
    {
        cerr << "WARNING EightNodeBrick::sendSelf() - " << this->getTag() << " failed to send String stiffness_type\n";
        return -1;
    }
    ...
    {
        cerr << "WARNING EightNodeBrick::sendSelf() - " << this->getTag() << " failed to send String damping_type\n";
        return -1;
    }

    // Send the nodes

    // cout << this->getTag() << " -> EightNodeBrick::sendSelf() connectedExternalNodes = " << connectedExternalNodes << "\n";


    if ( theChannel.sendID( 0, commitTag, connectedExternalNodes ) < 0 )
    {
        cerr << "WARNING EightNodeBrick::sendSelf() - " << this->getTag() << " failed to send ID connectedExternalNodes\n";
        return -1;
    }
    ...
    {
        cerr << "WARNING EightNodeBrick::sendSelf() - " << this->getTag() << " failed to send its Q\n";
        return -1;
    }
    ...
    {
        cerr << "WARNING EightNodeBrick::sendSelf() - " << this->getTag() << " failed to send its bf\n";
        return -1;
    }
...
int EightNodeBrick::receiveSelf ( int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker )
{
    ...
    if (idData(2) == 1)
    {
        if ( theChannel.sendMatrix( 0, commitTag, K ) < 0 )
        {
            cerr << "WARNING EightNodeBrick::receiveSelf() - " << this->getTag() << " failed to recieve its K\n";
            return -1;
        }
    }
    ...
    {
        cerr << "WARNING EightNodeBrick::receiveSelf() - " << this->getTag() << " failed to recieve Vector floatData\n";
        return -1;
    }
    Volume                  = floatData(0) ;
    ...
    {
        cerr << "WARNING EightNodeBrick::receiveSelf() - " << this->getTag() << " failed to recieve String stiffness_type\n";
        return -1;
    }
    ...
    if ( theChannel.receiveString( 0, commitTag, tmp_string ) < 0 )
    {
        cerr << "WARNING EightNodeBrick::receiveSelf() - " << this->getTag() << " failed to recieve String damping_type\n";
        return -1;
    }
    ...
    {
        cerr << "WARNING EightNodeBrick::receiveSelf() - " << this->getTag() << " failed to recieve ID connectedExternalNodes\n";
        return -1;
    }
    ...
    for ( int i = 0; i < Num_TotalGaussPts; i++ )
    {
        ...
        if ( ndmat == 0 )
        {
            cerr << "EightNodeBrick::receiveSelf() - Broker could not create NDMaterialLT of class type " << matClassTag << "\n";
            return -1;
        }
    ...
    {
        cerr << "EightNodeBrick::receiveSelf() - failed to recv Q!\n";
        return -1;
    }
    ...
    {
        cerr << "EightNodeBrick::receiveSelf() - failed to recv bf!\n";
        return -1;
    }
...
{
    cout << "EightNodeBrick: LTensor-based 8 node brick." << endl << endl;
...
int EightNodeBrick::update( void )
{
    ...
    //Debug
    // cout << endl << endl << "EightNodeBrick::update()" << endl;
    ...
    for ( short gp = 0; gp < Num_TotalGaussPts; gp++ )
    {
        ...
        if ( ( material_array[gp]->setTrialStrainIncr( trial_strain ) ) )
        {
            ...
            cerr << "EightNodeBrick::update (tag: " << this->getTag()
...
Matrix &EightNodeBrick::get_Gauss_p_c( )
{
...
Matrix &EightNodeBrick::get_Gauss_p_w()
{
...
***********************************************************************************/
const vector<float> &EightNodeBrick::getGaussOutput()
{
    DTensor2 stress(3, 3);
    DTensor2 strain(3, 3);
...
void EightNodeBrick::zeroStrain()
{
    for (int gp = 0; gp < Num_TotalGaussPts; gp++)
    {
