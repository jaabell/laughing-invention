      ...
      theMaterial(0), connectedExternalNodes(NumNodes), Q(0), bf(NumDof), Ki(0)
{
    ...
    if (theMaterial == 0)
    {
        cerr << "FiniteDeformationElastic3D::FiniteDeformationElastic3D -- failed allocate material model pointer\n";
        exit(-1);
    }
...
void TotalLagrangianFD20NodeBrick::setDomain (Domain *theDomain)
{
    ...
    for (i = 0; i < NumNodes; i++)
    {
        ...
        if ( theNodes[i] == 0 )
        {
            cerr << "FATAL ERROR TotalLagrangianFD8NodeBrick (tag: " << this->getTag() <<
...
int TotalLagrangianFD20NodeBrick::commitState ()
{
    ...
    {
        cerr << "TotalLagrangianFD20NodeBrick::commitState () - failed in base class";
    }
...
{
    cerr << "TotalLagrangianFD20NodeBrick::addLoad - load type unknown for ele with tag: " << this->getTag();
    return -1;
}
...
int TotalLagrangianFD20NodeBrick::addInertiaLoadToUnbalance(const Vector &accel)
{
    ...
    for (i = 0; i < NumNodes; i++)
    {
        ...
        if ( RA.Size() != NumDof )
        {
            cerr << "TotalLagrangianFD20NodeBrick::addInertiaLoadToUnbalance(): matrix and vector sizes are incompatable \n";
            return (-1);
        }
...
const Vector &TotalLagrangianFD20NodeBrick::getResistingForceIncInertia ()
{
    ...
    if (rho != 0.0)
    {
        for (i = 0; i < NumNodes; i++)
        {
            ...
            if ( acc.Size() != NumDof )
            {
                cerr << "TotalLagrangianFD20NodeBrick::getResistingForceIncInertia matrix and vector sizes are incompatable \n";
                exit(-1);
            }
...
//=============================================================================
void TotalLagrangianFD20NodeBrick::Print(ostream &s, int flag)
{
