    ...
    Ki(0)
{
    ...
    if (theMaterial == 0)
    {
        cerr << " TwentyNodeBrick_u_p_U::TwentyNodeBrick_u_p_U -- failed allocate material model pointer\n";
        exit(-1);
    }
...
void TwentyNodeBrick_u_p_U::setDomain (Domain *theDomain)
{
    ...
    for (i = 0; i < Num_Nodes; i++)
    {
        ...
        if (theNodes[i] == 0)
        {
            cerr << "Error TwentyNodeBrick_u_p_U : node not found in the domain" << "\n";
            return ;
        }
        ...
        for ( j = 0; j < Num_Nodes; j++ )
        {
            for ( m = 0; m < Num_Dim; m++)
            {
                for ( n = 0; n < Num_Dim; n++)
                {
                    ...
                    if (a1 != 0.0)
                    {
                        MCK(i * Num_Dof + m, j * Num_Dof + n) += CRk.cval(i + 1, m + 1, n + 1, j + 1) * a1;
                    }
...
int TwentyNodeBrick_u_p_U::addLoad(ElementalLoad *theLoad, double loadFactor)
{
    ...
    {
        cerr << "TwentyNodeBrick_u_p_U::addLoad() " << this->getTag() << ", load type unknown\n";
        return -1;
    }
    ...
    for ( GP_c_r = 0 ; GP_c_r < Num_IntegrationPts; GP_c_r++ )
    {
        ...
        for ( GP_c_s = 0 ; GP_c_s < Num_IntegrationPts; GP_c_s++ )
        {
            ...
            for ( GP_c_t = 0 ; GP_c_t < Num_IntegrationPts; GP_c_t++ )
            {
                ...
                {
                    cerr << "TwentyNodeBrick_u_p_U::update (tag: " << this->getTag() << "), not converged\n";
                }
            }
        }
    }
...
const Matrix &TwentyNodeBrick_u_p_U::getStiff (int Ki_flag)
{
    ...
    {
        cerr << "Error TwentyNodeBrick_u_p_U::getStiff() - illegal use\n";
        exit(-1);
    }
    ...
    if (Ki == 0)
    {
        cerr << "Error TwentyNodeBrick_u_p_U::getStiff() -";
