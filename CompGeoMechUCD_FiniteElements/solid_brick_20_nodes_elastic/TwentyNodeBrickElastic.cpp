...
// FILE:              TwentyNodeBrickElastic.cpp
// CLASS:             TwentyNodeBrickElastic
// MEMBER FUNCTIONS:
//
// MEMBER VARIABLES
//
...
#ifndef TWENTYNODEBRICKELASTIC_CPP
#define TWENTYNODEBRICKELASTIC_CPP
...
#include <TwentyNodeBrickElastic.h>
#include <ElementalLoad.h>
#define FixedOrder 3


// Matrix TwentyNodeBrickElastic::K(60, 60);
Matrix TwentyNodeBrickElastic::C(60, 60);
// Matrix TwentyNodeBrickElastic::M(60, 60);
Vector TwentyNodeBrickElastic::P(60);
Vector InfoElastic(109 + 3); //For computing moment
Vector InfoPtElastic(FixedOrder *FixedOrder *FixedOrder * 4 + 1); //Plastic info
Vector InfoStElastic(FixedOrder *FixedOrder *FixedOrder * 6 + 1); //Stress info
Vector GscElastic(FixedOrder *FixedOrder *FixedOrder * 3 + 1); //Gauss point coordinates
Vector InfoSpq2Elastic(2); //p and q of count/2
const int TwentyNodeBrickElastic::Num_TotalGaussPts = FixedOrder * FixedOrder * FixedOrder; //Nima added for recording the Strain (02-21-2011)
Vector TwentyNodeBrickElastic::ShapeFunctionValues_in_function(8); // Nima added for surface load (July 2012)
Vector TwentyNodeBrickElastic::J_vector_in_function(3); // Nima added for surface load (July 2012)
double TwentyNodeBrickElastic::SurfaceLoadValues_in_function; // Nima added for surface load (July 2012)
...
TwentyNodeBrickElastic::TwentyNodeBrickElastic(int element_number,
    ...
    : Element(element_number, ELE_TAG_TwentyNodeBrickElastic ),
      connectedExternalNodes(20), Kinitial(0), Mass(0), Q(60), builtK(0), builtM(0),
      rho(0.0)
{
    ...
    Kinitial = new Matrix(60, 60);
    Mass = new Matrix(60, 60);
...
//====================================================================
TwentyNodeBrickElastic::TwentyNodeBrickElastic (): Element(0, ELE_TAG_TwentyNodeBrickElastic ),
    connectedExternalNodes(20), Kinitial(0), Mass(0), Q(60), builtK(0), builtM(0), rho(0.0), mmodel(0)
{
    matpoint = 0;
...
/////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////
TwentyNodeBrickElastic::~TwentyNodeBrickElastic ()
{
...
/////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////
void TwentyNodeBrickElastic::incremental_Update()
{
    double r  = 0.0;
    // double rw = 0.0;
    double s  = 0.0;
    ...
    for ( short GP_c_r = 1 ; GP_c_r <= integration_order ; GP_c_r++ )
    {
        ...
        for ( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for ( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                //::::::   ::printf(" void TwentyNodeBrickElastic::incremental_Update()\n");
                ...
                {
                    cerr << "TwentyNodeBrickElastic::incremental_Update (tag: " << this->getTag() << "), not converged\n";
                }
...
//#############################################################################
//***************************************************************
tensor TwentyNodeBrickElastic::H_3D(double r1, double r2, double r3)
{
...
//#############################################################################
//***************************************************************
tensor TwentyNodeBrickElastic::interp_poli_at(double r1, double r2, double r3)
{
...
tensor TwentyNodeBrickElastic::dh_drst_at(double r1, double r2, double r3)
{
...
////#############################################################################
TwentyNodeBrickElastic &TwentyNodeBrickElastic::operator[](int subscript)
{
    return ( *(this + subscript) );
}

//Finite_Element & TwentyNodeBrickElastic::operator[](short subscript)
//  {
//    return ( *(this+subscript) );
//  }

//Finite_Element & TwentyNodeBrickElastic::operator[](unsigned subscript)
//  {
//    return ( *(this+subscript) );
//  }
...
////#############################################################################
////#############################################################################
tensor TwentyNodeBrickElastic::getStiffnessTensor(void)
{
    int K_dim[] = {20, 3, 3, 20};
    ...
    for ( short GP_c_r = 1 ; GP_c_r <= integration_order ; GP_c_r++ )
    {
        ...
        for ( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for ( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                //}
                //else {
                //   g3ErrorHandler->fatal("TwentyNodeBrickElastic::incremental_Update (tag: %d), could not getTangentTensor", this->getTag());
                //   exit(1);
                //}
...
void TwentyNodeBrickElastic::set_strain_stress_tensor(FILE *fp, double *u)
{
    int dh_dim[] = {20, 3};
    tensor dh(2, dh_dim, 0.0);
...
////#############################################################################
//  tensor TwentyNodeBrickElastic::mass_tensor(Elastic  mmodel)
tensor TwentyNodeBrickElastic::getMassTensor(void)
{
    //int M_dim[] = {8,3,3,8};
    int M_dim[] = {60, 60};
...
tensor TwentyNodeBrickElastic::stiffness_matrix(const tensor &K)
{
    //    int K_dim[] = {20,3,3,20};
...
////#############################################################################
tensor TwentyNodeBrickElastic::mass_matrix(const tensor &M)
{
    //    int K_dim[] = {20,3,3,20};
    //    tensor K(4,K_dim,0.0);
...
////#############################################################################
tensor TwentyNodeBrickElastic::Jacobian_3D(tensor dh)
{
...
//#############################################################################
tensor TwentyNodeBrickElastic::Jacobian_3Dinv(tensor dh)
{
    tensor N_C = Nodal_Coordinates();
...
////#############################################################################
tensor TwentyNodeBrickElastic::Nodal_Coordinates(void)
{
...
////#############################################################################
tensor TwentyNodeBrickElastic::incr_disp(void)
{
    const int dimensions[] = {20, 3};
...
////#############################################################################
tensor TwentyNodeBrickElastic::total_disp(void)
{
    const int dimensions[] = {20, 3};
...
////#############################################################################
tensor TwentyNodeBrickElastic::total_disp(FILE *fp, double *u)
{
    const int dimensions[] = {20, 3};
    tensor total_disp(2, dimensions, 0.0);
...
////#############################################################################
int TwentyNodeBrickElastic::get_global_number_of_node(int local_node_number)
{
...
////#############################################################################
int  TwentyNodeBrickElastic::get_Brick_Number(void)
{
    //return elem_numb;
    return this->getTag();
}
...
//////#############################################################################
//int * TwentyNodeBrickElastic::get_LM(void)
//  {
//    return 0;
//  }
...
//////#############################################################################
//void TwentyNodeBrickElastic::set_LM(Node * node)
//  {
...
tensor TwentyNodeBrickElastic::nodal_forces(void)
{
    ...
    for ( short GP_c_r = 1 ; GP_c_r <= integration_order ; GP_c_r++ )
    {
        ...
        for ( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for ( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                //out May 2004, Guanzhou//     //cerr << " In nodal_force delta_incremental_strain tag "<< getTag() <<"  " <<incremental_strain << endln;
                //out May 2004, Guanzhou////  cerr << " el tag = "<< getTag();
                ...
                //out May 2004, Guanzhou                   cerr << "TwentyNodeBrickElastic::nodal_forces (tag: " << this->getTag() << ", not converged\n";
                ...
                //}
                //else {
                //   g3ErrorHandler->fatal("TwentyNodeBrickElastic::nodal_forces (tag: %d), could not getStress", this->getTag());
                //   exit(1);
                //}
...
tensor TwentyNodeBrickElastic::iterative_nodal_forces(void)
{
    int force_dim[] = {20, 3}; // Xiaoyan changed from {20,3 to {8,3} for 8 nodes
        ...
        for ( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for ( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                where =
                    ((GP_c_r - 1) * integration_order + GP_c_s - 1) * integration_order + GP_c_t - 1;
                //.....
                //.....::printf("TwentyNodeBrickElastic::iterative_nodal_forces(void)  ----**************** where = %d \n", where);
                //.....::printf("UPDATE ");
                //.....::printf("   GP_c_r = %d,  GP_c_s = %d,  GP_c_t = %d\n",
                //.....                           GP_c_r,GP_c_s,GP_c_t);
                // derivatives of local coordiantes with respect to local coordiantes
                ...
                nodal_forces =
                    nodal_forces + dhGlobal("ib") * stress_at_GP("ab") * weight;
                //nodal_forces.print("nf","\n TwentyNodeBrickElastic::iterative_nodal_forces Nodal Forces ~~~~\n");
...
tensor TwentyNodeBrickElastic::nodal_forces_from_stress(stresstensor &stress)
{
    int force_dim[] = {20, 3}; // Xiaoyan changed from {20,3 to {8,3} for 8 nodes
...
tensor TwentyNodeBrickElastic::linearized_nodal_forces(void)
{
    int force_dim[] = {20, 3}; // Xiaoyan changed from {20,3 to {8,3} for 8 nodes
    ...
    for ( short GP_c_r = 1 ; GP_c_r <= integration_order ; GP_c_r++ )
    {
        ...
        for ( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for ( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                if ( ! (matpoint[where]->matmodel)->setTrialStrainIncr( incremental_strain)  )
                {
                    cerr << "TwentyNodeBrickElastic::linearized_nodal_forces (tag: " << this->getTag() << "), not converged\n";
                }
...
//....void TwentyNodeBrickElastic::update_stress_strain(tensor & displacementsT)
//....  {
//....//    int force_dim[] = {20,3};
//....//    tensor nodal_forces(2,force_dim,0.0);
//....
...
////#############################################################################
////#############################################################################
//double TwentyNodeBrickElastic::get_first_q_ast(void)
//  {
...
//  }
////#############################################################################
//double TwentyNodeBrickElastic::get_first_etacone(void)
//  {
...
//#############################################################################
void TwentyNodeBrickElastic::report(char *msg)
{
    if ( msg )
    {
        ::printf("** %s", msg);
...
//#############################################################################
void TwentyNodeBrickElastic::reportshort(char *msg)
{
    if ( msg )
    {
        ::printf("** %s", msg);
    ...
    ::printf("\n Element Number = %d\n", this->getTag() );
    ::printf("\n Number of nodes in a TwentyNodeBrickElastic = %d\n",
             nodes_in_brick);
    ::printf("\n Determinant of Jacobian (! ==0 before comp.) = %f\n",
             determinant_of_Jacobian);
...
//#############################################################################
void TwentyNodeBrickElastic::reportPAK(char *msg)
{
    if ( msg )
    {
        ::printf("%s", msg);
...
//#############################################################################
void TwentyNodeBrickElastic::reportpqtheta(int GP_numb)
{
...
//#############################################################################
//void TwentyNodeBrickElastic::reportLM(char * msg)
//  {
//    if ( msg ) ::printf("%s",msg);
...
//#############################################################################
//Compute Gauss Point coordinates and store it in global GscElastic
void TwentyNodeBrickElastic::computeGaussPoint(void)
{
    //    if ( msg ) ::printf("** %s\n",msg);
    ...
    //Vector GscElastic(count*3+1); //+1: number of Gauss point in element
    GscElastic(0) = count;
    ...
    for ( short GP_c_r = 1 ; GP_c_r <= integration_order ; GP_c_r++ )
    {
        ...
        for ( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for ( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                GscElastic(where * 3 + 1) = matpointCoord.val(1, where + 1);
                GscElastic(where * 3 + 2) = matpointCoord.val(2, where + 1);
                GscElastic(where * 3 + 3) = matpointCoord.val(3, where + 1);
...
//#############################################################################
//void TwentyNodeBrickElastic::reportTensor(char * msg)
// ZHaohui added to print gauss point coord. to file fp

void TwentyNodeBrickElastic::reportTensorF(FILE *fp)
{
    //if ( msg ) ::printf("** %s\n",msg);
...
//=============================================================================
int TwentyNodeBrickElastic::getNumExternalNodes () const
{
...
//=============================================================================
const ID &TwentyNodeBrickElastic::getExternalNodes ()
{
    return connectedExternalNodes;
}

Node **
TwentyNodeBrickElastic::getNodePtrs(void)
{
    return theNodes;
}

//=============================================================================
int TwentyNodeBrickElastic::getNumDOF ()
{
    return 3 * nodes_in_brick; //Changed from 2*4=8 to 3*8=24 Xiaoyan 07/06/00
}

//=============================================================================
void TwentyNodeBrickElastic::setDomain (Domain *theDomain)
{
    ...
    else
    {
                ...
                theNodes[16] == 0 || theNodes[17] == 0 || theNodes[18] == 0 || theNodes[19] == 0 )
        {
            ...
            cerr << "FATAL ERROR TwentyNodeBrickElastic (tag: " << this->getTag() << " ), node not found in domain\n";
            exit(-1);
        }
        ...
        {
            cerr << "FATAL ERROR TwentyNodeBrickElastic (tag: " << this->getTag() <<
...
//=============================================================================
int TwentyNodeBrickElastic::commitState ()
{
    int retVal = 0;
    ...
    {
        cerr << "TwentyNodeBrickElastic::commitState () - failed in base class";
    }
...
//=============================================================================
int TwentyNodeBrickElastic::revertToLastCommit ()
{
    //  int order = theQuadRule->getOrder(); // Commented by Xiaoyan
    int i;
    //int j, k;     // Xiaoyan added k for three dimension
...
//=============================================================================
int TwentyNodeBrickElastic::revertToStart ()
{
...
//=============================================================================
const Matrix &TwentyNodeBrickElastic::getTangentStiff()
{
...
//=============================================================================
const Matrix &TwentyNodeBrickElastic::getInitialStiff()
{
    //cerr << "WARNING - TwentyNodeBrickElastic::getInitialStiff() - not yet implemented\n";
...
//=============================================================================
//Get lumped mass
//const Matrix &TwentyNodeBrickElastic::getMass ()
const Matrix &TwentyNodeBrickElastic::getConsMass()
{
...
//=============================================================================
void TwentyNodeBrickElastic::zeroLoad(void)
{
    Q.Zero();
}
...
//======================================================================
const Vector &TwentyNodeBrickElastic::getBodyForce(double loadFactor, const Vector &data)
{
...
//======================================================================
const Vector &TwentyNodeBrickElastic::getSurfaceForce(double loadFactor, const Vector &data)
{
    ...
    Pressure(0) = data(8)  * loadFactor;
    Pressure(1) = data(9)  * loadFactor;
    Pressure(2) = data(10) * loadFactor;
    Pressure(3) = data(11) * loadFactor;
    Pressure(4) = data(12) * loadFactor;
    Pressure(5) = data(13) * loadFactor;
    Pressure(6) = data(14) * loadFactor;
    Pressure(7) = data(15) * loadFactor;
    Pressure(8) = data(16) * loadFactor;
        ...
        for (int j = 0; j < 8; j++)
        {
            ...
            for (int i = 0; i < 9; i++)
            {
                ...
                J_vector = Direction_Weight(GsPts(i, 0) , GsPts(i, 1),
                                            ...
                                            coordnode5, coordnode6, coordnode7, coordnode8);
...
//=============================================================================
int
TwentyNodeBrickElastic::addLoad(ElementalLoad *theLoad, double loadFactor)
{
    ...
    {
        cerr << "TwentyNodeBrick::addLoad() - 20NodeBrick " << this->getTag() << ",load type " << type << "unknown\n";
        return -1;
    }
...
//
//
//   } else if (type == LOAD_TAG_BrickSurfaceLoad20Node)
//   {
//
//
//     int node_exist =0;
...
//
//
//     Vector ShapeFunctionValues(8);
//     Vector J_vector(3);
//     double Pressure = data(8) * loadFactor;
//
//
//     Vector NodalForces(60);
//
//     for (int m=0; m<60; m++)
//         {
//           NodalForces(m) = 0;
...
//   } else
//   {
//     cerr << "TwentyNodeBrickElastic::addLoad() - 8NodeBrick " << this->getTag() << ",load type " << type << "unknown\n";
//     return -1;
//   }
//
//
...
////=============================================================================
//int  TwentyNodeBrickElastic::addLoad(const Vector &addLoad)
//{
//     if (addLoad.Size() != 60) {
//      cerr << "TwentyNodeBrickElastic::addLoad %s\n",
...
//=============================================================================
int TwentyNodeBrickElastic::addInertiaLoadToUnbalance(const Vector &accel)
{
    // Check for a quick return
    if (rho == 0.0)
    {
            ...
            3 != Raccel17.Size() || 3 != Raccel18.Size() || 3 != Raccel19.Size() || 3 != Raccel20.Size()   )
    {
        ...
        cerr << "TwentyNodeBrickElastic::addInertiaLoadToUnbalance " <<
...
//=============================================================================
const Vector TwentyNodeBrickElastic::FormEquiBodyForce(const Vector &data)
{
    Vector bforce(60);
...
//void TwentyNodeBrickElastic::setInitE(void)
//{
...
//=============================================================================
const Vector &TwentyNodeBrickElastic::getResistingForce ()
{
    int force_dim[] = {20, 3};
...
//=============================================================================
const Vector &TwentyNodeBrickElastic::getResistingForceIncInertia ()
{
...
//=============================================================================
int TwentyNodeBrickElastic::sendSelf (int commitTag, Channel &theChannel)
{
...
//=============================================================================
//Nima Comment out (11/12/2009)
/*
int TwentyNodeBrickElastic::displaySelf (Renderer &theViewer, int displayMode, float fact)
{
...
//=============================================================================
void TwentyNodeBrickElastic::Print(ostream &s, int flag)
{
    //report(" TwentyNodeBrickElastic ");
    s << "TwentyNodeBrickElastic, element id:  " << this->getTag() << endln;
...
// //=============================================================================
// Response* TwentyNodeBrickElastic::setResponse (const char** argv, int argc, Information& eleInformation)
// {
//     //========================================================
...
//         return new ElementResponse(this, 2, InfoPtElastic);
//     }
//     //========================================================
...
//     {
//         return new ElementResponse(this, 3, InfoStElastic);
//     }
//     //========================================================
//     else if (strcmp(argv[0], "stress") == 0 || strcmp(argv[0], "stresses") == 0)
//     {
//         return new ElementResponse(this, 4, InfoStElastic);
//     }
...
//     {
//         return new ElementResponse(this, 41, InfoSpq2Elastic);
//     }

//     //========================================================
//     else if (strcmp(argv[0], "GaussPoint") == 0 || strcmp(argv[0], "gausspoint") == 0)
//     {
//         return new ElementResponse(this, 6, GscElastic);
//     }
...
// //=============================================================================
// int TwentyNodeBrickElastic::getResponse (int responseID, Information& eleInfo)
// {
//     switch (responseID)
//     {
...
//                 //Vector GscElastic(81+1);  // 27*3 + count
//                 //GscElastic = this->reportGaussPoint();
//                 this->computeGaussPoint();

//                 //Vector Info(109); // count * 4 +1
//                 InfoPtElastic(0) = GscElastic(0);
...
//                 {
//                     //                   plastify = 0;
//                     InfoPtElastic(i * 4 + 1) = GscElastic(i * 3 + 1); //x
//                     InfoPtElastic(i * 4 + 2) = GscElastic(i * 3 + 2); //y
//                     InfoPtElastic(i * 4 + 3) = GscElastic(i * 3 + 3); //z
//                     pl_stn = matpoint[i]->getPlasticStrainTensor();
//                     //double  p_plastc = pl_stn.p_hydrostatic();
...
//                     InfoPtElastic(i * 4 + 4) = q_plastc; //plastify; //Plastified?

//                 }

//                 return eleInfo.setVector( InfoPtElastic );
//                 //return plastify;
...
//                 InfoElastic(0) = GscElastic(0);
//                 Vector Mt(3), Q(3);

//                 //Vector Zcoor(3);
//                 InfoElastic(109 + 0) = GscElastic(6); //Zcoor of middle layer
...
//                 //if ( (coor(2) - GscElastic(6)) > 0.0001 )
//                 //  cerr << " Warning: Middle layer Gauss Point Z-coor. wrong...\n";

//                 //Info(109+0) = GscElastic(6);
//                 InfoElastic(109 + 1) = TotDis(0); //x-displacement ...Lateral displacement
//                 //Info(109+3) = GscElastic(6);
//                 //Info(109+6) = GscElastic(9);
//                 //cerr << " Zz " << GscElastic(3) << " " << GscElastic(6) << " "<< GscElastic(9) << endln;
...
//                             InfoElastic(i * 4 + 1) = GscElastic(i * 3 + 1); //x
//                             InfoElastic(i * 4 + 2) = GscElastic(i * 3 + 2); //y
//                             InfoElastic(i * 4 + 3) = GscElastic(i * 3 + 3); //z
//                             InfoElastic(i * 4 + 4) = sts.cval(3, 3); //Assign sigma_zz
...
//                             if (strcmp(tp, "ElasticIsotropic3D") == 0 )
//                             {
//                                 Mt(GP_c_t - 1) += wt(rs) * sts.cval(3, 3) * ( InfoElastic(i * 4 + 1) - offset[tag] ) / height; //x--Calculating Moment_y wt(ts) / height = Area corresponding to the gauss point stress
...
//                 InfoElastic(109 + 2) = ( Mt(0) + Mt(1) + Mt(2) ) * 0.3333;
...
//                 return eleInfo.setVector( InfoElastic );
//             }
...
//                 int i;
//                 stresstensor sts;
//                 //Vector GscElastic(81+1);  // 8*3 + count
//                 //GscElastic = this->reportTensor("Gauss Point Coor.");

//                 //Vector Info(109 + 3 ); //Z values, x-disp. and corresponding avg. moment
//                 InfoStElastic(0) = count;
...
//                             sts = matpoint[i]->getStressTensor();
//                             InfoStElastic(i * 6 + 1) = sts.cval(1, 1); //sigma_xx
//                             InfoStElastic(i * 6 + 2) = sts.cval(2, 2); //sigma_yy
//                             InfoStElastic(i * 6 + 3) = sts.cval(3, 3); //sigma_zz
//                             InfoStElastic(i * 6 + 4) = sts.cval(1, 2); //Assign sigma_xy
//                             InfoStElastic(i * 6 + 5) = sts.cval(1, 3); //Assign sigma_xz
//                             InfoStElastic(i * 6 + 6) = sts.cval(2, 3); //Assign sigma_yz
//                         }
//                     }
//                 }

//                 return eleInfo.setVector( InfoStElastic );
//             }
...
//                 InfoSpq2Elastic(0) = sts.p_hydrostatic();
//                 InfoSpq2Elastic(1) = sts.q_deviatoric();
//                 return eleInfo.setVector( InfoSpq2Elastic );
//             }
...
//             {
//                 this->computeGaussPoint();
//                 return eleInfo.setVector( GscElastic );
//             }
...
////#############################################################################
Vector TwentyNodeBrickElastic::getWeightofGP(void)
{
    //int M_dim[] = {8,3,3,8};
    //int M_dim[] = {60,60};
    //tensor Mm(2,M_dim,0.0);
...
//const Matrix&
//TwentyNodeBrickElastic::getTangentStiff ()
//{
...
//const Matrix&
//TwentyNodeBrickElastic::getSecantStiff ()
//{
// return K;
//}

//Commented by Xiaoyan     Use the form like Brick3d
//const Matrix & TwentyNodeBrickElastic::getDamp ()
//{
// return C;
//}
...
//const Matrix&
//TwentyNodeBrickElastic::getMass ()
//{
// int order = theQuadRule->getOrder();
// const Vector &intPt = theQuadRule->getIntegrPointCoords();
// const Vector &intWt = theQuadRule->getIntegrPointWeights();
...
//
//const Vector&
//TwentyNodeBrickElastic::getResistingForce ()
//{
// int order = theQuadRule->getOrder();
// const Vector &intPt = theQuadRule->getIntegrPointCoords();
// const Vector &intWt = theQuadRule->getIntegrPointWeights();
...
//
//const Vector&
//TwentyNodeBrickElastic::getResistingForceIncInertia ()
//{
// // Yet to implement
// return P;
//}
...
//
//void
//TwentyNodeBrickElastic::Print (ostream &s, int flag)
//{
// s << "TwentyNodeBrickElastic, element id:  " << this->getTag() << endln;
// s << "Connected external nodes:  " << connectedExternalNodes;
...
//
//int
//TwentyNodeBrickElastic::displaySelf (Renderer &theViewer, int displayMode, float fact)
//{
// first determine the end points of the quad based on
// the display factor (a measure of the distorted image)
...
//
//void
//TwentyNodeBrickElastic::formNMatrix (double r, double s,double t)
...
// }
//void
//TwentyNodeBrickElastic::setJacobian (double r, double s, double t)
////Changed xi, eta to r,s and added t Xiaoyan 07/06/00
//{
// const Vector &nd1Crds = theNodes[0]->getCrds();
// const Vector &nd2Crds = theNodes[1]->getCrds();
...
//
//void
//TwentyNodeBrickElastic::formBMatrix (double r, double s, double t)
////Changed xi, eta to r,s and added t Xiaoyan  07/06/00
//{
//    B.Zero();
//
...
//double
//TwentyNodeBrickElastic::formDetJ (double r, double s, double t)
//{
...
double TwentyNodeBrickElastic::get_Gauss_p_c(short order, short point_numb)
{
...
double TwentyNodeBrickElastic::get_Gauss_p_w(short order, short point_numb)
{
...
int TwentyNodeBrickElastic::update() //Added by Guanzhou, May 7 2004
{
    double r  = 0.0;
    double s  = 0.0;
    double t  = 0.0;
    ...
    for ( short GP_c_r = 1 ; GP_c_r <= integration_order ; GP_c_r++ )
    {
        ...
        for ( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for ( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                if ( ( (matpoint[where]->matmodel)->setTrialStrainIncr( incremental_strain)) )
                {
                    cerr << "TwentyNodeBrickElastic::update (tag: " << this->getTag() << "), update() failed\n";
                }
            }
        }
    }
...
double  TwentyNodeBrickElastic::SurfaceShapeFunctionValues(double Xi , double Eta, int whichcomponent)
{
...
Vector &TwentyNodeBrickElastic::Direction_Weight(double Xi , double Eta,
...
double TwentyNodeBrickElastic::SurfaceLoadValues(double Xi , double Eta, Vector Pressure)
{
...
int
TwentyNodeBrickElastic::CheckMesh(ofstream &checkmesh_file)
{
...
//==================================================================================
Vector *
TwentyNodeBrickElastic::getStress(void)
{
