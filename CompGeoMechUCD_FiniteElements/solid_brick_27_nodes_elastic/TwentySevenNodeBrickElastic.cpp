...
//                    by Guanzhou Jie and Boris Jeremic, it is modified
...
#ifndef TWENTYSEVENNODEBRICKELASTIC_CPP
#define TWENTYSEVENNODEBRICKELASTIC_CPP
...
#include <TwentySevenNodeBrickElastic.h>
#include <ElementalLoad.h>
#define FixedOrder 3


// Matrix TwentySevenNodeBrickElastic::K(81, 81);
Matrix TwentySevenNodeBrickElastic::C(81, 81);
// Matrix TwentySevenNodeBrickElastic::M(81, 81);
Vector TwentySevenNodeBrickElastic::P(81);
// int TwentySevenNodeBrickElastic::builtK(0);
// int TwentySevenNodeBrickElastic::builtM(0);
Vector InfoMomentElastic(109 + 3); //For computing moment
Vector InfoPlasticElastic(FixedOrder *FixedOrder *FixedOrder * 4 + 1); //Plastic info
Vector InfoStressElastic(FixedOrder *FixedOrder *FixedOrder * 6 + 1); //Stress info
Vector GaussCoordElastic(FixedOrder *FixedOrder *FixedOrder * 3 + 1); //Gauss point coordinates
Vector Info_pq2Elastic(2); //p and q of count/2
const int TwentySevenNodeBrickElastic::Num_TotalGaussPts = FixedOrder * FixedOrder * FixedOrder; //Nima added for recording the Strain (02-21-2011)
Vector TwentySevenNodeBrickElastic::ShapeFunctionValues_in_function(9); // Nima added for surface load (July 2012)
Vector TwentySevenNodeBrickElastic::J_vector_in_function(3); // Nima added for surface load (July 2012)
double TwentySevenNodeBrickElastic::SurfaceLoadValues_in_function; // Nima added for surface load (July 2012)
...
TwentySevenNodeBrickElastic::TwentySevenNodeBrickElastic(int element_number,
    ...
    : Element(element_number, ELE_TAG_TwentySevenNodeBrickElastic),
      connectedExternalNodes(27), Kinitial(0), Mass(0), Q(81), builtK(0), builtM(0), rho(0.0)
{
...
//====================================================================
TwentySevenNodeBrickElastic::TwentySevenNodeBrickElastic (): Element(0, ELE_TAG_TwentySevenNodeBrickElastic ),
    connectedExternalNodes(27), Kinitial(0), Mass(0), Q(81), builtK(0), builtM(0), rho(0.0), mmodel(0)
{
    matpoint = 0;
...
/////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////
TwentySevenNodeBrickElastic::~TwentySevenNodeBrickElastic ()
{
...
/////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////
void TwentySevenNodeBrickElastic::incremental_Update()
{
    double r  = 0.0;
    // double rw = 0.0;
    double s  = 0.0;
    ...
    for ( short GP_c_r = 1 ; GP_c_r <= integration_order ; GP_c_r++ )
    {
        ...
        for ( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for ( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                //::::::   ::printf(" void TwentySevenNodeBrickElastic::incremental_Update()\n");
                ...
                {
                    cerr << "TwentySevenNodeBrickElastic::incremental_Update (tag: " << this->getTag() << "), not converged\n";
                }
...
// //#############################################################################
// //***************************************************************
// tensor TwentySevenNodeBrickElastic::H_3D(double r1, double r2, double r3)
//   {
//
//     int dimension[] = {81,3};
//
...
//#############################################################################
//***************************************************************
/*tensor TwentySevenNodeBrickElastic::interp_poli_at(double r1, double r2, double r3)
  {
...
// Nima Tafazzoli removed (Nov. 2011)
//***************************************************************
// tensor TwentySevenNodeBrickElastic::dh_drst_at(double r1, double r2, double r3)
//   {
//
...
////#############################################################################
TwentySevenNodeBrickElastic &TwentySevenNodeBrickElastic::operator[](int subscript)
{
    return ( *(this + subscript) );
}

//Finite_Element & TwentySevenNodeBrickElastic::operator[](short subscript)
//  {
//    return ( *(this+subscript) );
//  }

//Finite_Element & TwentySevenNodeBrickElastic::operator[](unsigned subscript)
//  {
//    return ( *(this+subscript) );
//  }
...
////#############################################################################
////#############################################################################
tensor TwentySevenNodeBrickElastic::getStiffnessTensor(void)
{
    int K_dim[] = {27, 3, 3, 27};
    ...
    for ( short GP_c_r = 1 ; GP_c_r <= integration_order ; GP_c_r++ )
    {
        ...
        for ( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for ( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                //}
                //else {
                //   g3ErrorHandler->fatal("TwentySevenNodeBrickElastic::incremental_Update (tag: %d), could not getTangentTensor", this->getTag());
                //   exit(1);
                //}
...
void TwentySevenNodeBrickElastic::set_strain_stress_tensor(FILE *fp, double *u)
{
    int dh_dim[] = {27, 3};
    tensor dh(2, dh_dim, 0.0);
...
////#############################################################################
//  tensor TwentySevenNodeBrickElastic::mass_tensor(Elastic  mmodel)
tensor TwentySevenNodeBrickElastic::getMassTensor(void)
{
    //int M_dim[] = {8,3,3,8};
    int M_dim[] = {81, 81};
    ...
    for ( short GP_c_r = 1 ; GP_c_r <= integration_order ; GP_c_r++ )
    {
        ...
        for ( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for ( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                // determinant of Jacobian tensor ( matrix )
                det_of_Jacobian  = Jacobian.determinant();
                //      cout << "Det of Jabobian 3 is: " << det_of_Jacobian << endl;
...
tensor TwentySevenNodeBrickElastic::stiffness_matrix(const tensor &K)
{
    //    int K_dim[] = {20,3,3,20};
...
////#############################################################################
tensor TwentySevenNodeBrickElastic::mass_matrix(const tensor &M)
{
    //    int K_dim[] = {20,3,3,20};
    //    tensor K(4,K_dim,0.0);
...
////#############################################################################
tensor TwentySevenNodeBrickElastic::Jacobian_3D(tensor dh)
{
...
//#############################################################################
tensor TwentySevenNodeBrickElastic::Jacobian_3Dinv(tensor dh)
{
    tensor N_C = Nodal_Coordinates();
...
////#############################################################################
tensor TwentySevenNodeBrickElastic::Nodal_Coordinates(void)
{
...
////#############################################################################
tensor TwentySevenNodeBrickElastic::incr_disp(void)
{
    const int dimensions[] = {27, 3};
...
////#############################################################################
tensor TwentySevenNodeBrickElastic::total_disp(void)
{
    const int dimensions[] = {27, 3};
...
////#############################################################################
tensor TwentySevenNodeBrickElastic::total_disp(FILE *fp, double *u)  //Not implemented yet??? Guanzhou Oct. 2003
{
    const int dimensions[] = {27, 3};
    tensor total_disp(2, dimensions, 0.0);
...
////#############################################################################
int TwentySevenNodeBrickElastic::get_global_number_of_node(int local_node_number)
{
...
////#############################################################################
int  TwentySevenNodeBrickElastic::get_Brick_Number(void)
{
    //return elem_numb;
    return this->getTag();
}
...
//////#############################################################################
//int * TwentySevenNodeBrickElastic::get_LM(void)
//  {
//    return 0;
//  }
...
//////#############################################################################
//void TwentySevenNodeBrickElastic::set_LM(Node * node)
//  {
...
tensor TwentySevenNodeBrickElastic::nodal_forces(void)
{
    ...
    for ( short GP_c_r = 1 ; GP_c_r <= integration_order ; GP_c_r++ )
    {
        ...
        for ( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for ( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                //out May 5 2004, Guanzhou//       //cerr << " In nodal_force delta_incremental_strain tag "<< getTag() <<"  " <<incremental_strain << endln;
                //out May 5 2004, Guanzhou////    cerr << " el tag = "<< getTag();
                ...
                //out May 5 2004, Guanzhou                    cerr << "TwentySevenNodeBrickElastic::nodal_forces (tag: " << this->getTag() << ", not converged\n";
                ...
                //}
                //else {
                //   g3ErrorHandler->fatal("TwentySevenNodeBrickElastic::nodal_forces (tag: %d), could not getStress", this->getTag());
                //   exit(1);
                //}
...
tensor TwentySevenNodeBrickElastic::iterative_nodal_forces(void)
{
    int force_dim[] = {27, 3};
        ...
        for ( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for ( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                where =
                    ((GP_c_r - 1) * integration_order + GP_c_s - 1) * integration_order + GP_c_t - 1;
                //.....
                //.....::printf("TwentySevenNodeBrickElastic::iterative_nodal_forces(void)  ----**************** where = %d \n", where);
                //.....::printf("UPDATE ");
                //.....::printf("   GP_c_r = %d,  GP_c_s = %d,  GP_c_t = %d\n",
                //.....                           GP_c_r,GP_c_s,GP_c_t);
                // derivatives of local coordiantes with respect to local coordiantes
                ...
                // determinant of Jacobian tensor ( matrix )
                det_of_Jacobian  = Jacobian.determinant();
                //      cout << "Det of Jabobian 5 is: " << det_of_Jacobian << endl;
                //....  ::printf("determinant of Jacobian is %f\n",Jacobian_determinant );
                ...
                nodal_forces =
                    nodal_forces + dhGlobal("ib") * stress_at_GP("ab") * weight;
                //nodal_forces.print("nf","\n TwentySevenNodeBrickElastic::iterative_nodal_forces Nodal Forces ~~~~\n");
...
tensor TwentySevenNodeBrickElastic::nodal_forces_from_stress(stresstensor &stress)
{
    int force_dim[] = {27, 3};
...
tensor TwentySevenNodeBrickElastic::linearized_nodal_forces(void)
{
    int force_dim[] = {27, 3};
    ...
    for ( short GP_c_r = 1 ; GP_c_r <= integration_order ; GP_c_r++ )
    {
        ...
        for ( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for ( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                if ( ! (matpoint[where]->matmodel)->setTrialStrainIncr( incremental_strain)  )
                {
                    cerr << "TwentySevenNodeBrickElastic::linearized_nodal_forces (tag: " << this->getTag() << "), not converged\n";
                }
...
//....void TwentySevenNodeBrickElastic::update_stress_strain(tensor & displacementsT)
//....  {
//....//    int force_dim[] = {20,3};
//....//    tensor nodal_forces(2,force_dim,0.0);
//....
...
////#############################################################################
////#############################################################################
//double TwentySevenNodeBrickElastic::get_first_q_ast(void)
//  {
...
//  }
////#############################################################################
//double TwentySevenNodeBrickElastic::get_first_etacone(void)
//  {
...
//#############################################################################
void TwentySevenNodeBrickElastic::report(char *msg)
{
    if ( msg )
    {
        ::printf("** %s", msg);
...
//#############################################################################
void TwentySevenNodeBrickElastic::reportshort(char *msg)
{
    if ( msg )
    {
        ::printf("** %s", msg);
    ...
    ::printf("\n Element Number = %d\n", this->getTag() );
    ::printf("\n Number of nodes in a TwentySevenNodeBrickElastic = %d\n",
             nodes_in_brick);
    ::printf("\n Determinant of Jacobian (! ==0 before comp.) = %f\n",
             determinant_of_Jacobian);
...
//#############################################################################
void TwentySevenNodeBrickElastic::reportPAK(char *msg)
{
    if ( msg )
    {
        ::printf("%s", msg);
...
//#############################################################################
void TwentySevenNodeBrickElastic::reportpqtheta(int GP_numb)
{
...
//#############################################################################
//void TwentySevenNodeBrickElastic::reportLM(char * msg)
//  {
//    if ( msg ) ::printf("%s",msg);
...
//#############################################################################
//Compute Gauss Point coordinates and store it in global GaussCoordElastic
void TwentySevenNodeBrickElastic::computeGaussPoint(void)
{
    //    if ( msg ) ::printf("** %s\n",msg);
    ...
    //Vector GaussCoordElastic(count*3+1); //+1: number of Gauss point in element
    GaussCoordElastic(0) = count;
    ...
    for ( short GP_c_r = 1 ; GP_c_r <= integration_order ; GP_c_r++ )
    {
        ...
        for ( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for ( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                GaussCoordElastic(where * 3 + 1) = matpointCoord.val(1, where + 1);
                GaussCoordElastic(where * 3 + 2) = matpointCoord.val(2, where + 1);
                GaussCoordElastic(where * 3 + 3) = matpointCoord.val(3, where + 1);
...
//#############################################################################
//void TwentySevenNodeBrickElastic::reportTensor(char * msg)
// ZHaohui added to print gauss point coord. to file fp

void TwentySevenNodeBrickElastic::reportTensorF(FILE *fp)
{
    //if ( msg ) ::printf("** %s\n",msg);
...
//=============================================================================
int TwentySevenNodeBrickElastic::getNumExternalNodes () const
{
...
//=============================================================================
const ID &TwentySevenNodeBrickElastic::getExternalNodes ()
{
    return connectedExternalNodes;
}

Node **
TwentySevenNodeBrickElastic::getNodePtrs(void)
{
    return theNodes;
}

//=============================================================================
int TwentySevenNodeBrickElastic::getNumDOF ()
{
    return 3 * nodes_in_brick;
}

//=============================================================================
void TwentySevenNodeBrickElastic::setDomain (Domain *theDomain)
{
    ...
    else
    {
                ...
                theNodes[24] == 0 || theNodes[25] == 0 || theNodes[26] == 0)
        {
            ...
            cerr << "FATAL ERROR TwentySevenNodeBrickElastic (tag: " << this->getTag() << " ), node not found in domain\n";
            exit(-1);
        }
        ...
        {
            cerr << "FATAL ERROR TwentySevenNodeBrickElastic (tag: " << this->getTag() <<
...
//=============================================================================
int TwentySevenNodeBrickElastic::commitState ()
{
    int retVal = 0;
    ...
    {
        cerr << "TwentySevenNodeBrickElastic::commitState () - failed in base class";
    }
...
//=============================================================================
int TwentySevenNodeBrickElastic::revertToLastCommit ()
{
    //  int order = theQuadRule->getOrder();  // Commented by Xiaoyan
    int i;
    //int j, k;     // Xiaoyan added k for three dimension
...
//=============================================================================
int TwentySevenNodeBrickElastic::revertToStart ()
{
...
//=============================================================================
const Matrix &TwentySevenNodeBrickElastic::getTangentStiff()
{
...
//=============================================================================
const Matrix &TwentySevenNodeBrickElastic::getInitialStiff ()
{
    //cerr << "WARNING - TwentySevenNodeBrickElastic::getInitialStiff() - not yet implemented\n";
...
//=============================================================================
//Get lumped mass
//const Matrix &TwentySevenNodeBrickElastic::getMass ()
const Matrix &TwentySevenNodeBrickElastic::getConsMass ()
{
...
const Matrix &TwentySevenNodeBrickElastic::getMass()
{
    ...
    if (builtM == 0)
    {
        ...
        for ( int i = 1 ; i <= nodes_in_brick * 3 ; i++ )
        {
            ...
            for ( int j = 1 ; j <= nodes_in_brick * 3 ; j++ )
            {
                (*Mass)( i - 1 , j - 1 ) = masstensor.cval(i, j);
...
//=============================================================================
void TwentySevenNodeBrickElastic::zeroLoad(void)
{
    Q.Zero();
}
...
//======================================================================
const Vector &TwentySevenNodeBrickElastic::getBodyForce(double loadFactor, const Vector &data)
{
...
//======================================================================
const Vector &TwentySevenNodeBrickElastic::getSurfaceForce(double loadFactor, const Vector &data)
{
...
//=============================================================================
int
TwentySevenNodeBrickElastic::addLoad(ElementalLoad *theLoad, double loadFactor)
{
    int type;
    ...
    {
        cerr << "TwentySevenNodeBrickElastic::addLoad() - 27NodeBrick " << this->getTag() << ",load type " << type << "unknown\n";
        return -1;
    }
...
//
//
//   } else if (type == LOAD_TAG_BrickSurfaceLoad27Node)
//   {
//
//
//     int node_exist =0;
...
//     Vector ShapeFunctionValues(9);
//     Vector J_vector(3);
//     double Pressure = data(9) * loadFactor;
//
//
//     Vector NodalForces(81);
//
//     for (int m=0; m<81; m++)
//         {
//           NodalForces(m) = 0;
...
//                                                        coordnode5, coordnode6, coordnode7, coordnode8, coordnode9);
//
//                            NodalForces(r*3+k) = NodalForces(r*3+k) + Pressure * J_vector(k) * ShapeFunctionValues(j) * Weight(i,0) * Weight(i,1);
//                           }
//              }
//         }
//
...
//   } else
//   {
//     cerr << "TwentySevenNodeBrickElastic::addLoad() - 8NodeBrick " << this->getTag() << ",load type " << type << "unknown\n";
//     return -1;
//   }
//
//
...
////=============================================================================
//int  TwentySevenNodeBrickElastic::addLoad(const Vector &addLoad)
//{
//     if (addLoad.Size() != 81) {
//       cerr << "TwentySevenNodeBrickElastic::addLoad %s\n",
...
//=============================================================================
int TwentySevenNodeBrickElastic::addInertiaLoadToUnbalance(const Vector &accel)
{
    // Check for a quick return
    if (rho == 0.0)
    {
            ...
            3 != Raccel25.Size() || 3 != Raccel26.Size() || 3 != Raccel27.Size())
    {
        ...
        cerr << "TwentySevenNodeBrickElastic::addInertiaLoadToUnbalance " <<
...
//=============================================================================
const Vector TwentySevenNodeBrickElastic::FormEquiBodyForce(const Vector &data)
{
    Vector bforce(81);
...
//void TwentySevenNodeBrickElastic::setInitE(void)
//{
...
//=============================================================================
const Vector &TwentySevenNodeBrickElastic::getResistingForce ()
{
    int force_dim[] = {27, 3};
...
//=============================================================================
const Vector &TwentySevenNodeBrickElastic::getResistingForceIncInertia ()
{
...
//=============================================================================
/*
int TwentySevenNodeBrickElastic::displaySelf (Renderer &theViewer, int displayMode, float fact)
{
...
//=============================================================================
void TwentySevenNodeBrickElastic::Print(ostream &s, int flag)
{
    //report(" TwentySevenNodeBrickElastic ");
    s << "TwentySevenNodeBrickElastic, element id:  " << this->getTag() << endln;
...
// //=============================================================================
// Response* TwentySevenNodeBrickElastic::setResponse (const char** argv, int argc, Information& eleInformation)
// {
//     //========================================================
...
//     {
//         return new ElementResponse(this, 5, *Kinitial);
//     }
...
//         return new ElementResponse(this, 2, InfoPlasticElastic);
//     }
//     //========================================================
...
//     {
//         return new ElementResponse(this, 3, InfoStressElastic);
//     }
//     //========================================================
//     else if (strcmp(argv[0], "stress") == 0 || strcmp(argv[0], "stresses") == 0)
//     {
//         return new ElementResponse(this, 4, InfoStressElastic);
//     }
...
//     {
//         return new ElementResponse(this, 41, Info_pq2Elastic);
//     }

//     //========================================================
//     else if (strcmp(argv[0], "GaussPoint") == 0 || strcmp(argv[0], "gausspoint") == 0)
//     {
//         return new ElementResponse(this, 6, GaussCoordElastic);
//     }
...
// //=============================================================================
// int TwentySevenNodeBrickElastic::getResponse (int responseID, Information& eleInfo)
// {
//     switch (responseID)
//     {
...
//                 //Vector GaussCoordElastic(81+1);  // 27*3 + count
//                 //GaussCoordElastic = this->reportGaussPoint();
//                 this->computeGaussPoint();

//                 //Vector Info(109); // count * 4 +1
//                 InfoPlasticElastic(0) = GaussCoordElastic(0);
...
//                 {
//                     //                    plastify = 0;
//                     InfoPlasticElastic(i * 4 + 1) = GaussCoordElastic(i * 3 + 1); //x
//                     InfoPlasticElastic(i * 4 + 2) = GaussCoordElastic(i * 3 + 2); //y
//                     InfoPlasticElastic(i * 4 + 3) = GaussCoordElastic(i * 3 + 3); //z
//                     pl_stn = matpoint[i]->getPlasticStrainTensor();
//                     //double  p_plastc = pl_stn.p_hydrostatic();
...
//                     InfoPlasticElastic(i * 4 + 4) = q_plastc; //plastify; //Plastified?

//                 }

//                 return eleInfo.setVector( InfoPlasticElastic );
//                 //return plastify;
...
//                 InfoMomentElastic(0) = GaussCoordElastic(0);
//                 Vector Mt(3), Q(3);

//                 //Vector Zcoor(3);
//                 InfoMomentElastic(109 + 0) = GaussCoordElastic(6); //Zcoor of middle layer
...
//                 //if ( (coor(2) - GaussCoordElastic(6)) > 0.0001 )
//                 //  cerr << " Warning: Middle layer Gauss Point Z-coor. wrong...\n";

//                 //InfoMomentElastic(109+0) = GaussCoordElastic(6);
//                 InfoMomentElastic(109 + 1) = TotDis(0); //x-displacement ...Lateral displacement
//                 //InfoMomentElastic(109+3) = GaussCoordElastic(6);
//                 //InfoMomentElastic(109+6) = GaussCoordElastic(9);
//                 //cerr << " Zz " << GaussCoordElastic(3) << " " << GaussCoordElastic(6) << " "<< GaussCoordElastic(9) << endln;
...
//                             InfoMomentElastic(i * 4 + 1) = GaussCoordElastic(i * 3 + 1); //x
//                             InfoMomentElastic(i * 4 + 2) = GaussCoordElastic(i * 3 + 2); //y
//                             InfoMomentElastic(i * 4 + 3) = GaussCoordElastic(i * 3 + 3); //z
//                             InfoMomentElastic(i * 4 + 4) = sts.cval(3, 3); //Assign sigma_zz

//                             //if  (getTag() == 432) {
//                             //    sts.print();
//                             //    cerr << " rs " << rs << "\n "<< InfoMomentElastic(i*4+1) << " " ;
//                             //    cerr << InfoMomentElastic(i*4+2) << " "<< InfoMomentElastic(i*4+3)<< " sts "<< InfoMomentElastic(i*4+4) << "\n ";
//                             //}
//                             if (strcmp(tp, "ElasticIsotropic3D") == 0 )
//                             {
//                                 Mt(GP_c_t - 1) += wt(rs) * sts.cval(3, 3) * ( InfoMomentElastic(i * 4 + 1) - offset[tag] ) / height; //x--Calculating Moment_y wt(ts) / height = Area corresponding to the gauss point stress
//                                 //Mt(GP_c_t-1) += wt(rs)*sts.cval(3,3)*( InfoMomentElastic(i*4+2)-offset[tag] )/height; //y--Calculating Moment_x wt(ts) / height = Area corresponding to the gauss point stress
...
//                 //Storing avg. M and Q to InfoMomentElastic
//                 InfoMomentElastic(109 + 2) = ( Mt(0) + Mt(1) + Mt(2) ) * 0.3333;
//                 //InfoMomentElastic(109+3) = (  Q(0)+ Q(1)+ Q(2) )*0.3333;
//                 //InfoMomentElastic(109+4) = Mt(1);
//                 //InfoMomentElastic(109+5) = Q(1);
//                 //InfoMomentElastic(109+7) = Mt(2);
//                 //InfoMomentElastic(109+8) = Q(2);

//                 //cerr << " Mt " << Mt(0) << " " << Mt(1) << " "<< Mt(2) << endln;
//                 return eleInfo.setVector( InfoMomentElastic );
//             }
...
//                 int i;
//                 stresstensor sts;
//                 //Vector GaussCoordElastic(81+1);  // 8*3 + count
//                 //GaussCoordElastic = this->reportTensor("Gauss Point Coor.");

//                 //Vector InfoMomentElastic(109 + 3 ); //Z values, x-disp. and corresponding avg. moment
//                 InfoStressElastic(0) = count;
...
//                             sts = matpoint[i]->getStressTensor();
//                             InfoStressElastic(i * 6 + 1) = sts.cval(1, 1); //sigma_xx
//                             InfoStressElastic(i * 6 + 2) = sts.cval(2, 2); //sigma_yy
//                             InfoStressElastic(i * 6 + 3) = sts.cval(3, 3); //sigma_zz
//                             InfoStressElastic(i * 6 + 4) = sts.cval(1, 2); //Assign sigma_xy
//                             InfoStressElastic(i * 6 + 5) = sts.cval(1, 3); //Assign sigma_xz
//                             InfoStressElastic(i * 6 + 6) = sts.cval(2, 3); //Assign sigma_yz
//                         }
//                     }
//                 }

//                 return eleInfo.setVector( InfoStressElastic );
//             }
...
//                 Info_pq2Elastic(0) = sts.p_hydrostatic();
//                 Info_pq2Elastic(1) = sts.q_deviatoric();
//                 return eleInfo.setVector( Info_pq2Elastic );
//             }
...
//             {
//                 this->computeGaussPoint();
//                 return eleInfo.setVector( GaussCoordElastic );
//             }
...
////#############################################################################
Vector TwentySevenNodeBrickElastic::getWeightofGP(void)
{
    //int M_dim[] = {8,3,3,8};
    //int M_dim[] = {81,81};
    //tensor Mm(2,M_dim,0.0);
...
//const Matrix&
//TwentySevenNodeBrickElastic::getTangentStiff ()
//{
...
//const Matrix&
//TwentySevenNodeBrickElastic::getSecantStiff ()
//{
//  return K;
//}

//Commented by Xiaoyan     Use the form like Brick3d
//const Matrix & TwentySevenNodeBrickElastic::getDamp ()
//{
//  return C;
//}
...
//const Matrix&
//TwentySevenNodeBrickElastic::getMass ()
//{
//  int order = theQuadRule->getOrder();
//  const Vector &intPt = theQuadRule->getIntegrPointCoords();
//  const Vector &intWt = theQuadRule->getIntegrPointWeights();
...
//
//const Vector&
//TwentySevenNodeBrickElastic::getResistingForce ()
//{
//  int order = theQuadRule->getOrder();
//  const Vector &intPt = theQuadRule->getIntegrPointCoords();
//  const Vector &intWt = theQuadRule->getIntegrPointWeights();
...
//
//const Vector&
//TwentySevenNodeBrickElastic::getResistingForceIncInertia ()
//{
//  // Yet to implement
//  return P;
//}
...
//
//void
//TwentySevenNodeBrickElastic::Print (ostream &s, int flag)
//{
//  s << "TwentySevenNodeBrickElastic, element id:  " << this->getTag() << endln;
//  s << "Connected external nodes:  " << connectedExternalNodes;
...
//
//int
//TwentySevenNodeBrickElastic::displaySelf (Renderer &theViewer, int displayMode, float fact)
//{
// first determine the end points of the quad based on
// the display factor (a measure of the distorted image)
...
//
//void
//TwentySevenNodeBrickElastic::formNMatrix (double r, double s,double t)
...
// }
//void
//TwentySevenNodeBrickElastic::setJacobian (double r, double s, double t)
////Changed xi, eta to r,s and added t Xiaoyan 07/06/00
//{
//  const Vector &nd1Crds = theNodes[0]->getCrds();
//  const Vector &nd2Crds = theNodes[1]->getCrds();
...
//
//void
//TwentySevenNodeBrickElastic::formBMatrix (double r, double s, double t)
////Changed xi, eta to r,s and added t Xiaoyan  07/06/00
//{
//    B.Zero();
//
...
//double
//TwentySevenNodeBrickElastic::formDetJ (double r, double s, double t)
//{
...
double TwentySevenNodeBrickElastic::get_Gauss_p_c(short order, short point_numb)
{
...
double TwentySevenNodeBrickElastic::get_Gauss_p_w(short order, short point_numb)
{
...
int TwentySevenNodeBrickElastic::update()  //Guanzhou added May 6, 2004
{
    double r  = 0.0;
    // double rw = 0.0;
    double s  = 0.0;
    ...
    for ( short GP_c_r = 1 ; GP_c_r <= integration_order ; GP_c_r++ )
    {
        ...
        for ( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for ( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                if ( ( (matpoint[where]->matmodel)->setTrialStrainIncr( incremental_strain)) )
                {
                    cerr << "TwentySevenNodeBrickElastic::update (tag: " << this->getTag() << "), update() failed\n";
                }
            }
        }
    }
...
Vector &TwentySevenNodeBrickElastic::SurfaceShapeFunctionValues(double Xi , double Eta)
{
...
Vector &TwentySevenNodeBrickElastic::Direction_Weight(double Xi , double Eta,
...
double &TwentySevenNodeBrickElastic::SurfaceLoadValues(double Xi , double Eta, Vector Pressure)
{
...
int
TwentySevenNodeBrickElastic::CheckMesh(ofstream &checkmesh_file)
{
...
//==================================================================================
Vector *
TwentySevenNodeBrickElastic::getStress(void)
{
