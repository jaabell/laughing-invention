...
// This is a Header file that is taken from MasterEightNodeBrick.h
...
Matrix BRICK_TYPE::K(60, 60);
Vector BRICK_TYPE::P( 60 );
...
void BRICK_TYPE::computeGaussPoint(void)
{
    ...
    for ( int i = 0 ; i < 20 ; i++ )
    {
        for ( int j = 0 ; j < 20 ; j++ )
        {
            for ( int k = 0 ; k < 3 ; k++ )
            {
                for ( int l = 0 ; l < 3 ; l++ )
                {
                    K( k+3*i, l+3*j) = stifftensor(i, k, l, j);
                }
            }
        }
    }
...
////#############################################################################
//  tensor BRICK_TYPE::mass_tensor(Elastic  mmodel)
...
{
    cout<< BRICK_ELEMENT_NAME << ":: LTensor-based 27 node brick." << endl << endl;
...
int BRICK_TYPE::update() //Added by Guanzhou, May 7 2004
{
    double r  = 0.0;
    double s  = 0.0;
    double t  = 0.0;
    ...
    for ( short gp = 0; gp < Num_TotalGaussPts; gp++ )
    {
        ...
        {
            cerr << "WARNING " << BRICK_ELEMENT_NAME << "::update (tag: " << this->getTag() << "), Update Failed\n";
        }
    }
