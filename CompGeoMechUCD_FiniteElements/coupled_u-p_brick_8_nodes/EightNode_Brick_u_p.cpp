      ...
      rho_s(rs), rho_f(rf), ks(kks), kf(kkf), Q(0), Ki(0)
{
    ...
    {
        cerr << " Error EightNode_Brick_u_p::getDampTensorC123 -- permeability (x/y/z) is zero\n";
        exit(-1);
    }
    ...
    if (theMaterial == 0)
    {
        cerr << " Error! EightNode_Brick_u_p::EightNode_Brick_u_p -- failed allocate material model pointer\n";
        exit(-1);
    }
    ...
    {
        cerr << " Error! EightNode_Brick_u_p::EightNode_Brick_u_p -- zero or almost solid or fluid compressive modulus!\n";
        exit(-1);
    }
}
...
void EightNode_Brick_u_p::setDomain (Domain *theDomain)
{
    ...
    for (i = 0; i < Num_Nodes; i++)
    {
        ...
        if (theNodes[i] == 0)
        {
            cerr << "Error: EightNode_Brick_u_p: node not found in the domain" << "\n";
            return ;
        }
        ...
        if ( Ndof != Num_Dof )
        {
            cerr << "Error: EightNode_Brick_u_p: has wrong number of DOFs at its nodes " << i << " \n";
            return ;
        }
    }
...
int EightNode_Brick_u_p::commitState (void)
{
    ...
    {
        cerr << "EightNode_Brick_u_p::commitState () - failed in base class";
        return (-1);
    }
...
int EightNode_Brick_u_p::addLoad(ElementalLoad *theLoad, double loadFactor)
{
    ...
    if ( type == LOAD_TAG_ElementSelfWeight )
    {
        ...
        *Q = (this->getForceP(data) + this->getForceU(data)) * loadFactor;
    }
    else
    {
        cerr << "EightNode_Brick_u_p::addLoad() " << this->getTag() << ", load type unknown\n";
        return -1;
    }
...
int EightNode_Brick_u_p::addInertiaLoadToUnbalance(const Vector &accel)
{
    ...
    for (i = 0; i < Num_Nodes; i++)
    {
        ...
        if ( RA.Size() != Num_Dof )
        {
            cerr << "EightNode_Brick_u_p::addInertiaLoadToUnbalance(): matrix and vector sizes are incompatable \n";
            return (-1);
        }
...
const Vector &EightNode_Brick_u_p::getResistingForce ()
{
    ...
    for (i = 0; i < Num_Nodes; i++)
    {
        ...
        {
            cerr << "EightNode_Brick_u_p::getResistingForce(): matrix and vector sizes are incompatable \n";
            exit(-1);
        }
...
//=============================================================================
int EightNode_Brick_u_p::receiveSelf (int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
    // Not implemtented yet
    return 0;
}
...
//=============================================================================
void EightNode_Brick_u_p::Print(ostream &s, int flag)
{
...
int EightNode_Brick_u_p::update()
{
    ...
    static straintensor strn;
    static straintensor Symmetric_strn;
    ...
    for ( GP_c_r = 0 ; GP_c_r < Num_IntegrationPts; GP_c_r++ )
    {
        ...
        for ( GP_c_s = 0 ; GP_c_s < Num_IntegrationPts; GP_c_s++ )
        {
            ...
            for ( GP_c_t = 0 ; GP_c_t < Num_IntegrationPts; GP_c_t++ )
            {
                ...
                Symmetric_strn = strn.symmetrize11();

                if ( (theMaterial[where]->setTrialStrain(Symmetric_strn) ) )
                {
                    cerr << "EightNode_Brick_u_p::update (tag: " << this->getTag() << "), not converged\n";
                }
            }
        }
    }
...
//======================================================================
const Vector &EightNode_Brick_u_p::getForceU(const Vector &data)
{
...
//======================================================================
const Vector &EightNode_Brick_u_p::getForceP(const Vector &data)
{
...
const Matrix &EightNode_Brick_u_p::getStiff (int Ki_flag)
{
    ...
    {
        cerr << "Error EightNode_Brick_u_p::getStiff() - illegal use\n";
        exit(-1);
    }
    ...
    if (Ki == 0)
    {
        cerr << "Error EightNode_Brick_u_p::getStiff() -";
...
Vector *
EightNode_Brick_u_p::getStress(void)
{
    ...
    for ( short GP_c_r = 1 ; GP_c_r <= 2 ; GP_c_r++ )
    {
        for ( short GP_c_s = 1 ; GP_c_s <= 2 ; GP_c_s++ )
        {
            for ( short GP_c_t = 1 ; GP_c_t <= 2 ; GP_c_t++ )
            {
                i = ((GP_c_r - 1) * 2 + GP_c_s - 1) * 2 + GP_c_t - 1;
