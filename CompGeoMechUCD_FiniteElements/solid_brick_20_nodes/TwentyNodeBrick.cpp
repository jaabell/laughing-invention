...
/////////////////////////////////////////////////////////////////////////////////////
// Adapted by Sumeet, for TwentyNodeBrick to use the LTensor library for
...
DTensor2 TwentyNodeBrick::gp_coords(Num_TotalGaussPts, 3, 0.0);
DTensor2 TwentyNodeBrick::gp_weight(Num_TotalGaussPts, 3, 0.0);
Matrix TwentyNodeBrick::gauss_points(Num_TotalGaussPts, 3);
vector<float> TwentyNodeBrick::Gauss_Output_Vector(Num_TotalGaussPts*18);

Matrix TwentyNodeBrick::K(60, 60);

Vector TwentyNodeBrick::ShapeFunctionValues_in_function(8); // Nima added for surface load (July 2012)
Vector TwentyNodeBrick::J_vector_in_function(3);            // Nima added for surface load (July 2012)
double TwentyNodeBrick::SurfaceLoadValues_in_function;
      ...
      M( 60, 60), P( 60 )
{
    ...
    if (material_array == 0)
    {
        cerr << " TwentyNodeBrick::TwentyNodeBrick -- failed allocate material model pointer\n";
        exit(-1);
    }
...
//#############################################################################
//***************************************************************
const DTensor2 &TwentyNodeBrick::H_3D(double r1, double r2, double r3) const
{
...
//#############################################################################
//***************************************************************
const DTensor1 &TwentyNodeBrick::interp_poli_at(double r1, double r2, double r3) const
{
...
const DTensor2 &TwentyNodeBrick::dh_drst_at(double r1, double r2, double r3) const
{
...
////#############################################################################
const DTensor2 &TwentyNodeBrick::Jacobian_3D( const DTensor2 &dh) const
{
...
//#############################################################################
const DTensor2 &TwentyNodeBrick::Jacobian_3Dinv(const DTensor2 &dh) const
{
    const DTensor2 &N_C = Nodal_Coordinates();
...
////#############################################################################
const DTensor2 &TwentyNodeBrick::Nodal_Coordinates(void) const
{
...
////#############################################################################
const DTensor2 &TwentyNodeBrick::incr_disp(void) const
{
...
////#############################################################################
const DTensor2 &TwentyNodeBrick::total_disp(void) const
{
...
const DTensor2 &TwentyNodeBrick::nodal_forces(void) const
{
    double r  = 0.0;
    double w_r = 0.0;
    double s  = 0.0;
...
# endif
                ...
                theNodes[16] == 0 || theNodes[17] == 0 || theNodes[18] == 0 || theNodes[19] == 0 )
        {
            ...
            cerr << "FATAL ERROR TwentyNodeBrick (tag: " << this->getTag() << " ), node not found in domain\n";
            exit(-1);
        }
        ...
        {
            cerr << "FATAL ERROR TwentyNodeBrick (tag: " << this->getTag() <<
...
const Matrix &TwentyNodeBrick::getInitialStiff ()
{
    ...
    if ( Ki == 0 )
    {
        cerr << "FATAL TwentyNodeBrick::getInitialStiff() -";
...
//======================================================================
const Vector &TwentyNodeBrick::getBodyForce(double loadFactor, const Vector &data)
{
...
//======================================================================
const Vector &TwentyNodeBrick::getSurfaceForce(double loadFactor, const Vector &data)
{
...
//=============================================================================
int TwentyNodeBrick::addLoad(ElementalLoad *theLoad, double loadFactor)
{
...
int TwentyNodeBrick::addInertiaLoadToUnbalance(const Vector &accel)
{
            ...
            3 != Raccel17.Size() || 3 != Raccel18.Size() || 3 != Raccel19.Size() || 3 != Raccel20.Size()   )
    {
        ...
        cerr << "TwentyNodeBrick::addInertiaLoadToUnbalance " <<
...
//=============================================================================
const Vector TwentyNodeBrick::FormEquiBodyForce(const Vector &data)
{
    Vector bforce(60);
...
int TwentyNodeBrick::sendSelf (int commitTag, Channel &theChannel)
{
    ...
    {
        cerr << "WARNING TwentyNodeBrick::sendSelf() - " << this->getTag() << " failed to send ID idData\n";
        return -1;
    }
    ...
    if(idData( 2 )==1){
        ...
        {
            cerr << "WARNING TwentyNodeBrick::sendSelf() - " << this->getTag() << " failed to send its K\n";
            return -1;
        }
    }
    ...
    {
        cerr << "WARNING TwentyNodeBrick::sendSelf() - " << this->getTag() << " failed to send Vector floatData\n";
        return -1;
    }
    ...
    {
        cerr << "WARNING TwentyNodeBrick::sendSelf() - " << this->getTag() << " failed to send String stiffness_type\n";
        return -1;
    }
    ...
    {
        cerr << "WARNING TwentyNodeBrick::sendSelf() - " << this->getTag() << " failed to send String damping_type\n";
        return -1;
    }
    ...
    {
        cerr << "WARNING TwentyNodeBrick::sendSelf() - " << this->getTag() << " failed to send ID connectedExternalNodes\n";
        return -1;
    }
    ...
    for ( int i = 0; i < Num_TotalGaussPts; i++ )
    {
        ...
        {
            cerr << "WARNING TwentyNodeBrick::sendSelf() - " << this->getTag() << " failed to send material models\n";
            return -1;
        }
    }
    ...
    {
        cerr << "WARNING TwentyNodeBrick::sendSelf() - " << this->getTag() << " failed to send its Q\n";
        return -1;
    }
    ...
    {
        cerr << "WARNING TwentyNodeBrick::sendSelf() - " << this->getTag() << " failed to send its bf\n";
        return -1;
    }
...
int TwentyNodeBrick::receiveSelf (int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
    ...
    {
        cerr << "WARNING TwentyNodeBrick::receiveSelf() - " << this->getTag() << " failed to recieve Vector floatData\n";
        return -1;
    }
    Volume                  = floatData(0) ;
    ...
    {
        cerr << "WARNING TwentyNodeBrick::receiveSelf() - " << this->getTag() << " failed to recieve String stiffness_type\n";
        return -1;
    }
    ...
    if ( theChannel.receiveString( 0, commitTag, tmp_string ) < 0 )
    {
        cerr << "WARNING TwentyNodeBrick::receiveSelf() - " << this->getTag() << " failed to recieve String damping_type\n";
        return -1;
    }
    ...
    {
        cerr << "WARNING TwentyNodeBrick::receiveSelf() - " << this->getTag() << " failed to recieve ID connectedExternalNodes\n";
        return -1;
    }

    // cout << "TwentyNodeBrick::receiveSelf() connectedExternalNodes = " << connectedExternalNodes << "\n";
    ...
    for ( int i = 0; i < Num_TotalGaussPts; i++ )
    {
        ...
        if ( ndmat == 0 )
        {
            cerr << "TwentyNodeBrick::receiveSelf() - Broker could not create NDMaterialLT of class type " << matClassTag << "\n";
            return -1;
        }
        ...
        {
            cerr << "TwentyNodeBrick::receiveSelf() - material " << i << "failed to recv itself\n";
            return -1;
        }
    ...
    {
        cerr << "TwentyNodeBrick::receiveSelf() - failed to recv Q!\n";
        return -1;
    }
    ...
    {
        cerr << "TwentyNodeBrick::receiveSelf() - failed to recv bf!\n";
        return -1;
    }
...
int TwentyNodeBrick::getObjectSize()
{
...
{
    cout << "TwentyNodeBrick: LTensor-based 27 node brick." << endl << endl;
...
void TwentyNodeBrick::ComputeVolume()
{
    Volume = 0.0;
...
int TwentyNodeBrick::update() //Added by Guanzhou, May 7 2004
{
    ...
    for ( short gp = 0; gp < Num_TotalGaussPts; gp++ )
    {
        ...
        {
            cerr << "TwentyNodeBrick::update (tag: " << this->getTag() << "), Update Failed\n";
        }
    }
...
double  TwentyNodeBrick::SurfaceShapeFunctionValues(double Xi , double Eta, int whichcomponent)
{
...
Vector &TwentyNodeBrick::Direction_Weight(double Xi , double Eta,
...
double TwentyNodeBrick::SurfaceLoadValues(double Xi , double Eta, Vector Pressure)
{
...
int
TwentyNodeBrick::CheckMesh(ofstream &checkmesh_file)
{
    double r  = 0.0;
    double s  = 0.0;
    double t  = 0.0;
...
Vector *
TwentyNodeBrick::getStress( void )
{
...
Matrix &TwentyNodeBrick::getGaussCoordinates(void)
{
    computeGaussPoint();
    return gauss_points;
}
...
Matrix &TwentyNodeBrick::get_Gauss_p_c( )
{
...
Matrix &TwentyNodeBrick::get_Gauss_p_w()
{
...
***********************************************************************************/
const vector<float> &TwentyNodeBrick::getGaussOutput()
{
    DTensor2 stress(3, 3);
    DTensor2 strain(3, 3);
...
void TwentyNodeBrick::zeroStrain()
{
    for (int gp = 0; gp < Num_TotalGaussPts; gp++)
    {
