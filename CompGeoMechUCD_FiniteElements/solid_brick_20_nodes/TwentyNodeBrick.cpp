...
const int TwentyNodeBrick::Num_TotalGaussPts = FixedOrder * FixedOrder * FixedOrder; //Nima added for recording the Strain (02-21-2011)
...
double TwentyNodeBrick::SurfaceLoadValues_in_function; // Nima added for surface load (July 2012)
    ...
    for( short GP_c_r = 1 ; GP_c_r <= integration_order ; GP_c_r++ )
    {
        ...
        for( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                {
                    cerr << "TwentyNodeBrick::incremental_Update (tag: " << this->getTag() << "), not converged\n";
                }
    ...
    for( short GP_c_r = 1 ; GP_c_r <= integration_order ; GP_c_r++ )
    {
        ...
        for( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                //out May 2004, Guanzhou                   cerr << "TwentyNodeBrick::nodal_forces (tag: " << this->getTag() << ", not converged\n";
    ...
    for( short GP_c_r = 1 ; GP_c_r <= integration_order ; GP_c_r++ )
    {
        ...
        for( short GP_c_s = 1 ; GP_c_s <= integration_order ; GP_c_s++ )
        {
            ...
            for( short GP_c_t = 1 ; GP_c_t <= integration_order ; GP_c_t++ )
            {
                ...
                if ( ! (matpoint[where]->matmodel)->setTrialStrainIncr( incremental_strain)  )
                {
                    cerr << "TwentyNodeBrick::linearized_nodal_forces (tag: " << this->getTag() << "), not converged\n";
                }
...
//   for (int i=0; i<20; i++)
//          {
//             cout << "\nNode(" << i+1 << "): " << NodalForces(3*i) << "      " << NodalForces(3*i+1) << "      " << NodalForces(3*i+2);
//          }
//
//
//
...
//   } else
//   {
//     cerr << "TwentyNodeBrick::addLoad() - 8NodeBrick " << this->getTag() << ",load type " << type << "unknown\n";
//     return -1;
//   }
//
//
...
//{
//     if (addLoad.Size() != 60) {
//      cerr << "TwentyNodeBrick::addLoad %s\n",
