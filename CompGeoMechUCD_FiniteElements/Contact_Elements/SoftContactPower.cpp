...
//   * Output: void
SoftContactPower::SoftContactPower(int tag, double a_, double b_, int node1, int node2, double kn_, double kt_, double cn_, double ct_,  double mu_, double e1_x, double e1_y, double e1_z):
	Element(tag, ELE_TAG_SoftContactPower),
	a(a_),
	b(b_),
	kn(kn_),
	kt(kt_),
	...
	external_nodes(2)
{
	...
	// cout << "SoftContactPower Element " <<  tag << endl;
...
//   * Input: Defined by user. At least should receive an integer tag, so that base class can be initialized.
//   * Output: void
SoftContactPower::SoftContactPower():
	Element(0, ELE_TAG_SoftContactPower), //ATTENTION! Define the class tag in classTags.h
	a(0.0),
	b(0.0),
	kn(0.0),
	kt(0.0),
...
int SoftContactPower::getNumExternalNodes(void) const
{
...
const ID &SoftContactPower::getExternalNodes(void)
{
...
Node **SoftContactPower::getNodePtrs(void)
{
	return nodes;
}
...
int SoftContactPower::getNumDOF(void)
{
	return 6;
}
...
//   * Output: void
void SoftContactPower::setDomain(Domain *theDomain)
{
...
# endif
		...
		{
			cerr << "FATAL ERROR SoftContactPower (tag: " << this->getTag() << "), node not found in domain\n";
			return;
		}
		...
		{
			cerr << "FATAL ERROR SoftContactPower (tag: " << this->getTag() << "), has differing number of DOFs at its nodes\n";
			return;
		}
...
int SoftContactPower::commitState(void)
{
...
//   * Input: void
//   * Output: error flag, 0 if success
int SoftContactPower::revertToLastCommit(void)
{
	*tC = *tA;
	*g = *g_commit;
...
//   * Input: void
//   * Output: error flag, 0 if success
int SoftContactPower::revertToStart(void)
{
	// you must implement
	return 0;
}
...
//   * Input: void
//   * Output: error flag, 0 if success
int SoftContactPower::update(void)
{
	...
	if (is_in_contact){
		...
		cout << "Power Function -> " << a << "  : " << b << endl;
		kn_m = (a+1)*b*pow(0.5*(u1+u2),a);
		cout <<" kn_m "<< kn_m << endl;
		/////////////////////////////////////////////////////////////////////
...
//   * Input: void
//   * Output: void
void SoftContactPower::zeroLoad(void)
{
...
int SoftContactPower::addLoad(ElementalLoad *theLoad, double loadFactor)
{
	// optional to implement
	//
...
int SoftContactPower::addInertiaLoadToUnbalance(const Vector &accel)
{
	//cout << "SoftContactPower::addInertiaLoadToUnbalance()\n";
	return 0;
}
...
//             where nDOF = SoftContactPower::getNumDOF();
...
//   memory.
const Matrix &SoftContactPower::getTangentStiff(void)
{
	static Matrix K(6, 6);
	K.Zero();
	if (is_in_contact)
...
//             where nDOF = SoftContactPower::getNumDOF();
const Matrix &SoftContactPower::getInitialStiff(void)
{
...
//             where nDOF = SoftContactPower::getNumDOF();
const Matrix &SoftContactPower::getDamp(void)
{
	// cout << "SoftContactPower::getDamp(void)\n";
	static Matrix C(6, 6);
	C.Zero();
...
//   * Output: reference to damping stiffness matrix (of size nDOF x nDOF,
//             where nDOF = SoftContactPower::getNumDOF();
const Matrix &SoftContactPower::getMass(void)
{
...
//   * Output: reference to damping stiffness matrix (of size nDOF x nDOF,
//             where nDOF = SoftContactPower::getNumDOF();
const Vector &SoftContactPower::getResistingForce(void)
{
	R->Zero();
...
//         adds inertial terms.
const Vector &SoftContactPower::getResistingForceIncInertia(void)
{
...
int SoftContactPower::sendSelf(int commitTag, Channel &theChannel)
{
	// Useful constructs
	// int error_flag;
...
int SoftContactPower::receiveSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
	// Useful constructs
	// int error_flag;
...
void SoftContactPower::Print(ostream &s, int flag )
{
	// you must implement
	if (flag >= 0)
	{
		s << " SoftContactPower, tag =  " << this->getTag() << endl;
	}
}
...
int SoftContactPower::CheckMesh(ofstream &)
{
	return 0;
}
...
#define SoftContactPower_NOUTPUT 6

int SoftContactPower::getOutputSize() const
{
	return  SoftContactPower_NOUTPUT;
}
...
const Vector &SoftContactPower::getOutput()
{
	static Vector output_vector(SoftContactPower_NOUTPUT);
...
Matrix &SoftContactPower::getGaussCoordinates(void)
{
	// you must implement
...
void SoftContactPower::computeGap()
{
...
void SoftContactPower::initialize()
{
...
// vect3[] = vect1[] X vect2[] 
void SoftContactPower::cross_product(double vect1[],double vect2[],double vect3[]){
...
void SoftContactPower::norm(double vect[],double* nrm){
