...
//   * Output: void
SoftContactInverse::SoftContactInverse(int tag, double a_, double b_, int node1, int node2, double kn_, double kt_, double cn_, double ct_,  double mu_, double e1_x, double e1_y, double e1_z):
	Element(tag, ELE_TAG_SoftContactInverse),
	a(a_),
	b(b_),
	kn(kn_),
	kt(kt_),
	...
	external_nodes(2)
{
	...
	// cout << "SoftContactInverse Element " <<  tag << endl;
...
//   * Input: Defined by user. At least should receive an integer tag, so that base class can be initialized.
//   * Output: void
SoftContactInverse::SoftContactInverse():
	Element(0, ELE_TAG_SoftContactInverse), //ATTENTION! Define the class tag in classTags.h
	a(0.0),
	b(0.0),
	kn(0.0),
	kt(0.0),
...
int SoftContactInverse::getNumExternalNodes(void) const
{
...
const ID &SoftContactInverse::getExternalNodes(void)
{
...
Node **SoftContactInverse::getNodePtrs(void)
{
	return nodes;
}
...
int SoftContactInverse::getNumDOF(void)
{
	return 6;
}
...
//   * Output: void
void SoftContactInverse::setDomain(Domain *theDomain)
{
...
# endif
		...
		{
			cerr << "FATAL ERROR SoftContactInverse (tag: " << this->getTag() << "), node not found in domain\n";
			return;
		}
		...
		{
			cerr << "FATAL ERROR SoftContactInverse (tag: " << this->getTag() << "), has differing number of DOFs at its nodes\n";
			return;
		}
...
int SoftContactInverse::commitState(void)
{
...
//   * Input: void
//   * Output: error flag, 0 if success
int SoftContactInverse::revertToLastCommit(void)
{
	*tC = *tA;
	*g = *g_commit;
...
//   * Input: void
//   * Output: error flag, 0 if success
int SoftContactInverse::revertToStart(void)
{
	// you must implement
	return 0;
}
...
//   * Input: void
//   * Output: error flag, 0 if success
int SoftContactInverse::update(void)
{
...
//   * Input: void
//   * Output: void
void SoftContactInverse::zeroLoad(void)
{
...
int SoftContactInverse::addLoad(ElementalLoad *theLoad, double loadFactor)
{
	// optional to implement
	//
...
int SoftContactInverse::addInertiaLoadToUnbalance(const Vector &accel)
{
	//cout << "SoftContactInverse::addInertiaLoadToUnbalance()\n";
	return 0;
}
...
//             where nDOF = SoftContactInverse::getNumDOF();
...
//   memory.
const Matrix &SoftContactInverse::getTangentStiff(void)
{
	static Matrix K(6, 6);
	K.Zero();
	if (is_in_contact)
...
//             where nDOF = SoftContactInverse::getNumDOF();
const Matrix &SoftContactInverse::getInitialStiff(void)
{
...
//             where nDOF = SoftContactInverse::getNumDOF();
const Matrix &SoftContactInverse::getDamp(void)
{
	// cout << "SoftContactInverse::getDamp(void)\n";
	static Matrix C(6, 6);
	C.Zero();
...
//   * Output: reference to damping stiffness matrix (of size nDOF x nDOF,
//             where nDOF = SoftContactInverse::getNumDOF();
const Matrix &SoftContactInverse::getMass(void)
{
...
//   * Output: reference to damping stiffness matrix (of size nDOF x nDOF,
//             where nDOF = SoftContactInverse::getNumDOF();
const Vector &SoftContactInverse::getResistingForce(void)
{
	R->Zero();
...
//         adds inertial terms.
const Vector &SoftContactInverse::getResistingForceIncInertia(void)
{
...
int SoftContactInverse::sendSelf(int commitTag, Channel &theChannel)
{
	// Useful constructs
	// int error_flag;
...
int SoftContactInverse::receiveSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
	// Useful constructs
	// int error_flag;
...
void SoftContactInverse::Print(ostream &s, int flag )
{
	// you must implement
	if (flag >= 0)
	{
		s << " SoftContactInverse, tag =  " << this->getTag() << endl;
	}
}
...
int SoftContactInverse::CheckMesh(ofstream &)
{
	return 0;
}
...
#define SoftContactInverse_NOUTPUT 6

int SoftContactInverse::getOutputSize() const
{
	return  SoftContactInverse_NOUTPUT;
}
...
const Vector &SoftContactInverse::getOutput()
{
	static Vector output_vector(SoftContactInverse_NOUTPUT);
...
Matrix &SoftContactInverse::getGaussCoordinates(void)
{
	// you must implement
...
void SoftContactInverse::computeGap()
{
...
void SoftContactInverse::initialize()
{
...
// vect3[] = vect1[] X vect2[] 
void SoftContactInverse::cross_product(double vect1[],double vect2[],double vect3[]){
...
void SoftContactInverse::norm(double vect[],double* nrm){
