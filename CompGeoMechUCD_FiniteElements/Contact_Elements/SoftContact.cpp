...
//   * Output: void
SoftContact::SoftContact(int tag, int node1, int node2, double kn_, double sr_, double kt_, double cn_, double ct_,  double mu_, double e1_x, double e1_y, double e1_z):
	Element(tag, ELE_TAG_SoftContact),
	kn(kn_),
	sr(sr_),
	kt(kt_),
	cn(cn_),
...
SoftContact::SoftContact():
	Element(0, ELE_TAG_SoftContact), //ATTENTION! Define the class tag in classTags.h
	kn(0.0),
	sr(0.0),
	kt(0.0),
	cn(0.0),
...
int SoftContact::getNumExternalNodes(void) const
{
	return 2;
}
...
const ID &SoftContact::getExternalNodes(void)
{
...
Node **SoftContact::getNodePtrs(void)
{
	return nodes;
}
...
int SoftContact::getNumDOF(void)
{
	return 6;
}
...
//   * Output: void
void SoftContact::setDomain(Domain *theDomain)
{
...
int SoftContact::commitState(void)
{
...
//   * Input: void
//   * Output: error flag, 0 if success
int SoftContact::revertToLastCommit(void)
{
	*tC = *tA;
	*g = *g_commit;
	*g_prev = *g_commit;
...
//   * Input: void
//   * Output: error flag, 0 if success
int SoftContact::revertToStart(void)
{
	g_elastic->Zero();
	tC_pred_commit ->Zero();
	tA->Zero();
...
//   * Input: void
//   * Output: void
void SoftContact::zeroLoad(void)
{
...
int SoftContact::addLoad(ElementalLoad *theLoad, double loadFactor)
{
	// optional to implement
	//
...
int SoftContact::addInertiaLoadToUnbalance(const Vector &accel)
{
	//cout << "SoftContact::addInertiaLoadToUnbalance()\n";
	return 0;
}
...
//             where nDOF = SoftContact::getNumDOF();
...
//   memory.
const Matrix &SoftContact::getTangentStiff(void)
{
	static Matrix K(6, 6);
	K.Zero();
	if (is_in_contact)
...
//             where nDOF = SoftContact::getNumDOF();
const Matrix &SoftContact::getInitialStiff(void)
{
...
//             where nDOF = SoftContact::getNumDOF();
const Matrix &SoftContact::getDamp(void)
{
	// cout << "SoftContact::getDamp(void)\n";
	static Matrix C(6, 6);
	C.Zero();
...
//   * Output: reference to damping stiffness matrix (of size nDOF x nDOF,
//             where nDOF = SoftContact::getNumDOF();
const Matrix &SoftContact::getMass(void)
{
...
//   * Output: reference to damping stiffness matrix (of size nDOF x nDOF,
//             where nDOF = SoftContact::getNumDOF();
const Vector &SoftContact::getResistingForce(void)
{
	R->Zero();
...
//         adds inertial terms.
const Vector &SoftContact::getResistingForceIncInertia(void)
{
...
int SoftContact::sendSelf(int commitTag, Channel &theChannel)
{
	// Useful constructs
	// int error_flag;
...
int SoftContact::receiveSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
	// Useful constructs
	// int error_flag;
...
void SoftContact::Print(ostream &s, int flag )
{
	// you must implement
	if (flag >= 0)
	{
		s << " SoftContact, tag =  " << this->getTag() << endl;
	}
}
...
int SoftContact::CheckMesh(ofstream &)
{
	return 0;
}
...
#define SoftContact_NOUTPUT 9

int SoftContact::getOutputSize() const
{
	return  SoftContact_NOUTPUT;
}
...
const Vector &SoftContact::getOutput()
{
	static Vector output_vector(SoftContact_NOUTPUT);
...
// vect3[] = vect1[] X vect2[] 
void SoftContact::cross_product(double vect1[],double vect2[],double vect3[]){
...
void SoftContact::norm(double vect[],double* nrm){
