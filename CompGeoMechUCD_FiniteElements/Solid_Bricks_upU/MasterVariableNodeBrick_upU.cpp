...
DTensor3 BRICK_TYPE::TensorG12(Num_Nodes, 3, Num_Nodes, 0.0);;
DTensor4 BRICK_TYPE::TensorC123(Num_Nodes, 3, 3, Num_Nodes, 0.0);
DTensor2 BRICK_TYPE::TensorMsf(Num_Nodes, Num_Nodes, 0.0);
...
//======================================================================
void BRICK_TYPE::getStiffnessTensorG12()
{
...
//======================================================================
void BRICK_TYPE::getDampTensorC123()
{
...
//======================================================================
void BRICK_TYPE::getMassTensorMsf()
{
            ...
            if(connectedExternalNodes(i) != 0)
            {
                ...
                for (int j = 0; j < Num_Nodes; j++ )
                {
                    if(connectedExternalNodes(j) != 0)
                    {
                        ...
                        M(a * Num_Dof + 4, b * Num_Dof + 4) = Mtemp2;
                        M(a * Num_Dof + 5, b * Num_Dof + 5) = Mtemp2;
                        M(a * Num_Dof + 6, b * Num_Dof + 6) = Mtemp2;
                    ...
                    if(connectedExternalNodes(j) != 0)
                    {
                        for (int m = 0; m < Num_Dim; m++)
                        {
                            for (int n = 0; n < Num_Dim; n++)
                            {
                                ...
                                //C1
                                C(a * Num_Dof + m, b * Num_Dof + n) = Ctemp;

                                //C3
                                C(a * Num_Dof + m + 4, b * Num_Dof + n + 4) = Ctemp;

                                //C2 and C2^T
                                C(a * Num_Dof + m, b * Num_Dof + n + 4) = - Ctemp;
                                C(b * Num_Dof + n + 4, a * Num_Dof + m) = - Ctemp;
                            }
                        }
                        b++;
                    }
            ...
            if(connectedExternalNodes(i) != 0)
            {
                ...
                for (int j = 0; j < Num_Nodes; j++ )
                {
                    if(connectedExternalNodes(j) != 0)
                    {
                        for (int m = 0; m < Num_Dim; m++)
                        {
                            KFluid(a * Num_Dof + m, b * Num_Dof + 3) = -TensorG12(i, m, j) * (alpha - poro);
                            KFluid(b * Num_Dof + 3, a * Num_Dof + m) = -TensorG12(i, m, j) * (alpha - poro);
                        }
                        b++;
                    }
                }
            ...
            if(connectedExternalNodes(i) != 0)
            {
                ...
                for (int j = 0; j < Num_Nodes; j++ )
                {
                    if(connectedExternalNodes(j) != 0)
                    {
                        for (int m = 0; m < Num_Dim; m++)
                        {
                            KFluid(a * Num_Dof + m + 4, b * Num_Dof + 3) = -TensorG12(i, m, j) * poro;
                            KFluid(b * Num_Dof + 3, a * Num_Dof + m + 4) = -TensorG12(i, m, j) * poro;
                        }
                        b++;
                    }
                }
        ...
        for (int  i = 0 ; i < Num_Nodes; i++ )
        {
            if(connectedExternalNodes(i) != 0)
            {
                ...
                for (int  j = 0; j < Num_Nodes; j++ )
                {
                    if(connectedExternalNodes(j) != 0)
                    {
                        KFluid(i * Num_Dof + 3, j * Num_Dof + 3) = TensorMsf(i, j) * (-oneOverQ);
                        b++;
                    }
                }
                a++;
...
const Vector &BRICK_TYPE::getBodyForce( double loadFactor, const Vector &data )
{
    ...
    for (int i = 0; i < Num_Nodes; i++)
    {
        if(connectedExternalNodes(i) != 0)
        {
            ...
            {
                P(a * Num_Dof + j) = Pexf(i) * bf(j) * (1.0 - poro) * rho_s;
                P(a * Num_Dof + j + 4) = Pexf(i) * bf(j) * poro * rho_f;
            }
            a++;
        }
    }
    ...
    for ( int k = 0; k < 3; k++ )
    {
        ...
        for ( int j = 0; j < nodes_in_brick; j++ )
        {
            ...
            for ( int i = 0; i < Order*Order; i++ )
            {
                ...
                P( r * Num_Dof + 4 + k ) = P( r * Num_Dof + 4 + k ) + poro * LoadValue * J_vector( k ) * ShapeFunctionValues * Weight(i, 0) * Weight(i, 1); // fluid part
            }
        }
    }
