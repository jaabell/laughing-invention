...
Vector BRICK_TYPE::ShapeFunctionValues_in_function( 8 );  // Nima added for surface load (July 2012)
    ...
    if(not is_KFluid_computed)
    {
        ...
        for (int i = 0 ; i < Num_Nodes; i++ )
        {
            for (int j = 0; j < Num_Nodes; j++ )
            {
                for (int m = 0; m < Num_Dim; m++)
                {
                    KFluid(i * Num_Dof + m, j * Num_Dof + 3) = -TensorG12(i, m, j) * (alpha - poro);
                    KFluid(j * Num_Dof + 3, i * Num_Dof + m) = -TensorG12(i, m, j) * (alpha - poro);
                }
            }
        }
        ...
        for (int i = 0 ; i < Num_Nodes; i++ )
        {
            for (int j = 0; j < Num_Nodes; j++ )
            {
                for (int m = 0; m < Num_Dim; m++)
                {
                    KFluid(i * Num_Dof + m + 4, j * Num_Dof + 3) = -TensorG12(i, m, j) * poro;
                    KFluid(j * Num_Dof + 3, i * Num_Dof + m + 4) = -TensorG12(i, m, j) * poro;
                }
            }
        }
    ...
    for (int k = 0; k < 3; k++)
    {
        ...
        for (int j = 0; j < 8; j++)
        {
            ...
            for (int i = 0; i < Order*Order; i++)
            {
                ...
                J_vector = Direction_Weight(GsPts(i, 0) , GsPts(i, 1), coordnode1, coordnode2,coordnode3, coordnode4, coordnode5, coordnode6, coordnode7, coordnode8);
                ...
                NodalForces( r * Num_Dof + 4 + k ) = NodalForces( r * Num_Dof + 4 + k ) + poro * LoadValue * J_vector( k ) * ShapeFunctionValues * Weight(i, 0) * Weight(i, 1); // fluid part
            }
        }
    }
...
Vector &BRICK_TYPE::Direction_Weight(double Xi , double Eta,
