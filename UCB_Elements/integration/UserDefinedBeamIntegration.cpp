    ...
    pts(nIP), wts(nIP)
{
    for (int i = 0; i < nIP; i++)
    {
        ...
        {
            std::cerr << "UserDefinedBeamIntegration::UserDefinedBeamIntegration -- point lies outside [0,1]" << endln;
        }

        if (wt(i) < 0.0 || wt(i) > 1.0)
        {
            std::cerr << "UserDefinedBeamIntegration::UserDefinedBeamIntegration -- weight lies outside [0,1]" << endln;
        }
...
void
UserDefinedBeamIntegration::Print(ostream& s, int flag)
{
    s << "UserDefined" << endln;
    s << " Points: " << pts;
    s << " Weights: " << wts;
