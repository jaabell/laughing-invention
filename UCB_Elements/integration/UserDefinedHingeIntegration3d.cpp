    ...
    E(ee), A(aa), Iz(iiz), Iy(iiy), G(gg), J(jj)
{
    ...
    for (i = 0; i < npL; i++)
    {
        ...
        {
            std::cerr << "UserDefinedHingeIntegration3d::UserDefinedHingeIntegration3d -- point lies outside [0,1]" << endln;
        }

        if (wtL(i) < 0.0 || wtL(i) > 1.0)
        {
            std::cerr << "UserDefinedHingeIntegration3d::UserDefinedHingeIntegration3d -- weight lies outside [0,1]" << endln;
        }
...
void
UserDefinedHingeIntegration3d::Print(ostream& s, int flag)
{
    s << "UserHinge3d" << endln;
    s << " E = " << E;
    s << " A = " << A;
