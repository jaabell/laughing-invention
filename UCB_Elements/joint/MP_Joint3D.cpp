      ...
      RotationNode(0), DisplacementNode(0)
{
    ...
    if( thisDomain == NULL )
    {
        std::cerr << "WARNING MP_Joint3D(): Specified domain does not exist";
        std::cerr << "Domain = 0\n";
        return;
    }
    ...
    if (ConstrainedNode == NULL)
    {
        ...
        std::cerr << "MP_Joint3D::MP_Joint3D: nodeConstrained: ";
        std::cerr << nodeConstrained << "does not exist in model\n";
        exit(0);
    }
    ...
    if (RetainedNode == NULL)
    {
        ...
        std::cerr << "MP_Joint3D::MP_Joint3D: nodeRetained: ";
        std::cerr << nodeRetained << "does not exist in model\n";
        exit(0);
    }
    ...
    if (RotationNode == NULL)
    {
        ...
        std::cerr << "MP_Joint3D::MP_Joint3D: nodeRotation: ";
    ...
    {
        std::cerr << "MP_Joint3D::MP_Joint3D: nodeDisplacement: ";
    ...
    {
        std::cerr << "MP_Joint3D::MP_Joint3D - mismatch in dimnesion\n dimension not supported by this type of constraint";
        return;
    }
    ...
    if ( Length0 <= 1.0e-12 )
    {
        std::cerr << "MP_Joint3D::MP_Joint3D - The constraint length is zero\n";
    }
    ...
    {
        std::cerr << "MP_Joint3D::MP_Joint3D - the normal vector for the rotation mode or the displacement mode is zero\n";
    }
    ...
    {
        std::cerr << "MP_Joint3D::MP_Joint3D - ran out of memory \ncan not generate ID for nodes\n";
        exit(-1);
    }
...
const Matrix& MP_Joint3D::getConstraint(void)
{
    if (constraint == 0)
    {
        std::cerr << "MP_Joint3D::getConstraint - no Matrix was set\n";
        exit(-1);
    }
    ...
    if ( LargeDisplacement == 2 )
    {
        ...
        if ( NewLength < 1e-12 )
        {
            std::cerr << "MP_Joint3D::applyConstraint : length of rigid link is too small or zero";
        }
...
void MP_Joint3D::Print(ostream& s, int flag )
{
