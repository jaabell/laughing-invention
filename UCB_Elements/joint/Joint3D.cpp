      ...
      TheDomain(0), numDof(0), nodeDbTag(0), dofDbTag(0)
{
    ...
    for ( i = 0 ; i < 6 ; i++)
    {
        ...
        if (theNodes[i] == NULL)
        {
            std::cerr << "WARNING Joint3D::setDomain(): Nd" << (i + 1) << ": ";
            std::cerr << ExternalNodes(i) << "does not exist in model for element \n";
            return;
        }
    }
    ...
    {
        std::cerr << "WARNING Joint3D::setDomain(): has incorrect space dimension \n";
        std::cerr << "                                    space dimension not supported by Joint3D";
        return;
    }
    ...
    {
        std::cerr << "WARNING Joint3D::Joint3D: has incorrect degrees of freedom \n";
        std::cerr << "                                    DOF not supported by Joint3D";
        return;
    }
    ...
    {
        std::cerr << "WARNING Joint3D::(): can not construct a shear block over external nodes\n";
    ...
    if ( theNodes[6] == NULL )
    {
        std::cerr << "Joint3D::Joint3D - Unable to generate new nodes , out of memory\n" ;
    }
    else
    {
    ...
    if ( &springx == NULL )
    {
        std::cerr << "ERROR Joint3D::Joint3D(): The rotational spring in y'z' plane does not exist ";
        exit(-1);
    }
    else
    {
        theSprings[0] = springx.getCopy();
    }

    if ( &springy == NULL )
    {
        std::cerr << "ERROR Joint3D::Joint3D(): The rotational spring in x'z' plane does not exist ";
        exit(-1);
    }
    else
    {
        theSprings[1] = springy.getCopy();
    }

    if ( &springz == NULL )
    {
        std::cerr << "ERROR Joint3D::Joint3D(): The rotational spring in x'y' plane does not exist ";
        exit(-1);
    }
    else
    {
        theSprings[2] = springz.getCopy();
    }
    ...
    for ( i = 0 ; i < 3 ; i++ )
    {
        ...
        {
            std::cerr << "ERROR Joint3D::Joint3D(): Can not make copy of uniaxial materials, out of memory ";
            exit(-1);
        }
    }
    ...
    {
        std::cerr << "WARNING Joint3D::Joint3D(): can not generate ForJoint MP at node 1\n";
        return;
    }
    ...
    {
        std::cerr << "WARNING Joint3D::Joint3D(): can not generate ForJoint MP at node 2\n";
        return;
    }
    ...
    {
        std::cerr << "WARNING Joint3D::Joint3D(): can not generate ForJoint MP at node 3\n";
        return;
    }
    ...
    {
        std::cerr << "WARNING Joint3D::Joint3D(): can not generate ForJoint MP at node 4\n";
        return;
    }
...
int Joint3D::addMP_Joint(Domain *theDomain, int mpNum, int RetNodeID, int ConNodeID, int RotNodeID, int Rdof, int DspNodeID, int Ddof, int LrgDispFlag )
{
    ...
    if (Temp_MP == NULL)
    {
        std::cerr << "Joint3D::addMP_Joint - WARNING ran out of memory for MP_Joint3D MP_Constraint ";
        return -1;
    }
    ...
    {
        std::cerr << "Joint3D::addMP_Joint - WARNING could not add equalDOF MP_Constraint to domain ";
        delete Temp_MP;
        return -2;
    }
...
int Joint3D::update(void)
{
    ...
    for ( int i = 0 ; i < 3 ; i++ )
    {
        if ( theSprings[i] != NULL )
        {
            result = theSprings[i]->setTrialStrain(dispC(i + 6));
        }
...
int Joint3D::commitState()
{
    ...
    for ( int i = 0 ; i < 3 ; i++ )
    {
        if ( theSprings[i] != NULL )
        {
            result = theSprings[i]->commitState();
        }
...
int Joint3D::revertToLastCommit()
{
    ...
    for ( int i = 0 ; i < 3 ; i++ )
    {
        if ( theSprings[i] != NULL )
        {
            result = theSprings[i]->revertToLastCommit();
        }
...
int Joint3D::revertToStart(void)
{
    ...
    for ( int i = 0 ; i < 3 ; i++ )
    {
        if ( theSprings[i] != NULL )
        {
            result = theSprings[i]->revertToStart();
        }
...
const Matrix &Joint3D::getInitialStiff(void)
{
    ...
    for ( int i = 0 ; i < 3 ; i++ )
    {
        ...
        if ( theSprings[i] != NULL )
        {
            Kintial[i] = theSprings[i]->getTangent();
        }
    }
...
const Vector &Joint3D::getResistingForce()
{
    ...
    for ( int i = 0 ; i < 3 ; i++ )
    {
        ...
        if ( theSprings[i] != NULL )
        {
            Force[i] = theSprings[i]->getStress();
        }
    }
