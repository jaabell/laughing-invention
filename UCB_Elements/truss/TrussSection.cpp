      ...
      L(0.0), rho(r), theSection(0)
{
    ...
    {
        std::cerr << "FATAL TrussSection::TrussSection - failed to create an ID of correct size\n";
        exit(-1);
    }
...
TrussSection::setDomain(Domain *theDomain)
{
    ...
    if ((theNodes[0] == 0) || (theNodes[1] == 0))
    {
        ...
        std::cerr << " for truss with id " << this->getTag() << endln;
    ...
    if (dofNd1 != dofNd2)
    {
        std::cerr << "WARNING TrussSection::setDomain(): nodes " << Nd1 << " and " <<
    ...
    {
        std::cerr << "WARNING TrussSection::setDomain cannot handle " << dimension <<
    ...
    if (dimension == 1)
    {
        ...
        if (L == 0.0)
        {
            std::cerr << "WARNING TrussSection::setDomain() - truss " << this->getTag() << " has zero length\n";
            return;
        }
    ...
    if (theLoad == 0)
    {
        std::cerr << "TrussSection::setDomain - truss " << this->getTag() <<
...
TrussSection::commitState()
{
    ...
    {
        std::cerr << "TrussSection::commitState () - failed in base class";
    }
...
TrussSection::addInertiaLoadToUnbalance(const Vector &accel)
{
    ...
    {
        std::cerr << "TrussSection::addInertiaLoadToUnbalance " <<
...
TrussSection::sendSelf(int commitTag, Channel &theChannel)
{
    ...
    if (res < 0)
    {
        std::cerr << "WARNING TrussSection::sendSelf() - " << this->getTag() << " failed to send Vector\n";
        return -1;
    }
    ...
    if (res < 0)
    {
        std::cerr << "WARNING TrussSection::sendSelf() - " << this->getTag() << " failed to send ID\n";
        return -2;
    }
    ...
    if (res < 0)
    {
        std::cerr << "WARNING TrussSection::sendSelf() - " << this->getTag() << " failed to send its Section\n";
        return -3;
    }
