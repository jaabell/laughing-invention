...
ZeroLengthSection::setDomain(Domain* theDomain)
{
    ...
    if (theNodes[0] == 0 || theNodes[1] == 0)
    {
        ...
        std::cerr << "model for ZeroLengthSection with id " << this->getTag() << endln;
    ...
    {
        std::cerr << "ZeroLengthSection::setDomain() -- element only works for 3 (2d) or 6 (3d) dof per node\n";
    }
...
{
    std::cerr << "ZeroLengthSection::addLoad - load type unknown for truss with tag: " << this->getTag() << endln;
    return -1;
}
...
ZeroLengthSection::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    if (res < 0)
    {
        std::cerr << "ZeroLengthSection::sendSelf -- failed to send ID data\n";
    ...
    if (res < 0)
    {
        std::cerr << "ZeroLengthSection::sendSelf -- failed to send transformation Matrix\n";
        return res;
    }
    ...
    if (res < 0)
    {
        std::cerr << "ZeroLengthSection::sendSelf -- failed to send Section\n";
        return res;
    }
...
ZeroLengthSection::setUp(int Nd1, int Nd2, const Vector& x, const Vector& yp)
{
    ...
    {
        std::cerr << "ZeroLengthSection::setUp -- failed to create an ID of correct size\n";
        exit(-1);
    }
    ...
    {
        std::cerr << "ZeroLengthSection::setUp -- incorrect dimension of orientation vectors\n";
    }
    ...
    if (xn == 0 || yn == 0 || zn == 0)
    {
        std::cerr << "ZeroLengthSection::setUp -- invalid vectors to constructor\n";
    }
...
ZeroLengthSection::setTransformation(void)
{
    ...
    if (A == 0)
    {
        std::cerr << "ZeroLengthSection::setTransformation -- failed to allocate transformation Matrix\n";
    }
