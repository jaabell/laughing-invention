                              ...
                              TclModelBuilder* theBuilder)
{
    ...
    if (argc < 9)
    {
        std::cerr << "WARNING too few arguments " <<
    ...
    {
        std::cerr << "WARNING invalied eleTag " << argv[2] <<
    ...
    {
        std::cerr << "WARNING invalied iNode " << argv[3] <<
    ...
    {
        std::cerr << "WARNING invalid jNode " << argv[4] <<
                  "- element ZeroLength eleTag? iNode? jNode? " <<
                  "-mat matID1? ... -dir dirMat1? .. " <<
                  "<-orient x1? x2? x3? y1? y2? y3?>\n";
        return TCL_ERROR;
    ...
    {
        std::cerr << "WARNING expecting -mat flag %s %s %s %s\n" << argv[5] <<
                  "- element ZeroLength eleTag? iNode? jNode? " <<
                  "-mat matID1? ... -dir dirMat1? .. " <<
                  "<-orient x1? x2? x3? y1? y2? y3?>\n";
    ...
    {
        std::cerr << "WARNING no -dirn flag encountered " <<
                  "- element ZeroLength eleTag? iNode? jNode? " <<
                  "-mat matID1? ... -dir dirMat1? .. " <<
                  "<-orient x1? x2? x3? y1? y2? y3?>\n";
        return TCL_ERROR;
    ...
    if (numMat == 0)
    {
        std::cerr << "WARNING no materials specified " <<
                  "- element ZeroLength eleTag? iNode? jNode? " <<
    ...
    for (int i = 0; i < numMat; i++)
    {
        ...
        {
            std::cerr << "WARNING invalid matID " << argv[argi] <<
                      "- element ZeroLength eleTag? iNode? jNode? " <<
                      "-mat matID1? ... -dir dirMat1? .. " <<
                      "<-orient x1? x2? x3? y1? y2? y3?>\n";
            delete [] theMats;
        ...
        else
        {
            ...
            if (theMat == 0)
            {
                std::cerr << "WARNING no material " << matID <<
    ...
    {
        std::cerr << "WARNING expecting -dirn flag " << argv[argi] <<
                  "- element ZeroLength eleTag? iNode? jNode? " <<
                  "-mat matID1? ... -dir dirMat1? .. " <<
                  "<-orient x1? x2? x3? y1? y2? y3?>\n";
    ...
    if ((argi + numMat) > argc)
    {
        std::cerr << "WARNING not enough directions provided for ele " << eleTag <<
                  "- element ZeroLength eleTag? iNode? jNode? " <<
                  "-mat matID1? ... -dir dirMat1? .. " <<
                  "<-orient x1? x2? x3? y1? y2? y3?>\n";
    ...
    for (int j = 0; j < numMat; j++)
    {
        ...
        {
            std::cerr << "WARNING invalid directiion " << argv[argi] <<
                      "- element ZeroLength eleTag? iNode? jNode? " <<
                      "-mat matID1? ... -dir dirMat1? .. " <<
                      "<-orient x1? x2? x3? y1? y2? y3?>\n";
    ...
    while (argi < argc)
    {
        if (strcmp(argv[argi], "-orient") == 0)
        {
            if (argc < (argi + 7))
            {
                std::cerr << "WARNING not enough paramaters after -orient flag for ele " << eleTag <<
                          "- element ZeroLength eleTag? iNode? jNode? " <<
                          "-mat matID1? ... -dir dirMat1? .. " <<
                          "<-orient x1? x2? x3? y1? y2? y3?>\n";
                delete [] theMats;
            ...
            else
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    {
                        std::cerr << "WARNING invalid -orient value for ele  " << eleTag << argv[i] <<
                                  "- element ZeroLength eleTag? iNode? jNode? " <<
                                  "-mat matID1? ... -dir dirMat1? .. " <<
                                  "<-orient x1? x2? x3? y1? y2? y3?>\n";
                        delete [] theMats;
                ...
                for (int j = 0; j < 3; j++)
                {
                    if (Tcl_GetDouble(interp, argv[argi], &value) != TCL_OK)
                    {
                        std::cerr << "WARNING invalid -orient value for ele  " <<
                                  eleTag << argv[argi] <<
                                  "- element ZeroLength eleTag? iNode? jNode? " <<
                                  "-mat matID1? ... -dir dirMat1? .. " <<
                                  "<-orient x1? x2? x3? y1? y2? y3?>\n";
                                     ...
                                     TclModelBuilder* theBuilder)
{
    ...
    {
        std::cerr << "WARNING invalid secTag " << argv[5] <<
                  "- element zeroLengthSection eleTag? iNode? jNode? " <<
                  "secTag? " <<
                  "<-orient x1? x2? x3? y1? y2? y3?>\n";
        return TCL_ERROR;
    ...
    while (argi < argc)
    {
        if (strcmp(argv[argi], "-orient") == 0)
        {
            if (argc < (argi + 7))
            {
                std::cerr << "WARNING not enough paramaters after -orient flag for ele " <<
    ...
    if (theSection == 0)
    {
        std::cerr << "zeroLengthSection -- no section with tag " << secTag << " exists in Domain\n";
        return TCL_ERROR;
    }
                                       ...
                                       TclModelBuilder* theBuilder)
{
    ...
    if (argc < 11)
    {
        std::cerr << "ZeroLengthContact2D::WARNING too few arguments " <<
    ...
    if (Tcl_GetInt(interp, argv[2], &eleTag) != TCL_OK)
    {
        std::cerr << "ZeroLengthContact2D::WARNING invalied eleTag " << argv[2] << "\n";
        return TCL_ERROR;
    }
    ...
    if (Tcl_GetInt(interp, argv[3], &iNode) != TCL_OK)
    {
        std::cerr << "ZeroLengthContact2D::WARNING invalied iNode " << argv[3] <<  "\n";
    ...
    if (Tcl_GetInt(interp, argv[4], &jNode) != TCL_OK)
    {
        std::cerr << "ZeroLengthContact2D::WARNING invalid jNode " << argv[4] << "\n" ;
        return TCL_ERROR;
    }
    ...
    {
        std::cerr << "ZeroLengthContact2D::WARNING invalid Kn " << argv[5] << "\n" ;
        return TCL_ERROR;
    }

    if (Tcl_GetDouble(interp, argv[6], &Kt) != TCL_OK)
    {
        std::cerr << "ZeroLengthContact2D::WARNING invalid Kt " << argv[6] << "\n" ;
        return TCL_ERROR;
    }

    if (Tcl_GetDouble(interp, argv[7], &fs) != TCL_OK)
    {
        std::cerr << "ZeroLengthContact2D::WARNING invalid fs " << argv[7] << "\n" ;
        return TCL_ERROR;
    }
    ...
    {
        std::cerr << "ZeroLengthContact2D:: expecting " <<
    ...
    for (int i = 0; i < 2; i++)
    {
        if (Tcl_GetDouble(interp, argv[argi], &value) != TCL_OK)
        {
            std::cerr << "ZeroLengthContact2D:: invalid -normal value for ele " <<
                      eleTag <<
                      "- element ZeroLengthContact2D eleTag? iNode? jNode? Kn? Kt? fs? -normal Nx? Ny? \n" ;
                                       ...
                                       TclModelBuilder* theBuilder)
{
    ...
    if (argc < 10)
    {
        std::cerr << "ZeroLengthContact3D::WARNING too few arguments " <<
    ...
    if (Tcl_GetInt(interp, argv[2], &eleTag) != TCL_OK)
    {
        std::cerr << "ZeroLengthContact3D::WARNING invalied eleTag " << argv[2] << "\n";
        return TCL_ERROR;
    }
    ...
    if (Tcl_GetInt(interp, argv[3], &iNode) != TCL_OK)
    {
        std::cerr << "ZeroLengthContact3D::WARNING invalied iNode " << argv[3] <<  "\n";
    ...
    if (Tcl_GetInt(interp, argv[4], &jNode) != TCL_OK)
    {
        std::cerr << "ZeroLengthContact3D::WARNING invalid jNode " << argv[4] << "\n" ;
        return TCL_ERROR;
    }
    ...
    if (Tcl_GetDouble(interp, argv[5], &Kn) != TCL_OK)
    {
        std::cerr << "ZeroLengthContact3D::WARNING invalid Kn " << argv[5] << "\n" ;
        return TCL_ERROR;
    }

    if (Tcl_GetDouble(interp, argv[6], &Kt) != TCL_OK)
    {
        std::cerr << "ZeroLengthContact3D::WARNING invalid Kt " << argv[6] << "\n" ;
        return TCL_ERROR;
    }

    if (Tcl_GetDouble(interp, argv[7], &fs) != TCL_OK)
    {
        std::cerr << "ZeroLengthContact3D::WARNING invalid fs " << argv[7] << "\n" ;
        return TCL_ERROR;
    }

    if (Tcl_GetDouble(interp, argv[8], &c) != TCL_OK)
    {
        std::cerr << "ZeroLengthContact3D::WARNING invalid c " << argv[8] << "\n" ;
        return TCL_ERROR;
    }
    ...
    {
        std::cerr << "ZeroLengthContact3D::WARNING invalid direction " << argv[9] << "\n" ;
        return TCL_ERROR;
    }
    ...
    if (dir == 0)
    {
        if (argc == 12)
        {
            ...
            {
                std::cerr << "ZeroLengthContact3D::WARNING invalid originX " << argv[9] << "\n" ;
                return TCL_ERROR;
            }

            if (Tcl_GetDouble(interp, argv[11], &originY) != TCL_OK)
            {
                std::cerr << "ZeroLengthContact3D::WARNING invalid originY " << argv[10] << "\n" ;
                return TCL_ERROR;
            }
        }
    }
                                ...
                                TclModelBuilder* theBuilder)
{
    ...
    if (argc < 6)
    {
        std::cerr << "WARNING too few arguments %s %s %s\n" <<
    ...
    if (Tcl_GetInt(interp, argv[2], &eleTag) != TCL_OK)
    {
        std::cerr << "WARNING invalied eleTag " <<
    ...
    if (Tcl_GetInt(interp, argv[3], &iNode) != TCL_OK)
    {
        std::cerr << "WARNING invalied iNode " <<
                  argv[3] <<
    ...
    {
        std::cerr << "WARNING invalid NDTag %s %s %s %s\n" <<
    ...
    if (argc > 6 && strcmp(argv[6], "-orient") != 0)
    {
        ...
        {
            std::cerr << "WARNING invalid NDTag " <<
                      argv[5] << "- element zeroLengthND eleTag? iNode? jNode? " <<
                      "NDTag? <1DTag?> <-orient x1? x2? x3? y1? y2? y3?>\n";
            return TCL_ERROR;
        }
        ...
        if (the1DMat == 0)
        {
            std::cerr << "WARNING UniaxialMaterial " << uniTag << " not found in model, proceeding without\n";
        }
    ...
    if (theNDMat == 0)
    {
        std::cerr << "zeroLengthND -- no NDMaterial with tag " << NDTag << " exists in Domain\n";
        return TCL_ERROR;
    }
