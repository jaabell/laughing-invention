    ...
    end1Ptr(0), end2Ptr(0), theNDMaterial(0), the1DMaterial(0), order(0)
{
    ...
    if (theNDMaterial == 0)
    {
        std::cerr << "ZeroLengthND::zeroLengthND-- failed to get copy of NDMaterial\n";
        exit(-1);
    }
    ...
    if (order < 2 || order > 3)
    {
        std::cerr << "ZeroLengthND::  -- NDMaterial not of order 2 or 3\n";
        exit(-1);
    }
    ...
    end1Ptr(0), end2Ptr(0), theNDMaterial(0), the1DMaterial(0), order(0)
{
    ...
    if (theNDMaterial == 0)
    {
        std::cerr << "ZeroLengthND::  -- failed to get copy of NDMaterial\n";
        exit(-1);
    }
    ...
    if (the1DMaterial == 0)
    {
        std::cerr << "ZeroLengthND""ZeroLengthND -- failed to get copy of UniaxialMaterial\n";
        exit(-1);
    }
    ...
    if (order != 2)
    {
        std::cerr << "ZeroLengthND::ZeroLengthND-- NDMaterial not of order 2\n";
        exit(-1);
    }
...
ZeroLengthND::setDomain(Domain* theDomain)
{
    ...
    if (end1Ptr == 0 || end2Ptr == 0)
    {
        if (end1Ptr == 0)
        {
            std::cerr << "ZeroLengthND::setDomain()-- Nd1 does not exist in model\n";
        }
        else
        {
            std::cerr << "ZeroLengthND::setDomain -- Nd2 does not exist in model\n";
        }
    ...
    if (dofNd1 != dofNd2)
    {
        std::cerr << "ZeroLengthND::setDomain -- nodes have differing dof's at end\n";
        end1Ptr = 0;
        end2Ptr = 0;
        return;
    }
    ...
    {
        std::cerr << "ZeroLengthND::setDomain -  element only works for 3 (2d) or 6 (3d) dof per node\n";
        end1Ptr = 0;
        end2Ptr = 0;
        return;
    }
    ...
    if (L > LENTOL * vm)
    {
        std::cerr << "ZeroLengthND::setDomain -- Element has L=, which is greater than the tolerance\n";
    }
...
{
    std::cerr << "ZeroLength::addLoad - load type unknown for ZeroLengthND\n";
...
ZeroLengthND::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    if (res < 0)
    {
        std::cerr << "ZeroLengthND::sendSelf() -- failed to send ID data\n";
        return res;
    }
    ...
    if (res < 0)
    {
        std::cerr << "ZeroLengthND::sendSelf -- failed to send transformation Matrix\n";
        return res;
    }
    ...
    if (res < 0)
    {
        std::cerr << "ZeroLengthND::  -- failed to send NDMaterial\n";
        return res;
    }
    ...
    if (the1DMaterial != 0)
    {
        ...
        if (res < 0)
        {
            std::cerr << "ZeroLengthND::sendSelf-- failed to send UniaxialMaterial";
            return res;
        }
    }
...
ZeroLengthND::receiveSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
    ...
    if (res < 0)
    {
        std::cerr << "zeroLengthND::revbSelf -- failed to receive transformation Matrix\n";
        return res;
    }
    ...
    if (theNDMaterial == 0)
    {
        std::cerr << "ZeroLengthND::  -- failed to allocate new NDMaterial\n";
        return -1;
    }
    ...
    if (res < 0)
    {
        std::cerr << "ZeroLengthND::  -- failed to receive NDMaterial\n";
        return res;
    }
    ...
    if (idData(4) == 1)
    {
        ...
        if (the1DMaterial == 0)
        {
            std::cerr << "ZeroLengthND::  -- failed to allocate new UniaxialMaterial\n";
            return -1;
        }
        ...
        if (res < 0)
        {
            std::cerr << "ZeroLengthND::  -- failed to receive UniaxialMaterial\n";
            return res;
        }
    }
...
Response*
ZeroLengthND::setResponse(const char** argv, int argc, Information& eleInformation)
{
    ...
    */
            ...
            || (strcmp(argv[0], "globalForces") == 0) || (strcmp(argv[0], "globalforces") == 0))
    {
        ...
        /*
                    ...
                    for (int i=0; i<numDOFperNode; i++) {
                        ...
                        eleInformation.tag("ResponseType", outputData);
                    }
    ...
    else if (strcmp(argv[0], "material") == 0)
    {
        ...
        theResponse = theNDMaterial->setResponse(&argv[1], argc - 1, eleInformation);

        if ((theResponse == 0) && (the1DMaterial != 0))
        {
            theResponse = the1DMaterial->setResponse(&argv[1], argc - 1, eleInformation);
        }
    }

    //    eleInformation.endTag();
    return theResponse;
}
...
ZeroLengthND::setUp(int Nd1, int Nd2, const Vector& x, const Vector& yp)
{
    ...
    {
        std::cerr << "ZeroLengthND::setUp -- failed to create an ID of correct size\n";
        exit(-1);
    }
    ...
    {
        std::cerr << "ZeroLengthND -- incorrect dimension of orientation vectors\n";
        exit(-1);
    }
    ...
    if (xn == 0 || yn == 0 || zn == 0)
    {
        std::cerr << "ZeroLengthND::setUP -- invalid vectors to constructor\n";
        exit(-1);
    }
...
ZeroLengthND::setTransformation(void)
{
    ...
    if (A == 0)
    {
        std::cerr << "ZeroLengthND::setTransformation -- failed to allocate transformation Matrix\n";
        exit(-1);
    }
