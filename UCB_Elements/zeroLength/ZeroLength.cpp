      ...
      numMaterials1d(1), theMaterial1d(0), dir1d(0), t1d(0)
{
    ...
    {
        std::cerr << "FATAL ZeroLength::ZeroLength - failed to create a 1d  material or direction array\n";
        exit(-1);
    }
    ...
    {
        std::cerr << "FATAL ZeroLength::ZeroLength - failed to get a copy of material " << theMat.getTag() << endln;
        exit(-1);
    }
      ...
      numMaterials1d(n1dMat), theMaterial1d(0), dir1d(0), t1d(0)
{
    ...
    *dir1d = direction;
    ...
    for (int i = 0; i < numMaterials1d; i++)
    {
        ...
        {
            std::cerr << "FATAL ZeroLength::ZeroLength - failed to get a copy of material " << theMat[i]->getTag() << endln;
            exit(-1);
        }
    }
      ...
      dir1d(0), t1d(0)
{
    ...
    {
        std::cerr << "FATAL ZeroLength::ZeroLength - failed to create an ID of correct size\n";
    }
}
...
ZeroLength::setDomain(Domain* theDomain)
{
    ...
    if ( theNodes[0] == 0 || theNodes[1] == 0 )
    {
        if (theNodes[0] == 0)
        {
            std::cerr << "WARNING ZeroLength::setDomain() - Nd1: " << Nd1 << " does not exist in ";
        }
        else
        {
            std::cerr << "WARNING ZeroLength::setDomain() - Nd2: " << Nd2 << " does not exist in ";
        }

        std::cerr << "model for ZeroLength ele: " << this->getTag() << endln;
    ...
    if ( dofNd1 != dofNd2 )
    {
        std::cerr << "WARNING ZeroLength::setDomain(): nodes " << Nd1 << " and " << Nd2 <<
    ...
    if (L > LENTOL * vm)
        std::cerr << "WARNING ZeroLength::setDomain(): Element " << this->getTag() << " has L= " << L <<
    ...
    {
        std::cerr << "WARNING ZeroLength::setDomain cannot handle " << dimension <<
...
ZeroLength::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    if (res < 0)
    {
        std::cerr <<  "ZeroLength::sendSelf -- failed to send transformation Matrix\n";
        return res;
    }
    ...
    else
    {
        ...
        if (res < 0)
        {
            std::cerr << " ZeroLength::sendSelf -- failed to send classTags ID\n";
            return res;
        }
        ...
        for (i = 0; i < numMaterials1d; i++)
        {
            ...
            if (res < 0)
            {
                std::cerr << "ZeroLength::sendSelf -- failed to send Material1d " << i << endln;
                   ...
                   const Vector& yp )
{
    ...
    if (connectedExternalNodes.Size() != 2)
    {
        std::cerr << "FATAL ZeroLength::setUp - failed to create an ID of correct size\n";
    }
    ...
    {
        std::cerr << "FATAL ZeroLength::setUp - incorrect dimension of orientation vectors\n";
    }
    ...
    if (xn == 0 || yn == 0 || zn == 0)
    {
        std::cerr << "FATAL ZeroLength::setUp - invalid vectors to constructor\n";
    }
...
ZeroLength::checkDirection( ID& dir ) const
{
    for ( int i = 0; i < dir.Size(); i++)
        if ( dir(i) < 0 || dir(i) > 5 )
        {
            std::cerr << "WARNING ZeroLength::checkDirection - incorrect direction " << dir(i) << " is set to 0\n";
            dir(i) = 0;
        }
}
                       ...
                       int   numMat )
{
    ...
    if (t1d == 0)
    {
        std::cerr << "FATAL ZeroLength::setTran1d - can't allocate 1d transformation matrix\n";
    }
