    ...
    connectedExternalNodes(4), load(0), Ki(0)
{
    ...
    for ( i = 0 ;  i < 4; i++ )
    {
        ...
        {
            std::cerr << "ShellMITC4::constructor - failed to get a material of type: ShellSection\n";
        } //end if
...
void  ShellMITC4::setDomain( Domain* theDomain )
{
    ...
    for ( i = 0; i < 4; i++ )
    {
        ...
        if (nodePointers[i] == 0)
        {
            std::cerr << "ShellMITC4::setDomain - no node " << connectedExternalNodes(i);
...
int  ShellMITC4::commitState( )
{
    ...
    {
        std::cerr << "ShellMITC4::commitState () - failed in base class";
    }
...
{
    std::cerr << "ShellMITC4::addLoad - load type unknown for ele with tag: " << this->getTag() << endln;
    return -1;
}
...
int  ShellMITC4::sendSelf (int commitTag, Channel& theChannel)
{
    ...
    if (res < 0)
    {
        std::cerr << "WARNING ShellMITC4::sendSelf() - " << this->getTag() << " failed to send ID\n";
        return res;
    }
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        if (res < 0)
        {
            std::cerr << "WARNING ShellMITC4::sendSelf() - " << this->getTag() << " failed to send its Material\n";
            return res;
        }
    }
