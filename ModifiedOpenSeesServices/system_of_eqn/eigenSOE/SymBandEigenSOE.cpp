...
SymBandEigenSOE::setSize(Graph& theGraph)
{
    ...
    if (newSize > Asize)   // we have to get another space for A
    {
        ...
        if (A == 0)
        {
            cerr << "SymBandEigenSOE::setSize() -- ran out of memory for A, size = " <<
    ...
    if (solverOK < 0)
    {
        cerr << "SymBandEigenSOE::setSize() -- solver failed in setSize()\n";
...
SymBandEigenSOE::addA(const Matrix& m, const ID& id, double fact)
{
    ...
    {
        cerr << "SymBandEigenSOE::addA() -- Matrix and ID not of similar sizes,\n";
        return -1;
    }
...
SymBandEigenSOE::addM(const Matrix& m, const ID& id, double fact)
{
    ...
    if (M == 0 || Msize != size)
    {
        ...
        if (M == 0)
        {
            cerr << "WARNING SymBandEigenSOE::addM() - out of memory creating M for size: " << size << endln;
            return -1;
        }
    ...
    if (idSize != m.noRows() && idSize != m.noCols())
    {
        cerr << "WARING: SymBandEigenSOE::addM() -- Matrix and ID not of similar sizes!!\n";
        return -1;
    }
    ...
    {
        cerr << "WARNING SymBandEigenSOE::addM() - m passed was not diagonal, only diagonal entries added\n";
    }
