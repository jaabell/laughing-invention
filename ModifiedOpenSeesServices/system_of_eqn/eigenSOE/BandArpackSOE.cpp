...
BandArpackSOE::setSize(Graph &theGraph)
{
    ...
    if (newSize > Asize)   // we have to get another space for A
    {
        ...
        if (A == 0)
        {
            cout << "WARNING BandGenLinSOE::BandGenLinSOE :";
            cout << " ran out of memory for A (size,super,sub) (";
            cout << size << ", " << numSuperD << ", " << numSubD << ") \n";
            Asize = 0;
            size = 0;
            numSubD = 0;
            numSuperD = 0;
    ...
    if (solverOK < 0)
    {
        cout << "WARNING:BandArpackSOE::setSize :";
        cout << " solver failed setSize()\n";
        return solverOK;
    }
...
BandArpackSOE::addA(const Matrix &m, const ID &id, double fact)
{
    ...
    {
        cout << "BandArpackSOE::addA()	- Matrix and ID not of similar sizes\n";
        return -1;
    }
