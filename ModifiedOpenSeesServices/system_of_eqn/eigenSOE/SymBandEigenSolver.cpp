...
SymBandEigenSolver::solve(int nModes)
{
    if (theSOE == 0)
    {
        cerr << "SymBandEigenSolver::solve() -- no EigenSOE has been set yet\n";
        return -1;
    }
    ...
    if (M != 0)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            if (M[i] == 0.0)
            {
                ...
                cerr << "SymBandEigenSolver::solve() - M matrix singular\n";
                return -1;
            }
            else
            {
    ...
    if (info < 0)
    {
        cerr << "SymBandEigenSolver::solve() -- invalid argument number " << -info << " passed to LAPACK dsbevx\n";
        return info;
    }

    if (info > 0)
    {
        cerr << "SymBandEigenSolver::solve() -- LAPACK dsbevx returned error code " << info << endln;
        return -info;
    }

    if (m < numModes)
    {
        cerr << "SymBandEigenSolver::solve() -- LAPACK dsbevx only computed " << m << " eigenvalues, " <<
...
SymBandEigenSolver::getEigenvector(int mode)
{
    ...
    {
        cerr << "SymBandEigenSolver::getEigenVector() -- mode " << mode << " is out of range (1 - "
             << numModes << ")\n";
    ...
    {
        cerr << "SymBandEigenSolver::getEigenVector() -- eigenvectors not yet computed\n";
        eigenV->Zero();
    }
...
SymBandEigenSolver::getEigenvalue(int mode)
{
    if (mode < 1 || mode > numModes)
    {
        cerr << "SymBandEigenSolver::getEigenvalue() -- mode " << mode << " is out of range (1 - "
             << numModes << ")\n";
    ...
    {
        cerr << "SymBandEigenSolver::getEigenvalue() -- eigenvalues not yet computed\n";
        return 0.0;
    }
}
...
SymBandEigenSolver::setSize()
{
    ...
    if (eigenV == 0 || eigenV->Size() != size)
    {
        ...
        if (eigenV == 0 || eigenV->Size() != size)
        {
            cerr << "SymBandEigenSolver::ssetSize() -- ran out of memory for eigenvector of size " << size << endln;
            return -2;
        }
    }
