...
int FullGenEigenSOE::setSize(Graph& theGraph)
{
    ...
    if (newSize > Asize)
    {
        ...
        if (A == 0)
        {
            cerr << "WARNING FullGenEigenSOE::setSize() - "
...
int FullGenEigenSOE::addA(const Matrix& m, const ID& id, double fact)
{
    ...
    {
        cerr << "FullGenEigenSOE::addA() - Matrix and ID not of similar sizes\n";
        return -1;
    }
...
int FullGenEigenSOE::addM(const Matrix& m, const ID& id, double fact)
{
    ...
    if (idSize != m.noRows() && idSize != m.noCols())
    {
        cerr << "FullGenEigenSOE::addM() - Matrix and ID not of similar sizes\n";
        return -1;
    }
