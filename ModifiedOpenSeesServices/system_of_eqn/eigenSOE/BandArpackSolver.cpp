...
int
BandArpackSolver::solve(int numModes)
{
    return solve(numModes, true, true);
}
...
BandArpackSolver::solve(int numModes, bool generalized, bool findSmallest)
{
    ...
    {
        cout << "BandArpackSolver::solve(int numMode, bool generalized) - only solves generalized problem\n";
        return -1;
    }

    if (theSOE == 0)
    {
        cout << "WARNING BandGenLinLapackSolver::solve(void)- ";
        cout << " No LinearSOE object has been set\n";
        return -1;
    }
    ...
    {
        cout << "WARNING BandGenLinLapackSolver::solve(void)- ";
        cout << " iPiv not large enough - has setSize() been called?\n";
        return -1;
    }
    ...
    if ( ierr != 0 )
    {
        cout << " BandArpackSolver::Error in dgbtrf_ " << endln;
        return -1;
    }
    ...
    while (1)
    {
        ...
        if (ido == -1)
        {
            ...
            if (ierr != 0)
            {
                cout << "BandArpackSolver::Error with dgbtrs_ 1" << endln;
                exit(0);
            }
            continue;
        }
        else if (ido == 1)
        {
            ...
            if (ierr != 0)
            {
                cout << "BandArpackSolver::Error with dgbtrs_ 2" << endln;
                exit(0);
            }
            continue;
        }
    ...
    if (info < 0)
    {
        cout << "BandArpackSolver::Error with _saupd info = " << info << endln;
        switch (info)
        {
        ...
        case -1:
            ...
            break;
        case -2:
            cout << "NEV must be positive.\n";
            break;
        case -3:
            cout << "NCV must be greater than NEV and less than or equal to N.\n";
            break;
        case -4:
            cout << "The maximum number of Arnoldi update iterations allowed";
            break;
        case -5:
            cout << "WHICH must be one of 'LM', 'SM', 'LA', 'SA' or 'BE'.\n";
            break;
        case -6:
            cout << "BMAT must be one of 'I' or 'G'.\n";
            break;
        case -7:
            cout << "Length of private work array WORKL is not sufficient.\n";
            break;
        case -8:
            cout << "Error return from trid. eigenvalue calculation";
            cout << "Informatinal error from LAPACK routine dsteqr.\n";
            break;
        case -9:
            cout << "Starting vector is zero.\n";
            break;
        case -10:
            cout << "IPARAM(7) must be 1,2,3,4,5.\n";
            break;
        case -11:
            cout << "IPARAM(7) = 1 and BMAT = 'G' are incompatable.\n";
            break;
        case -12:
            cout << "IPARAM(1) must be equal to 0 or 1.\n";
            break;
        case -13:
            cout << "NEV and WHICH = 'BE' are incompatable.\n";
            break;
        case -9999:
            cout << "Could not build an Arnoldi factorization.";
    ...
    else
    {
        if (info == 1)
        {
            cout << "BandArpackSolver::Maximum number of iteration reached." << endln;
        }
        else if (info == 3)
        {
            cout << "BandArpackSolver::No Shifts could be applied during implicit,";
            cout << "Arnoldi update, try increasing NCV." << endln;
        }
        ...
        if (iparam[4] > 0)
        {
            ...
            if (info != 0)
            {
                cout << "BandArpackSolver::Error with dseupd_" << info;
                switch (info)
                {
                ...
                case -6:
                    ...
                    break;
                case -7:
                    cout << " Length of private work WORKL array is not sufficient.\n";
                    break;
                case -8:
                    cout << " Error return from trid. eigenvalue calculation";
                    cout << "Information error from LAPACK routine dsteqr.\n";
                    break;
                case -9:
                    cout << " Starting vector is zero.\n";
                    break;
                case -10:
                    ...
                    break;
                case -11:
                    cout << " IPARAM(7) = 1 and BMAT = 'G' are incompatibl\n";
                    break;
                case -12:
                    cout << " NEV and WHICH = 'BE' are incompatible.\n";
                    break;
                case -14:
                    cout << " DSAUPD did not find any eigenvalues to sufficient accuracy.\n";
                    break;
                case -15:
                    cout << " HOWMNY must be one of 'A' or 'S' if RVEC = .true.\n";
                    break;
                case -16:
                    cout << " HOWMNY = 'S' not yet implemented\n";
                    break;
                default:
                    ;
                }
...
BandArpackSolver::getEigenvector(int mode)
{
    ...
    {
        cout << "BandArpackSOE::getEigenvector() - mode is out of range(1 - nev)";
        eigenV->Zero();
        return *eigenV;
    }
    ...
    {
        cout << "BandArpackSOE::getEigenvalue() - eigenvectors not yet determined";
        eigenV->Zero();
    }
...
BandArpackSolver::getEigenvalue(int mode)
{
    if (mode <= 0 || mode > theNev)
    {
        cout << "BandArpackSOE::getEigenvalue() - mode is out of range(1 - nev)";
        return -1;
    }
    ...
    {
        cout << "BandArpackSOE::getEigenvalue() - eigenvalues not yet determined";
        return -2;
    }
}
...
BandArpackSolver::setSize()
{
    ...
    if (iPivSize < size)
    {
        ...
        if (iPiv == 0)
        {
            cout << "WARNING BandGenLinLapackSolver::setSize() ";
            cout << " - ran out of memory for iPiv of size ";
            cout << theSOE->size << endln;
            return -1;
        }
        else
        {
    ...
    if (eigenV == 0 || eigenV->Size() != size)
    {
        ...
        {
            cout << "WARNING BandGenLinLapackSolver::setSize() ";
            cout << " - ran out of memory for eigenVector of size ";
            cout << theSOE->size << endln;
            return -2;
        }
    }
