...
BandArpackSolver::solve(void)
{
    ...
    if ( ierr != 0 )
    {
        cerr << " BandArpackSolver::Error in dgbtrf_ " << endln;
        return -1;
    }
    ...
    while (1)
    {
        ...
        if (ido == -1)
        {
            ...
            if (ierr != 0)
            {
                cerr << "BandArpackSolver::Error with dgbtrs_ 1" << endln;
                exit(0);
            }
        ...
        else if (ido == 1)
        {
            ...
            if (ierr != 0)
            {
                cerr << "BandArpackSolver::Error with dgbtrs_ 2" << endln;
                exit(0);
            }
    ...
    if (info < 0)
    {
        cerr << "BandArpackSolver::Error with _saupd info = " << info << endln;
    ...
    else
    {
        if (info == 1)
        {
            cerr << "BandArpackSolver::Maximum number of iteration reached." << endln;
        }
        else if (info == 3)
        {
            cerr << "BandArpackSolver::No Shifts could be applied during implicit,";
            cerr << "Arnoldi update, try increasing NCV." << endln;
        }
