...
DistributedSparseGenRowLinSOE::setSize(Graph& theGraph)
{
    ...
    for (int i = 0; i < numRows; i++)
    {
        ...
        if ((theVertex = myGraph.getVertexPtr(row)) == 0)
        {
            ...
            {
                cerr << "WARNING DistributedDistributedSparseGenRowLinSOE::addVertex - error adding vertex\n";
            }
        }
    }
    ...
    if (newNNZ > Asize)   // we have to get more space for A and colA
    {
        ...
        if (A == 0 || colA == 0)
        {
            cerr << "WARNING DistributedSparseGenRowLinSOE::DistributedSparseGenRowLinSOE :";
    ...
    if (n != 0)
    {
        ...
        for (int a = 0; a < numAllRows; a++)
        {
            ...
            if (theVertex == 0)
            {
                cerr << "WARNING:DistributedSparseGenRowLinSOE::setSize :";
                cerr << " vertex " << a << " not in graph! - n set to 0\n";
                n = 0;
                return -1;
            }
...
DistributedSparseGenRowLinSOE::addA(const Matrix& m, const ID& id, double fact)
{
    ...
    {
        cerr << "DistributedSparseGenRowLinSOE::addA() ";
        cerr << " - Matrix and ID not of similar ns\n";
        return -1;
    }
    ...
    for (int i = 0; i < idSize; i++)
    {
        ...
        if (row < n && row >= 0)
        {
            ...
            if (localRow == -1)
            {
                cerr << "DistributedSparseGenRowLinSOE::addA() - you goofed\n";
            }
...
DistributedSparseGenRowLinSOE::addB(const Vector& v, const ID& id, double fact)
{
    ...
    if (idSize != v.Size() )
    {
        cerr << "DistributedSparseGenRowLinSOE::addB() ";
        cerr << " - Vector and ID not of similar ns\n";
        return -1;
    }
...
DistributedSparseGenRowLinSOE::setB(const Vector& v, double fact)
{
    ...
    if (v.Size() != n)
    {
        ...
        cerr << " incomptable ns " << n << " and " << v.Size() << endln;
        return -1;
    }
...
DistributedSparseGenRowLinSOE::getX(void)
{
    if (vectX == 0)
    {
        cerr << "FATAL DistributedSparseGenRowLinSOE::getX - vectX == 0";
        exit(-1);
    }
...
DistributedSparseGenRowLinSOE::getB(void)
{
    if (vectB == 0)
    {
        cerr << "FATAL DistributedSparseGenRowLinSOE::getB - vectB == 0";
        exit(-1);
    }
...
DistributedSparseGenRowLinSOE::setDistributedSparseGenRowSolver(DistributedSparseGenRowLinSolver& newSolver)
{
    ...
    if (n != 0)
    {
        ...
        if (solverOK < 0)
        {
            cerr << "WARNING:DistributedSparseGenRowLinSOE::setSolver :";
