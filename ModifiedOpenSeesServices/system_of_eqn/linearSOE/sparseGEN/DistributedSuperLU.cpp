...
{
    cerr << "DISTRIBUTED SuperLU: row " << npRow << " col " << npCol << endln;
}
...
DistributedSuperLU::solve(void)
{
    if (theSOE == 0)
    {
        cerr << "WARNING DistributedSuperLU::solve(void)- ";
...
DistributedSuperLU::setChannels(int nChannels, Channel** theC)
{
    ...
    if (theChannels == 0)
    {
        cerr << "DistributedSuperLU::sendSelf() - failed to allocate channel array of size: " <<
             numChannels << endln;
        return -1;
    }
...
DistributedSuperLU::sendSelf(int cTag, Channel& theChannel)
{
    ...
    if (res < 0)
    {
        cerr << "WARNING DistributedSuperLU::sendSelf() - failed to send data\n";
        return -1;
    }
