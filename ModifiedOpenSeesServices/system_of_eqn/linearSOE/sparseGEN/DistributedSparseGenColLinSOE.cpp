...

{
    ...
    fprintf(stderr, "DistributedSparseGenColLinSOE::\n");
}
...
DistributedSparseGenColLinSOE::setSize(Graph& theGraph)
{
    ...
    fprintf(stderr, "DistributedSparseGenColLinSOE::setSize\n");
    int result = 0;
    int oldSize = size;
    ...
    if (solverOK < 0)
    {
        cerr << "WARNING:DistributedSparseGenColLinSOE::setSize :";
...
{
    fprintf(stderr, "DistributedSparseGenColLinSOE::solve() - : %d %f\n", processID, vectX->Norm());
    static ID result(1);
...
DistributedSparseGenColLinSOE::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    if (processID == 0)
    {
        ...
        if (found == false)
        {
            ...
            {
                cerr << "DistributedSparseGenColLinSOE::sendSelf() - failed to allocate channel array of size: " <<
            ...
            if (localCol == 0)
            {
                cerr << "DistributedSparseGenColLinSOE::sendSelf() - failed to allocate id array of size: " <<
                     nextNumChannels << endln;
                return -1;
            }
