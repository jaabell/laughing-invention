...
DistributedDiagonalSOE::setSize(Graph& theGraph)
{
    ...
    {
        cerr << "ERROR DistributedDiagonalSOE::setSize() - ";
        cerr << " ran out of memory for size: " << size << endln;
    ...
    if (theModel == 0)
    {
        cerr << "WARNING DistributedDiagonalSOE::setSize - no AnalysisModel\n";
    }
    else
    {
    ...
    if (solverOK < 0)
    {
        cerr << "WARNING DistributedDiagonalSOE::setSize :";
...
DistributedDiagonalSOE::addB(const Vector& v, const ID& id, double fact)
{
    ...
    if (idSize != v.Size() )
    {
        cerr << "DistributedDiagonalSOE::addB() -";
...
DistributedDiagonalSOE::setB(const Vector& v, double fact)
{
    ...
    if (v.Size() != size)
    {
        cerr << "WARNING DistributedDiagonalSOE::setB() -";
...
DistributedDiagonalSOE::getX(void)
{
    if (vectX == 0)
    {
        cerr << "FATAL DistributedDiagonalSOE::getX - vectX == 0";
        exit(-1);
    }
...
DistributedDiagonalSOE::getB(void)
{
    if (vectB == 0)
    {
        cerr << "FATAL DistributedDiagonalSOE::getB - vectB == 0";
        exit(-1);
    }
...
DistributedDiagonalSOE::setDiagonalSolver(DistributedDiagonalSolver& newSolver)
{
    ...
    if (size != 0)
    {
        ...
        if (solverOK < 0)
        {
            cerr << "WARNING:DistributedDiagonalSOE::setSolver :";
...
DistributedDiagonalSOE::sendSelf(int cTag, Channel& theChannel)
{
    ...
    if (res < 0)
    {
        cerr << "WARNING DistributedDiagonalSOE::sendSelf() - failed to send data\n";
        return -1;
    }
