...
DistributedBandSPDLinSOE::setSize(Graph& theGraph)
{
    ...
    if (newSize != Asize)   // we have to get another space for A
    {
        ...
        if (A == 0)
        {
            cerr << "WARNING DistributedBandSPDLinSOE::DistributedBandSPDLinSOE :";
            cerr << " ran out of memory for A (size, half_band) (";
            cerr << size << ", " << half_band << "\n";
            Asize = 0;
            size = 0;
            half_band = 0;
    ...
    if (processID == 0)
    {
        ...
        if (solverOK < 0)
        {
            cerr << "WARNING:DistributedBandSPDLinSOE::setSize :";
...
DistributedBandSPDLinSOE::setB(const Vector& v, double fact)
{
    ...
    if (v.Size() != size)
    {
        cerr << "WARNING DistributedBandGenLinSOE::setB() -";
...
DistributedBandSPDLinSOE::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    if (processID == 0)
    {
        ...
        if (found == false)
        {
            ...
            {
                cerr << "DistributedBandSPDLinSOE::sendSelf() - failed to allocate channel array of size: " <<
            ...
            if (localCol == 0)
            {
                cerr << "DistributedBandSPDLinSOE::sendSelf() - failed to allocate id array of size: " <<
                     nextNumChannels << endln;
                return -1;
            }
    ...
    if (res < 0)
    {
        cerr << "WARNING DistributedBandSPDLinSOE::sendSelf() - failed to send data\n";
        return -1;
    }
...
int
DistributedBandSPDLinSOE::receiveSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
    ID idData(1);
    ...
    if (res < 0)
    {
        cerr << "WARNING DistributedBandSPDLinSOE::receiveSelf() - failed to send data\n";
        return -1;
    }
