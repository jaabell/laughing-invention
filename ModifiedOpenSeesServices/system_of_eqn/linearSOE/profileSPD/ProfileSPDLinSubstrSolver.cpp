...
ProfileSPDLinSubstrSolver::condenseA(int numInt)
{
    ...
    */
    ...
    cerr << "ProfileSPDLinSubstrSolver::condenseA  numDOF: " << size << "  numInt: " << numInt << "  numExt: " << size - numInt << endln;
...
ProfileSPDLinSubstrSolver::condenseRHS(int numInt, Vector* v)
{
    ...
    if (theSOE->numInt != numInt)
    {
        ...
        cerr << " numInt " << theSOE->numInt << endln;
        return -1;
    }
    ...
    /*
      for (int iii=0; iii<theSOE->size; iii++) {
      cerr << "COLUMN " << iii << " Biii -> " << B[iii] << endln;
      }
      cerr << endln;
      */
...
int
ProfileSPDLinSubstrSolver::receiveSelf(int cTag,
                                    Channel& theChannel,
