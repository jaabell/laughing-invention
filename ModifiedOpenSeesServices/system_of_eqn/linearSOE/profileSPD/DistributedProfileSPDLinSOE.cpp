...
DistributedProfileSPDLinSOE::setSize(Graph& theGraph)
{
    ...
    else
    {
        ...
        if (size != Bsize)
        {
            ...
            if (iDiagLoc == 0)
            {
                cerr << "WARNING DistributedProfileSPDLinSOE::setSize() : ";
    ...
    if (profileSize != Asize)
    {
        ...
        if (A == 0)
        {
            cerr << "DistributedProfileSPDLinSOE::ProfileSPDLinSOE :";
...
DistributedProfileSPDLinSOE::addA(const Matrix& m, const ID& id, double fact)
{
    ...
    {
        cerr << "DistributedProfileSPDLinSOE::addA()	- Matrix and ID not of similar sizes\n";
        return -1;
    }
...
DistributedProfileSPDLinSOE::addB(const Vector& v, const ID& id, double fact)
{
    ...
    if (idSize != v.Size() )
    {
        cerr << "DistributedProfileSPDLinSOE::addB() -";
...
DistributedProfileSPDLinSOE::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    if (processID == 0)
    {
        ...
        if (found == false)
        {
            ...
            {
                cerr << "DistributedProfileSPDLinSOE::sendSelf() - failed to allocate channel array of size: " <<
            ...
            if (localCol == 0)
            {
                cerr << "DistributedProfileSPDLinSOE::sendSelf() - failed to allocate id array of size: " <<
                     nextNumChannels << endln;
                return -1;
            }
    ...
    if (res < 0)
    {
        cerr << "WARNING DistributedProfileSPDLinSOE::sendSelf() - failed to send data\n";
        return -1;
    }
