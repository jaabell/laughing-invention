...
DistributedBandGenLinSOE::setSize(Graph& theGraph)
{
    ...
    if (newSize != Asize)   // we have to get another space for A
    {
        ...
        if (A == 0)
        {
            cerr << "WARNING DistributedBandGenLinSOE::DistributedBandGenLinSOE :";
    ...
    if (solverOK < 0)
    {
        cerr << "WARNING:DistributedBandGenLinSOE::setSize :";
...
DistributedBandGenLinSOE::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    if (res < 0)
    {
        cerr << "WARNING DistributedBandGenLinSOE::sendSelf() - failed to send data\n";
        return -1;
    }
