...
{
    cerr << "NEESData::getDbTag(void) - should not be called\n";
    return -1;
}
...
{
    cerr << "NEESData::commitState(int commitTag) - should not be called\n";
    return -1;
}
...
{
    cerr << "NEESData::sendMsg(int dataTag, int commitTag,  - should not be called\n";
    return -1;
}
...
NEESData::sendMatrix(int dataTag, int commitTag,
                     ...
                     ChannelAddress* theAddress)
{
    cerr << "NEESData::sendMatrix(int dataTag, int commitTag,  - should not be called\n";
    return -1;
}
...
NEESData::sendVector(int dataTag, int commitTag,
                     ...
                     ChannelAddress* theAddress)
{
    cerr << "NEESData::sendVector(int dataTag, int commitTag,  - should not be called\n";
    return -1;
}
...
NEESData::sendID(int dataTag, int commitTag,
                 ...
                 ChannelAddress* theAddress)
{
    cerr << "NEESData::sendID(int dataTag, int commitTag,  - should not be called\n";
    return -1;
}
...
NEESData::createTable(const char* tableName, int numColumns, char* columns[])
{
    ...
    {
        cerr << "NEESData::insertData - out of memory creating copy of string " << fileName << endln;
        return -1;
    }
    ...
    for (int i = 0; i < numTables; i++)
    {
        ...
        {
            cerr << "WARNING: NEESData::createTable - table already exists: " << copyTableName << endln;
            return -1;
        }
    ...
    if (numColumns <= 0)
    {
        cerr << "WARNING: NEESData::createTable - number of data columns < 0 for table name: " << copyTableName << endln;
        delete [] fileName;
    ...
    if (!dataFile.is_open())
    {
        cerr << "NEESData::createTable - failed to open file: " << fileName << endln;
        delete [] fileName;
        delete [] copyTableName;
        return -1;
    }
    ...
    {
        cerr << "NEESData::createData - failed to open file: " << copyTableName << endln;
        delete [] fileName;
        delete [] copyTableName;
        res = -1;
    }
    ...
    if (nextTable == 0)
    {
        cerr << "NEESData::createData - out of memory creating Table structure for table: " << copyTableName << endln;
        res = -1;
    }
                     ...
                     int commitTag, const Vector& data)
{
    ...
    if ( t == 0)
    {
        cerr << "NEESData::insertData - table: " << tableName << " has not been created\n";
        return -1;
    }

    if (t->numColumns != data.Size())
    {
        cerr << "NEESData::insertData - incorrect number of columns for table: " << tableName << "\n";
        return -2;
    }
    ...
    {
        cerr << "NEESData::insertData - failed to open file: " << fileName << endln;
        return -1;
    }
