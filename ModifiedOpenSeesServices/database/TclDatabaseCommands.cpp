               ...
               FEM_ObjectBroker& theBroker)
{
    ...
    if (argc < 2)
    {
        cerr << "WARNING need to specify a Database type; valid type File, MySQL, BerkeleyDB \n";
        return TCL_ERROR;
    }
    ...
    if (theDatabase == 0)
    {
        cerr << "WARNING ran out of memory - database " << argv[1] << endln;
        return TCL_ERROR;
    }
...
save(ClientData clientData, Tcl_Interp* interp, int argc, TCL_Char** argv)
{
    ...
    {
        cerr << "WARNING - save could not read commitTag " << argv[1] << endln;
        return TCL_OK;
    }
...
restore(ClientData clientData, Tcl_Interp* interp, int argc, TCL_Char** argv)
{
    ...
    if (Tcl_GetInt(interp, argv[1], &commitTag) != TCL_OK)
    {
        cerr << "WARNING - restore could not read commitTag " << argv[1] << endln;
        return TCL_OK;
    }
