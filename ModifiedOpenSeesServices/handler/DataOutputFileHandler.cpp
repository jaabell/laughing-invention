      ...
      fileName( 0 ), theEchoMode( theEMode ), theOpenMode( theOMode ), numColumns( -1 )
{
    if ( theFileName != 0 )
    {
        ...
        if ( fileName == 0 )
        {
            cerr << "DataOutputFileHandler::DataOutputFileHandler() - out of memory\n";
        }
        else
        {
    ...
    {
        cerr << "DataOutputFileHandler::DataOutputFileHandler() - setFile() failed\n";
...
DataOutputFileHandler::open( char** dataDescription, int numData )
{
    if ( fileName == 0 )
    {
        cerr << "DataOutputFileHandler::open() - no filename passed in constructor\n";
        return -1;
    }

    if ( dataDescription == 0 )
    {
        cerr << "DataOutputFileHandler::open() - no column description data passed\n";
        return -1;
    }

    if ( numData < 0 )
    {
        cerr << "DataOutputFileHandler::open() - numColumns (" << numData << ") < 0\n";
        return -1;
    }
    else
    {
    ...
    else if ( theEchoMode == XML_FILE )
    {
        ...
        if ( xmlFileName == 0 )
        {
            cerr << "DataOutputFileHandler::open - out of memory creating copy of string " << xmlFileName << endln;
            return -1;
        }
        ...
        {
            cerr << "DataOutputFileHandler::open - failed to open cml file: " << xmlFileName << endln;
            delete [] xmlFileName;
            res = -1;
        }
...
DataOutputFileHandler::write( Vector& data )
{
    ...
    {
        cerr << "DataOutputFileHandler::write() - no filename or data description has been set\n";
        return -1;
    }
    ...
    {
        cerr << fileName;
        cerr << "DataOutputStreamHandler::write() - Vector not of correct size\n";
        return -1;
    }
...
DataOutputFileHandler::sendSelf( int commitTag, Channel& theChannel )
{
    ...
    {
        cerr << "DataOutputFileHandler::sendSelf() - failed to send id data\n";
        return -1;
    }
    ...
    if ( fileNameLength != 0 )
    {
        ...
        {
            cerr << "DataOutputFileHandler::sendSelf() - failed to send message\n";
            return -1;
        }
    }
