...
int main(int argc, char** argv)
{
    ...
    cerr << " *******************************************************************\n";
    cerr << "                        DataOutputDatabaseHandler unit test\n";
    cerr << " *******************************************************************\n\n";
    cerr << " The DataOutputFile class provides implements 2 methods \n";
    cerr << "    open() - to open a connection to the o/p stream";
    cerr << "    write() - to send the contents of a vector to this stream\n\n";
    cerr << " *******************************************************************\n";
    ...
    cerr << "TEST: Creation of DataOutputFile object - will create 4 objects 1,2,3 and 4 (3,4 have no assigned database)\n";
    ...
    {
        cerr << "PASS: Creation of DataOutputDatabaseHandler object\n\n";
    }
    else
    {
        cerr << "FAIL: Creation of DataOutputDatabaseHandler object\n\n";
        exit(-1);
    }
    ...
    cerr << "Now We Test open()\n\t 1. invoke open() on 1 and 2\n";

    cerr << "Invoking on handler 1\n";
    result1 = theHandler1->open(columns, numColumns);
    cerr << "Invoking on handler 2\n";
    result2 = theHandler2->open(columns, numColumns);

    if (result1 == 0 && result2 == 0)
    {
        cerr << "PASS: open() method  with VALID args\n\n";
    }
    else
    {
        cerr << "FAIL: open() method  with VALID args\n\n";
    }
    ...
    cerr << "TEST: open() method with valid args but no database set\n";
    result = theHandler4->open(columns, numColumns - 1);

    if (result != 0)
    {
        cerr << "PASS: open() method with valid args but no database set\n\n";
    }
    else
    {
        cerr << "FAIL: open() method with valid args but no database set\n\n";
    }

    cerr << "TEST: open() method with bad column descriptions\n";
    result = theHandler1->open(0, 2);

    if (result != 0)
    {
        cerr << "PASS: open() method with bad column descriptors\n\n";
    }
    else
    {
        cerr << "FAIL: open() method with bad column descriptors\n\n";
    }

    cerr << "TEST: open() method with negative number of columns\n";
    result = theHandler1->open(columns, -1);

    if (result != 0)
    {
        cerr << "PASS: open() method with negative columns\n\n";
    }
    else
    {
        cerr << "FAIL: open() method with negative columns\n\n";
    }
    ...
    cerr << "Now We Test setDatabase & open after setDatabase on handler 2\n";
    result = theHandler3->setDatabase(theDatabase, "UnitTest3");
    result += theHandler3->open(columns, numColumns - 1);

    if (result == 0)
    {
        cerr << "PASS: setDatabase() then open()\n\n";
    }
    else
    {
        cerr << "FAIL: setDatabase() then open()\n\n";
    }
    ...
    cerr << "Now We Test write()\n\t 1. invoke write() 3 times with valid args on both handler1, handler2 and handler3\n";
    cerr << "\t 2. invoke write() with invalid args a number of times\n\n";
    ...
    {
        cerr << "PASS: write() method  with VALID args\n\n";
    }
    else
    {
        cerr << "FAIL: write() method  with VALID args\n\n";
    }
    ...
    cerr << "TEST: call write() method with incorrect vector size\n";
    result = theHandler3->write(data1);

    if (result != 0)
    {
        cerr << "PASS: write() method  with incorrect vector size\n\n";
    }
    else
    {
        cerr << "FAIL: write() method  with incorrect vector size\n\n";
    }

    cerr << "TEST: call write() method on handler which has not been set\n";
    result = theHandler4->write(data1);

    if (result != 0)
    {
        cerr << "PASS: write() method to handler where open has not been called\n\n";
    }
    else
    {
        cerr << "FAIL: write() method to handler where open has not been called\n\n";
    }
    ...
    cerr << "TEST: Destruction of DataOutputDatabaseHandler object\n";
    delete theHandler1;
    delete theHandler2;
    delete theHandler3;
    delete theHandler4;
    ...
    cerr << "PASS: Destruction of DataOutputDatabaseHandler object\n\n";
    exit(0);
}
