      ...
      theDatabase(database), tableName(0), numColumns(0), columns(0), commitTag(0)
{
    ...
    if (tName != 0)
    {
        ...
        if (tableName == 0)
        {
            cerr << "DataOutputDatabaseHandler::DataOutputDatabaseHandler - out of memory creating copy of tableName: " << tName << endln;
        }
    }
}
...
DataOutputDatabaseHandler::open(char** dataDescription, int numData)
{
    ...
    if (theDatabase == 0)
    {
        cerr << "DataOutputStreamHandler::open() - database has not been set\n";
        return -1;
    }

    if (tableName == 0)
    {
        cerr << "DataOutputDatabaseHandler::open() - no tableName passed or failed to get memory\n";
        return -1;
    }

    if (dataDescription == 0)
    {
        cerr << "DataOutputDatabaseHandler::open() - no column description data passed\n";
        return -1;
    }

    if (numData < 0)
    {
        cerr << "DataOutputDatabaseHandler::open() - numColumns (" << numData << ") < 0\n";
        return -1;
    }
    else
    {
    ...
    if (columns == 0)
    {
        cerr << "DataOutputDatabaseHandler::open() - out of memory creating array for columns of size (" << numData << ") < 0\n";
        numColumns = 0;
        return -1;
    }
    ...
    for (int i = 0; i < numColumns; i++)
    {
        ...
        if (columns[i] == 0)
        {
            cerr << "DataOutputDatabaseHandler::open() - out of memory creating copy of string " << dataDescription[i] << endln;
...
DataOutputDatabaseHandler::write(Vector& data)
{
    ...
    if (data.Size() == numColumns)
        ...
        {
            cerr << "DataOutputStreamHandler::write() - database has not been set\n";
            return -1;
        }
    else
    {
        cerr << "DataOutputStreamHandler::write() - Vector not of correct size or open() has not been called\n" << numColumns << " " << data.Size() << endln;
        return -1;
    }
...
DataOutputDatabaseHandler::setDatabase(FE_Datastore& database, const char* tName)
{
    ...
    {
        cerr << "DataOutputDatabaseHandler::DataOutputDatabaseHandler - no tableName passed\n";
        return -1;
    }
