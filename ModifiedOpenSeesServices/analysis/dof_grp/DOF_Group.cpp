      ...
      numDOF(node->getNumberDOF())
{
    ...
    if (numDOF <= 0)
    {
        ...
        cerr << " node must have at least 1 dof ";
    ...
    {
        cerr << "DOF_Group::DOF_Group(Node *) ";
        cerr << " ran out of memory creating ID for node ";
      ...
      numDOF(ndof)
{
    ...
    {
        cerr << "DOF_Group::DOF_Group(int, int ndof) ";
        cerr << " ran out of memory creating ID of size " << ndof << endln;
        exit(-1);
    }
...
DOF_Group::setID(int index, int value)
{
    ...
    else
    {
        ...
        cerr << index << " in ID of size " << numDOF << endln;
    }
}
...
DOF_Group::getM_Force(const Vector &Udotdot, double fact)
{
    if (myNode == 0)
    {
        cerr << "DOF_Group::getM_Force() - no Node associated";
        cerr << " subclass should not call this method \n";
        return *unbalance;
    }
...
DOF_Group::getC_Force(const Vector &Udotdot, double fact)
{
    if (myNode == 0)
    {
        cerr << "DOF_Group::getC_Force() - no Node associated";
        cerr << " subclass should not call this method \n";
        return *unbalance;
    }
...
DOF_Group::addLocalM_Force(const Vector &accel, double fact)
{
    if (myNode != 0)
    {
        if (unbalance->addMatrixVector(1.0, myNode->getMass(), accel, fact) < 0)
        {
            ...
            cerr << "DOF_Group::addLocalM_Force() ";
            cerr << " invoking addMatrixVector() on the unbalance failed\n";
        }
    }
    else
