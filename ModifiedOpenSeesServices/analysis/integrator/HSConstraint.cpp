...
HSConstraint::newStep(void)
{
    ...
    {
        cerr << "WARNING HSConstraint::newStep() ";
...
HSConstraint::update(const Vector& dU)
{
    ...
    if (b24ac < 0)
    {
        cerr << "HSConstraint::update() - imaginary roots due to multiple instability";
        cerr << " directions - initial load increment was too large\n";
        cerr << "a1: " << a1 << " a2: " << a2 << " a3: " << a3 << " b24ac: " << b24ac << endln;
        return -1;
    }
    ...
    if (a1 == 0.0)
    {
        // cerr << "HSConstraint::update() - zero denominator";
        // cerr << "\n";
        // return -2;
        dLambda = -a3 / (2.0 * a2);
...
HSConstraint::domainChanged(void)
{
    ...
    if (theModel == 0 || theLinSOE == 0)
    {
        cerr << "WARNING HSConstraint::domainChanged() ";
        cerr << "No AnalysisModel or LinearSOE has been set\n";
        return -1;
    }
    ...
    if (deltaUhat == 0 || deltaUhat->Size() != size)   // create new Vector
    {
        ...
        {
            cerr << "FATAL HSConstraint::domainChanged() - ran out of memory for";
    ...
    if (haveLoad == 0)
    {
        cerr << "WARNING HSConstraint::domainChanged() - zero reference load";
        return -1;
    }
                       ...
                       Channel& theChannel)
{
    ...
    {
        cerr << "HSConstraint::sendSelf() - failed to send the data\n";
        return -1;
    }
...
void
HSConstraint::Print(ostream& s, int flag)
{
    AnalysisModel* theModel = this->getAnalysisModelPtr();
