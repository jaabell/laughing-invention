      ...
      minIncrement(min), maxIncrement(max)
{
    ...
    if (numIncr == 0)
    {
        cerr << "WARNING DistributedDisplacementControl::DistributedDisplacementControl() -";
...
DistributedDisplacementControl::newStep(void)
{
    ...
    {
        cerr << "WARNING DistributedDisplacementControl::newStep() ";
    ...
    //   cerr << "DistributedDisplacementControl: " << dUahat  << " " << theDofID << endln;
    //   cerr << "DistributedDisplacementControl::newStep() : " << deltaLambdaStep << endln;
    ...
    {
        cerr << "DistributedDisplacementControl::newStep - model failed to update for new dU\n";
        return -1;
    }
...
DistributedDisplacementControl::update(const Vector& dU)
{
    ...
    if (theModel == 0 || theLinSOE == 0)
    {
        cerr << "WARNING DistributedDisplacementControl::update() ";
        cerr << "No AnalysisModel or LinearSOE has been set\n";
        return -1;
    }
    ...
    if (theModel->updateDomain() < 0)
    {
        cerr << "DistributedDisplacementControl::update - model failed to update for new dU\n";
        return -1;
    }
...
DistributedDisplacementControl::domainChanged(void)
{
    ...
    if (theDomain == 0)
    {
        cerr << "BUG WARNING DistributedDisplacementControl::domainChanged() - no Domain associated!!";
        return -1;
    }
    ...
    if (theNodePtr != 0)
    {
        ...
        if (theGroup == 0)
        {
            cerr << "BUG DistributedDisplacementControl::domainChanged() - no DOF_Group associated with the node!!\n";
            return -1;
        }
        ...
        {
            cerr << "DistributedDisplacementControl::domainChanged() - not a valid dof " << theDof << endln;
            return -1;
        }
        ...
        if (theDofID < 0)
        {
            cerr << "DistributedDisplacementControl::domainChanged() - constrained dof not a valid a dof\n";;
            return -1;
        }
    }
    ...
    if (deltaUhat == 0 || deltaUhat->Size() != size)   // create new Vector
    {
        ...
        {
            cerr << "FATAL DistributedDisplacementControl::domainChanged() - ran out of memory for";
    ...
    if (haveLoad == 0)
    {
        cerr << "WARNING DistributedDisplacementControl::domainChanged() - zero reference load";
        return -1;
    }
    ...
    if (theDofID == -1)
    {
        cerr << "DistributedDisplacementControl::setSize() - failed to find valid dof - are the node tag and dof values correct?\n";
        return -1;
    }
...
DistributedDisplacementControl::sendSelf(int cTag, Channel& theChannel)
{
    ...
    if (processID == 0)
    {
        ...
        if (found == false)
        {
            ...
            if (nextChannels == 0)
            {
                cerr << "DistributedDisplacementControl::sendSelf() - failed to allocate channel array of size: " <<
    ...
    if (res < 0)
    {
        cerr << "WARNING DistributedDisplacementControl::sendSelf() - failed to send data\n";
        return -1;
    }
...
void
DistributedDisplacementControl::Print(ostream& s, int flag)
{
    // TO FINISH
}
