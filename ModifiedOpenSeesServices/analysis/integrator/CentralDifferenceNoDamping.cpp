...
CentralDifferenceNoDamping::domainChanged()
{
    ...
    if (U == 0 || U->Size() != size)
    {
                ...
                Udotdot == 0 || Udotdot->Size() != size)
        {
            ...
            cerr << "CentralDifferenceNoDamping::domainChanged - ran out of memory\n";
...
CentralDifferenceNoDamping::update(const Vector& X)
{
    ...
    if (updateCount > 1)
    {
        cerr << "ERROR CentralDifferenceNoDamping::update() - called more than once -";
    ...
    if (theModel == 0)
    {
        cerr << "ERROR CentralDifferenceNoDamping::update() - no AnalysisModel set\n";
        return -2;
    }
    ...
    if (U == 0)
    {
        cerr << "WARNING CentralDifferenceNoDamping::update() - domainChange() failed or not called\n";
        return -2;
    }
    ...
    if (X.Size() != U->Size())
    {
        cerr << "WARNING CentralDifferenceNoDamping::update() - Vectors of incompatable size ";
        cerr << " expecting " << U->Size() << " obtained " << X.Size() << endln;
        return -3;
    }
...
CentralDifferenceNoDamping::commit(void)
{
    ...
    if (theModel == 0)
    {
        cerr << "WARNING CentralDifferenceNoDamping::commit() - no AnalysisModel set\n";
        return -1;
    }
...
void
CentralDifferenceNoDamping::Print(ostream& s, int flag)
{
    AnalysisModel* theModel = this->getAnalysisModelPtr();
