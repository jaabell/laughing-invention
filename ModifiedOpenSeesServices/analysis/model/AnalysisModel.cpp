...
AnalysisModel::getDOFGraph(void)
{
    if (myDOFGraph == 0)
    {
        ...
        if (myDOFGraph == NULL)
        {
            std::cerr << "AnalysisModel::getDOFGraph() -- can not allocate memory for myDOFGraph\n";
            exit (EXIT_FAILURE);
        }
    }
...
AnalysisModel::newStepDomain(double dT)
{
    ...
    if (myDomain == 0)
    {
        cerr << "WARNING: AnalysisModel::newStep. No Domain linked.\n";
        return -1;
    }
...
void
AnalysisModel::setDampingFactors()
{
    if (myDomain == 0)
    {
        cerr << "WARNING: AnalysisModel::getCurrentDomainTime.";
    ...
    // invoke the method
    //     myDomain->setDampingFactorsforElement();
    //     myDomain->setDampingFactorsforNode();
}
...
int
AnalysisModel::eigenAnalysis(int numMode)
{
    // check to see there is a Domain linked to the Model
    ...
    // invoke the method
    return myDomain->eigenAnalysis(numMode);
}
//Added by Babak to return the Maximum tag of the DOF in the Analysis Model 06/04/13:
//-----------
int AnalysisModel::getMaxDOFtag()
{
    int MaxDOFtag = 0;
    DOF_Group *dofPtr = 0;
