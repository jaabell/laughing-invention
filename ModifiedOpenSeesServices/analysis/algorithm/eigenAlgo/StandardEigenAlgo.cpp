...
StandardEigenAlgo::solveCurrentStep(int numModes)
{
    ...
    if ((theModel == 0) || (theIntegrator == 0) || (theSOE == 0))
    {
        ...
        cerr << "StandardEigenAlgo::solverCurrentStep() -- setLinks() has not been called\n";
        return -1;
    }

    if (theIntegrator->formK() < 0)
    {
        cerr << "StandardEigenAlgo::solverCurrentStep() -- the Integrator failed in formK()\n";
        return -2;
    }

    if (theSOE->solve(numModes) < 0)
    {
        cerr << "StandardEigenAlgo::solverCurrentStep() -- the EigenSOE failed in solve()\n";
        return -4;
    }
