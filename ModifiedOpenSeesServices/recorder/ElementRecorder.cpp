      ...
      initializationDone(false), responseArgs(0), numArgs(0)
{
    ...
    if (responseArgs == 0)
    {
        cerr << "ElementRecorder::ElementRecorder() - out of memory\n";
        numEle = 0;
    }
...
ElementRecorder::record(int commitTag, double timeStamp)
{
    ...
    if (initializationDone == false)
    {
        ...
        {
            cerr << "ElementRecorder::record() - failed to initialize\n";
            return -1;
        }
    }
    ...
    if (deltaT == 0.0 || timeStamp >= nextTimeStampToRecord)
    {
        ...
        //
        //OK cerr << "Element Recorder: number of elements\n" << numEle << endln;
        ...
        for (int i = 0; i < numEle; i++)
        {
            if (theResponses[i] != 0)
            {
                ...
                {
                    //OK cerr << "Warning: Element Recorder: getResponse < 0 \n";
                    result += res;
                }
                else
                {
...
{
    if (theChannel.isDatastore() == 1)   //Guanzhou changed
    {
        cerr << "ElementRecorder::sendSelf() - does not send data to a datastore\n";
        return -1;
    }
    ...
    {
        cerr << "ElementRecorder::sendSelf() - failed to send idData\n";
        return -1;
    }
    ...
    if (msgLength ==  0)
    {
        cerr << "ElementRecorder::sendSelf() - no data to send!!\n";
        return -1;
    }
    ...
    {
        cerr << "ElementRecorder::sendSelf() - out of memory\n";
        return -1;
    }
    ...
    {
        cerr << "ElementRecorder::sendSelf() - failed to send message\n";
        return -1;
    }
    ...
    {
        cerr << "ElementRecorder::sendSelf() - failed to send the DataOutputHandler\n";
        return -1;
    }
                          ...
                          FEM_ObjectBroker& theBroker)
{
    ...
    for (int j = 0; j < numArgs; j++)
    {
        ...
        cerr << argLength << responseArgs[j] << endln;
    ...
    if (theHandler == 0)
    {
        cerr << "NodeRecorder::sendSelf() - failed to get a data output handler\n";
        return -1;
    }
...
ElementRecorder::initialize(void)
{
    ...
    {
        cerr << "ElementRecorder::initialize, warning, domain ==0 or numEle ==0!\n";
        return 0;
    }
    ...
    for (i = 0; i < numEle; i++)
    {
        ...
        if (theEle == 0)
        {
            //OK cerr << "ElementRecorder::initialize, warning, element " << eleID(i) << "does not exist!\n";
            theResponses[i] = 0;
        }
        else
        {
    ...
    if (data == 0)
    {
        cerr << "ElementRecorder::initialize() - out of memory\n";
        return -1;
    }
