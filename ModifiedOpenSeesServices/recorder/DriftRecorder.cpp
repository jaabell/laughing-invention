      ...
      theDomain(&theDom), theHandler(&theDataOutputHandler)
{
    ...
    if (nodeI == 0 || nodeJ == 0)
    {
        cerr << "DriftRecorder::DriftRecorder-- node " << nodeI << " or " << nodeJ << " not in domain\n";
        oneOverL(0) = 0.0;
    }
    ...
    {
        cerr << "DriftRecorder::DriftRecorder-- Nodal projection has zero component along chosen direction\n";
        oneOverL(0) = 0.0;
    }
    else
    {
      ...
      theDomain(&theDom), theHandler(&theDataOutputHandler)
{
    ...
    if (ndIsize != ndJsize)
    {
        cerr << "FATAL - DriftRecorder::DriftRecorder() - error node arrays differ in size\n";
        exit(-1);
    }
...
{
    cerr << "DriftRecorder::restart() - should not be called\n";
    //  theHandler->restart();
    return 0;
}
...
{
    cerr << "DriftRecorder::sendSelf() - NOT YET IMPLEMENTED\n";
    return -1;
}
