...
int YieldSurface_BC2D::commitState(Vector& force)
{
    ...
    //    cerr << "YieldSurface_BC2D::commitState(..), status = " << status_hist << endln;

    //  cerr << "YieldSurface_BC2D::commitState " << getTag() <<" - force location: " << status_hist << endln;
    //  cerr << " FORCE = " << force;
    if(status_hist > 0)
    {
    ...
    // cerr << "yPos = " << yPos << ", fy = " << fy_hist << endln;
...
int YieldSurface_BC2D::modifySurface(double magPlasticDefo, Vector& Fsurface, Matrix& G, int flag)
{
    ...
    {
        cerr << "Can't modify surface with Force Location = " << getTrialForceLocation(Fsurface) << endln;
        return 0;
    }
    ...
    if(magPlasticDefo < 0)
    {
        ...
        cerr << "Warning -   magPlasticDefo < 0 " << magPlasticDefo << "\n";
        // magPlasticDefo = 0;
        return 0;
    }
    ...
    else if(isotropicFactor < isotropicFactor_hist)
    {
        ...
        cerr << "ModifySurface - this condition should not happen\n";
    */
    return state;
...
void YieldSurface_BC2D::getTrialGradient(Matrix& G, Vector& force)
{
    ...
    //  cerr << "YieldSurface_BC2D::getTrialGradient  trial_drift: " << this->getTrialDrift(force)
    //       << "  drift: " << drift << endln;
    //  cerr << "----> calling getGradient " << endln;

    getGradient(gx, gy, fx, fy);
    //  cerr << "<---- done calling getGradient " << endln;
...
int  YieldSurface_BC2D::forceLocation(double drift)
{
    ...
    {
        cerr << "YieldSurface_BC2D::forceLocation(double drift) - this condition not possible\n";
        cerr << "\a";
    }
...
double YieldSurface_BC2D::getDrift(double x, double y)
{
    ...
    if(x != 0)
    {
        ...
        {
            cerr << "YieldSurface_BC2D::getDrift(..) - condition not possible, x = " << x << ", y = " << y << endln;
        }

        if(driftDebug)
        {
            cerr << "Equation Internal: a = " << a_int << ", b = " << b_int << "\n";
            cerr << "Equation External: a = " << a_ext << ", b = " << b_ext << "\n";
        }
    ...
    if(R_xy < R0)
    {
        if(driftDebug)
        {
            cerr << " R_xy < R0, returning sdrift " << sdrift << "\n";
            cerr << "\a";
        }
    ...
    while(1)
    {
        ...
        if(R_xy < Ri)
        {
            if(driftDebug)
            {
                cerr << " R_xy < Ri, returning sdrift " << sdrift << "\n";
                cerr << "\a";
            }
        ...
        if(getSurfaceDrift(xNew, yNew) > 0) // we just crossed the surface
        {
            ...
            if(driftDebug)
            {
                cerr << " Set to surface at xi = " << xi << ", yi = " << yi << ", Ri = " << Ri << "\n";
                cerr << " Returning drift " << R_xy - Ri << "\n";
                cerr << "\a";
            }
...
double  YieldSurface_BC2D::interpolate(double xi, double yi, double xj, double yj)
{
    ...
    //cerr << "cerr for iterpolation = " << count << "\n"; 5 ~ 15
    return tr;
}
...
//      cerr << "Element system force = " << force << "\n";
//  }
//
...
//  if(returnDebug)
//  {
//      cerr << "Local system force - " << "fx = " << x2 << ",\tfy = " << y2 << "\n";
//      cerr << "toOriginalCoord    - " << "fx = " << xj << ",\tfy = " << yj << "\n";
//  }
//
//      switch(algoType)
//      {
...
/*
            ...
            case 2: //ConstantXReturn:
            {
                ...
                {
                  //cerr << "ConstantXReturn - Point is inside: " << getDrift(xj, yj) << "\n";
            ...
            case 3: //ConstantYReturn:
            {
                ...
                if(getDrift(xj, yj) < 0)    //point is inside the ys
                {
                    //cerr << " ConstantYReturn - Point Inside\n"; //
