                           ...
                           TCL_Char** argv, TclModelBuilder* theTclBuilder)
{
    ...
    if (strcmp(argv[1], "multiLinearKp") == 0)
    {
        ...
        if(numPoints < 2)
        {
            cerr << "WARNING invalid uniaxialMaterial MultilinearUniaxial tag" << endln;
        ...
        for (j1 = 0; j1 < numPoints; j1++)
        {
            if (Tcl_GetDouble(interp, argv[indx], &temp) != TCL_OK)
            {
                ...
                cerr << "MultilinearUniaxial material: " << tag << endln;
                return TCL_ERROR;
            }
                    ...
                    char **argv, TclModelBuilder *theTclBuilder)
{
    ...
    if (Tcl_GetInt(interp, argv[2], &tag) != TCL_OK)
    {
            cerr << "WARNING invalid  PlaticHardening quadrReducing tag" << endln;
            return TCL_ERROR;
    }
    if (Tcl_GetDouble(interp, argv[3], &kp_0) != TCL_OK)
    {
            cerr << "WARNING invalid  PlaticHardening quadrReducing kp_0" << endln;
            return TCL_ERROR;
    }
    if (Tcl_GetDouble(interp, argv[4], &kp_half) != TCL_OK)
    {
            cerr << "WARNING invalid  PlaticHardening quadrReducing kp_half" << endln;
            return TCL_ERROR;
    }
                            ...
                            TCL_Char** argv, TclModelBuilder* theTclBuilder)
{
    ...
    if (Tcl_GetInt(interp, argv[2], &tag) != TCL_OK)
    {
        cerr << "WARNING invalid  PlaticHardening exponReducing tag" << endln;
        return TCL_ERROR;
    }

    if (Tcl_GetDouble(interp, argv[3], &arg1) != TCL_OK)
    {
        cerr << "WARNING invalid double PlaticHardening exponReducing" << endln;
        return TCL_ERROR;
    }
