    ...
    otherDbTag(0)
{
    ...
    if (!theSection)
    {
        cerr << "SectionAggregator::SectionAggregator -- failed to get copy of section\n";
        exit(-1);
    }

    if (!theAdds)
    {
        cerr << "SectionAggregator::SectionAggregator -- null uniaxial material array passed\n";
        exit(-1);
    }
    ...
    if (!theAdditions)
    {
        cerr << "SectionAggregator::SectionAggregator -- failed to allocate pointers\n";
        exit(-1);
    }
    ...
    for (i = 0; i < numMats; i++)
    {
        if (!theAdds[i])
        {
            cerr << "SectionAggregator::SectionAggregator -- null uniaxial material pointer passed\n";
            exit(-1);
        }
        ...
        if (!theAdditions[i])
        {
            cerr << "SectionAggregator::SectionAggregator -- failed to copy uniaxial material\n";
            exit(-1);
        }
    }
    ...
    if (order > maxOrder)
    {
        cerr << "SectionAggregator::SectionAggregator -- order too big, need to modify the #define in SectionAggregator.cpp to " <<
             order << endln;
        exit(-1);
    }
    ...
    {
        cerr << "SectionAggregator::SectionAggregator -- out of memory\n";
        exit(-1);
    }
}
    ...
    otherDbTag(0)
{
    ...
    if (order > maxOrder)
    {
        cerr << "SectionAggregator::SectionAggregator -- order too big, need to modify the #define in SectionAggregator.cpp to %d\n";
        exit(-1);
    }
...
SectionAggregator::getSectionFlexibility(void)
{
    ...
    for ( ; i < order; i++)
    {
        ...
        if (k == 0.0)
        {
            cerr << "SectionAggregator::getSectionFlexibility -- singular section stiffness\n";
...
SectionAggregator::getCopy(void)
{
    ...
    if (theCopy == 0)
    {
        cerr << "SectionAggregator::getCopy -- failed to allocate copy\n";
        exit(-1);
    }
...
SectionAggregator::sendSelf(int cTag, Channel &theChannel)
{
    ...
    if (res < 0)
    {
        cerr << "SectionAggregator::sendSelf -- could not send data ID\n";
    ...
    if (res < 0)
    {
        cerr << "SectionAggregator::sendSelf -- could not send classTags ID\n";
        return res;
    }
    ...
    for (i = 0; i < numMats; i++)
    {
        ...
        if (res < 0)
        {
            cerr << "SectionAggregator::sendSelf -- could not send UniaxialMaterial, i = " << i << endln;
            return res;
        }
    }
    ...
    if (theSection != 0)
    {
        ...
        if (res < 0)
        {
            cerr << "SectionAggregator::sendSelf -- could not send SectionForceDeformation\n";
            return res;
        }
    }
