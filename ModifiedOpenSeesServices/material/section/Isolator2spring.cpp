...
Isolator2spring::getStressResultant(void)
{
    ...
    while (normf0 > tol)
    {
        ...
        if (iter > 19)
        {
            cerr << "WARNING! Iso2spring: Newton iteration failed. Norm Resid: " << normf0  << endln;
            break;
        }
    }
...
Isolator2spring::sendSelf(int cTag, Channel& theChannel)
{
    ...
    if (res < 0)
    {
        cerr << "Isolator2spring::sendSelf() - failed to send data\n";
    }
...
void
Isolator2spring::Print(ostream& s, int flag)
{
