    ...
    : SectionForceDeformation(tag, SEC_TAG_Generic1d), code(type)
{
    ...
    if (!theModel)
    {
        cerr << "GenericSection1d::GenericSection1d  -- failed to get copy of material model\n";
        exit(-1);
    }
}
...
GenericSection1d::sendSelf(int cTag, Channel& theChannel)
{
    ...
    if (res < 0)
    {
        cerr << "GenericSection1d::sendSelf -- could not send ID\n";
        return res;
    }
    ...
    if (res < 0)
    {
        cerr << "GenericSection1d::sendSelf -- could not send UniaxialMaterial\n";
        return res;
    }
