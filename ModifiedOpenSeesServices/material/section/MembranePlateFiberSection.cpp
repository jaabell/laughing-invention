...
MembranePlateFiberSection::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    if (res < 0)
    {
        cerr << "WARNING MembranePlateFiberSection::sendSelf() - " << this->getTag() << " failed to send ID\n";
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        if (res < 0)
        {
            cerr << "WARNING MembranePlateFiberSection::sendSelf() - " << this->getTag() << " failed to send its Material\n";
            return res;
        }
    }
