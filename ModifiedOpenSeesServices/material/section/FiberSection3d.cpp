...
FiberSection3d::FiberSection3d(int tag, int num, Fiber **fibers):
    ...
    yBar(0.0), zBar(0.0), e(3), eCommit(3), s(0), ks(0), SectionArea(0)
{
    if (numFibers != 0)
    {
        ...
        if (theMaterials == 0)
        {
            cerr << "FiberSection3d::FiberSection3d -- failed to allocate Material pointers\n";
            exit(-1);
        }
        ...
        if (matData == 0)
        {
            cerr << "FiberSection3d::FiberSection3d -- failed to allocate double array for material data\n";
            exit(-1);
        }
        ...
        for (int i = 0; i < numFibers; i++)
        {
            ...
            if (theMaterials[i] == 0)
            {
                cerr << "FiberSection3d::FiberSection3d -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
...
FiberSection3d::addFiber(Fiber &newFiber)
{
    ...
    {
        cerr << "FiberSection3d::addFiber -- failed to allocate Fiber pointers\n";
        exit(-1);
    }
    ...
    {
        cerr << "FiberSection3d::addFiber -- failed to get copy of a Material\n";
        exit(-1);
...
FiberSection3d::getCopy(void)
{
    ...
    if (numFibers != 0)
    {
        ...
        for (int i = 0; i < numFibers; i++)
        {
            ...
            {
                cerr << "FiberSection3d::getCopy -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
    }
...
FiberSection3d::sendSelf(int commitTag, Channel &theChannel)
{
    ...
    if (res < 0)
    {
        cerr << "FiberSection3d::sendSelf - failed to send ID data\n";
        return res;
    }
    ...
    if (numFibers != 0)
    {
        ...
        if (res < 0)
        {
            cerr << "FiberSection3d::sendSelf - failed to send material data\n";
            return res;
        }
                         ...
                         FEM_ObjectBroker &theBroker)
{
    ...
    if (res < 0)
    {
        cerr << "FiberSection3d::sendSelf - failed to recv ID data\n";
        return res;
    }
