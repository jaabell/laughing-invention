...

{
    ...
    if (theFibers == 0)
    {
        std::cerr << "FiberSectionRepr::FiberSectionRepr -- failed to allocate Fiber pointers\n";
    ...
    if (!patch)
    {
        std::cerr << "FATAL ERROR: FiberSectionRepr - not enough memory to allocate " << maxNumPatches << " patches";
        exit (-1);
    }
    int i;
    ...
    if (!reinfLayer)
    {
        std::cerr << "FATAL ERROR: FiberSectionRepr - not enough memory to allocate " << maxNumReinfLayers << "reinforcing layers";
        exit (-1);
    }
...
FiberSectionRepr::FiberSectionRepr(int sectionID)
    : SectionRepres(sectionID),
      ...
      nReinfLayers(0)
{
    std::cerr << "Function FiberSectionRepr::FiberSectionRepr not implemented yet";
    exit (-1);
}
...
{
    std::cerr << "Function FiberSectionRepr::setnumPatches not implemented yet";
    exit (-1);
}

int FiberSectionRepr::setPatches (Patch **patches)
{
    std::cerr << "Function FiberSectionRepr::setPatches not implemented yet";
    exit (-1);
...
{
    std::cerr << "Function FiberSectionRepr::setnumReinfLayers not implemented yet";
    exit (-1);
}

int FiberSectionRepr::setReinfLayers (ReinfLayer **reinfLayers)
{
    std::cerr << "Function FiberSectionRepr::setReinfLayers not implemented yet";
    exit (-1);
...
int FiberSectionRepr::addPatch (const Patch & aPatch)
{
    ...
    else
    {
        ...
        if (patches == 0)
        {
            std::cerr << "FiberSectionRepr::addPatch() - out of memory\n";
            return 1;
        }
...
int FiberSectionRepr::addReinfLayer (const ReinfLayer & aReinfLayer)
{
    ...
    else
    {
        ...
        if (reinfLayers == 0)
        {
            std::cerr << "FiberSectionRepr::addReinLayer() - out of memory\n";
            return 1;
        }
...
FiberSectionRepr::addFiber(Fiber &newFiber)
{
    ...
    else
    {
        ...
        if (newArray == 0)
        {
            std::cerr << "FiberSection::addFiber -- failed to allocate Fiber pointers\n";
            return -1;
        }
