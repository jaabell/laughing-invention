      ...
      peakPlstkRot(0.0), iFactor(1.0)
{
    if (E <= 0.0)
    {
        cerr << "YS_Section2D02::YS_Section2D02s -- Input E <= 0.0 ... setting E to 1.0\n";
        E = 1.0;
    }

    if (A <= 0.0)
    {
        cerr << "YS_Section2D02::YS_Section2D02s -- Input A <= 0.0 ... setting A to 1.0\n";
        A = 1.0;
    }

    if (I <= 0.0)
    {
        cerr << "YS_Section2D02::YS_Section2D02s -- Input I <= 0.0 ... setting I to 1.0\n";
        I = 1.0;
    }

    if(maxPlstkRot <= 0.0)
    {
        cerr << "YS_Section2D02::YS_Section2D02s -- Input maxPlstkRot <= 0.0 ... setting to 0.0\n";
        maxPlstkRot = 0.0;
    }
...
int YS_Section2D02::commitState(void)
{
    ...
    cerr << peakPlstkRot << "\t" << iFactor << endln;
