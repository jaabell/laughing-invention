      ...
      minElasticYieldStrain(gap0), damage(accum)
{
    if (E == 0.0)
    {
        cerr << "EPPGapMaterial::EPPGapMaterial -- E is zero, continuing with E = fy/0.002\n";
        ...
        {
            cerr << "EPPGapMaterial::EPPGapMaterial -- E and fy are zero\n";
            exit(-1);
        }
    }
    else
    ...
    if (fy * gap < 0)
    {
        cerr << "EPPGapMaterial::EPPGapMaterial -- Alternate signs on fy and E encountered, continuing anyway\n";
    }

    if ( (eta >= 1) || (eta <= -1) )
    {
        cerr << "EPPGapMaterial::EPPGapMaterial -- value of eta must be -1 <= eta <= 1, setting eta to 0\n";
        eta = 0;
    }

    if ( (damage < 0) || (damage > 1) )
    {
        cerr << "%s -- damage switch must be 0 or 1\n";
    }
}
