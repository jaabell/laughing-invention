      ...
      Cstrain(0.0), Cstress(0.0)
{
    ...
    {
        cerr << "error: negative values required for concrete stress-strain model" << endln;
    }

    if (fct < 0.0)
    {
        fct = 0.0;
        cerr << "warning: fct less than 0.0 so the tensile response part is being set to 0" << endln;
    }
...
int Concrete04::sendSelf (int commitTag, Channel& theChannel)
{
    ...
    if (res < 0)
    {
        cerr << "Concrete04::sendSelf() - failed to send data\n";
    }
...
void Concrete04::Print (ostream& s, int flag)
{
