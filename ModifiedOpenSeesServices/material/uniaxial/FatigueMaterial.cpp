      ...
      Cfailed(false), trialStrain(0)
{
    ...
    {
        cerr << "FatigueMaterial::FatigueMaterial - Dmax must be between 0 and 1, assuming Dmax = 1\n";
        Dmax = 1;
    }
    else
    {
    ...
    if (theMaterial == 0)
    {
        cerr <<  "FatigueMaterial::FatigueMaterial -- failed to get copy of material\n";
        exit(-1);
    }
}
...
FatigueMaterial::commitState(void)
{
    ...
    {
        Cfailed = true;
        cerr << "FatigueMaterial: material tag " << this->getTag() << " failed from excessive strain\n";
        DI = Dmax;
        DL = Dmax;
        return 0;
    }
    ...
    */
    ...
    if (sign(PS) != sign(CS) && sign(PS) != 0 )
    {
        ...
        if (DI >= Dmax )
        {
            ...
            Cfailed = true;
            cerr << "FatigueMaterial: material tag " << this->getTag() << " failed at peak\n";
            DL = DI;
        }
        else
        {
    ...
    if (Cfailed == false)
    {
        ...
        if (DL > Dmax && mStress > 0.0 )
        {
            DI = DL;
            Cfailed = true;
            cerr << "FatigueMaterial: material tag " << this->getTag() << " failed at pseudo peak\n";
        }
        else
        {
            Cfailed = false;
...
FatigueMaterial::sendSelf(int cTag, Channel &theChannel)
{
    ...
    {
        cerr << "FatigueMaterial::sendSelf() - failed to send the ID\n";
        return -1;
    }
    ...
    {
        cerr << "FatigueMaterial::sendSelf() - failed to send the Vector\n";
        return -2;
    }

    if (theMaterial->sendSelf(cTag, theChannel) < 0)
    {
        cerr << "FatigueMaterial::sendSelf() - failed to send the Material\n";
        return -3;
    }
