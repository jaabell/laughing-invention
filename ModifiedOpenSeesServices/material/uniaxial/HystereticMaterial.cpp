    ...
    mom1n(m1n), rot1n(r1n), mom2n(m2n), rot2n(r2n), mom3n(m3n), rot3n(r3n)
{
    ...
    if (error)
    {
        cerr << "HystereticMaterial::HystereticMaterial -- input backbone is not unique (one-to-one)\n";
        exit(-1);
    }
...
int
HystereticMaterial::receiveSelf(int commitTag, Channel& theChannel,
                             FEM_ObjectBroker& theBroker)
{
    ...
    if (res < 0)
    {
        cerr << "HystereticMaterial::receiveSelf() - failed to receive data\n";
        return res;
    }
    else
    {
