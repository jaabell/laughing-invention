    ...
    if (eleRemove != 2)
    {
        ...
        if (stateFlag == 0) //prior to failure
        {
                ...
                //if (deform >= deformSurface) // on/outside failure surface SDK
            {
                ...
                else
                {
                    ...
                    cerr << "AxialCurve - failure detected at deform = " << deform << ", force = " << force << ",element: " << eleTag << endln;//SDK
        ...
        else // after failure
        {
...

            {
                ...
                dP = fabs(force) - fabs(forceSurface); //change in axial load
                // SDK  cerr << "AxialCurve - on failure surface at deform = " << deform << ", force = " << force << endln;
            }
            else // inside failure surface
            {
                stateFlag = 3;
...
void
AxialCurve::Print(ostream& s, int flag)
{
