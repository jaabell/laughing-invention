    ...
    : UniaxialMaterial(tag, MAT_TAG_PathIndependent), theMaterial(0)
{
    ...
    if (theMaterial == 0)
    {
        cerr <<  "PathIndependentMaterial::PathIndependentMaterial -- failed to get copy of material\n";
        exit(-1);
    }
}
...
PathIndependentMaterial::sendSelf(int cTag, Channel& theChannel)
{
    ...
    if (res < 0)
    {
        cerr << "PathIndependentMaterial::sendSelf -- could not send ID\n";
        return res;
    }
    ...
    if (res < 0)
    {
        cerr << "PathIndependentMaterial::sendSelf -- could not send UniaxialMaterial\n";
        return res;
    }
