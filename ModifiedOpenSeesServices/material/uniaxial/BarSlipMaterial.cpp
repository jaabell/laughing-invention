...
void BarSlipMaterial::getBondStrength(void)
{
    if (fc <= 0.0)
    {
        cerr << "WARNING : BAR-SLIP -- fc should be positive entry" << endln;
    }
...
void BarSlipMaterial::createMaterial(void)
{
    ...
    */
    ...
    if (error)
    {
        cerr << "Error: -- input backbone not unique, BarSlipMaterial::BarSlipMaterial" << "\a";
    }
...
void BarSlipMaterial::Print(ostream& s, int flag)
{
