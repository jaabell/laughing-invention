...
ReinforcingSteel::setTrialStrain(double strain, double strainRate)
{
    ...
    if(strain < -0.95)
    {
        cerr << "Large trial compressive strain\n";
        return -1;
    }
    else
    {
...
ReinforcingSteel::getStress(void)
{
    ...
    {
        cerr << "bad Stress in ReinforcingSteel::getStress\n";
    }
...
ReinforcingSteel::getTangent(void)
{
    ...
    if(_fpclass(tempOut) < 8 || _fpclass(tempOut) == 512)
    {
        cerr << "bad tangent in ReinforcingSteel::getTangentat\n";
    }
...
ReinforcingSteel::sendSelf(int cTag, Channel& theChannel)
{
    ...
    if (--index != data.Size())
    {
        cerr << "ReinforcingSteel::sendSelf() wrong vector size\n";
    }
    ...
    if (res < 0)
    {
        cerr << "ReinforcingSteel::sendSelf() - failed to send data\n";
    }
...
void
ReinforcingSteel::Print(ostream& s, int flag)
{
    if(flag == 3)
    {
...
ReinforcingSteel::MPfunc(double a)
{
    if(a >= 1.0)
    {
        cerr << "a is one in ReinforcingSteel::MPfunc()\n";
    }
...
ReinforcingSteel::SetMP()
{
    ...
    else
    {
        if (TEsec != TEa)
        {
            ...
            if (Rmin < 0.0)
            {
                cerr << "R is negative in ReinforcingSteel::SetMP()\n";
                Rmin = 0.0;
            }
            ...
            while(notConverge)
            {
                ...
                {
                    cerr << "Stuck in infinite loop, return error, ReinforcingSteel::SetMP()\n";
                    da = da / 100.0;
                    ao = ao_last;
    ...
    if (TStrain - CStrain < 0.0)
    {
        if (strain - eshp > -ZeroTol)
        {
            ...
            if(TEsec < TEb)
            {
                ...
                Teb = Teo_n + emin;
                TEsec = (Tfb - Tfa) / (Teb - Tea);
                cerr << "Adjusted Compressive Curve anchor in ReinforcingSteel::Rule1()\n";
            }
