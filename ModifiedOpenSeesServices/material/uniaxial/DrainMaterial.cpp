      ...
      epsilon(0.0), epsilonDot(0.0), sigma(0.0), tangent(0.0)
{
    ...
    if (numHstv > 0)
    {
        ...
        if (hstv == 0)
        {
            cerr << "DrainMaterial::DrainMaterial -- failed to allocate history array -- type : " <<
    ...
    if (numData > 0)
    {
        ...
        if (data == 0)
        {
            cerr << "DrainMaterial::DrainMaterial -- failed to allocate data array -- type: " <<
                 this->getClassTag() << endln;
            exit(-1);
        }
...
DrainMaterial::invokeSubroutine(void)
{
    ...
    switch (this->getClassTag())
    {
        ...
        case MAT_TAG_DrainBilinear:
            // I don't know which subroutines to call, so fill in the XX for Bilinear later -- MHS
            cerr << "DrainMaterial::invokeSubroutine -- Bilinear subroutine not yet linked\n";
            exit(-1);
        ...
        case MAT_TAG_DrainClough1:
            // I don't know which subroutines to call, so fill in the XX for Clough1 later -- MHS
            cerr << "DrainMaterial::invokeSubroutine -- Clough1 subroutine not yet linked\n";
            exit(-1);
        ...
        case MAT_TAG_DrainClough2:
            // I don't know which subroutines to call, so fill in the XX for Clough2 later -- MHS
            cerr << "DrainMaterial::invokeSubroutine -- Clough2 subroutine not yet linked\n";
            exit(-1);
        ...
        case MAT_TAG_DrainPinch1:
            // I don't know which subroutines to call, so fill in the XX for Pinch1 later -- MHS
            cerr << "DrainMaterial::invokeSubroutine -- Pinch1 subroutine not yet linked\n";
            exit(-1);
        ...
        default:
            cerr << "DrainMaterial::invokeSubroutine -- unknown material type\n";
            exit(-1);
            return -1;
    }
