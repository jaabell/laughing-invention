...
MapOfTaggedObjects::setSize(int newSize)
{
    ...
    if (newSize > maxSize)
    {
        cerr << "MapOfTaggedObjects::setSize - failed as map stl has a max size of " << maxSize << "\n";
        return -1;
    }
...
MapOfTaggedObjects::addComponent(TaggedObject* newComponent)
{
    ...
    if (theEle == theMap.end())
    {
        ...
        if (theEle == theMap.end())
        {
            cerr << "MapOfTaggedObjects::addComponent - map STL failed to add object with tag : " <<
                 newComponent->getTag() << "\n";
            return false;
        }
    }
    ...
    {
        cerr << "MapOfTaggedObjects::addComponent - not adding as one with similar tag exists, tag: " <<
             newComponent->getTag() << "\n";
        return false;
    }
...
//Added by Babak 06/3/13
//-----------
bool
MapOfTaggedObjects::addComponent_graph(TaggedObject* newComponent)
{
    return true;  // o.k.
}
//-----------
...
MapOfTaggedObjects::removeComponent(int tag)
{
    ...
    else   // the object exists so we remove it
    {
        ...
        {
            cerr << "MapOfTaggedObjects::removeComponent - map STL failed to remove object with tag " <<
                 tag << "\n";
            return 0;
        }
    }
...
MapOfTaggedObjects::getEmptyCopy(void)
{
    ...
    if (theCopy == 0)
    {
        cerr << "MapOfTaggedObjects::getEmptyCopy-out of memory\n";
    }
