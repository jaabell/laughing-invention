...
# ifdef _PDD
    ...
    if ( mype != 0 )
    {
        ...
        {
            opserr << "WARNING ParMetis::partition - No partitioning done";
    ...
    //opserr << "ParMetis::partition() -5\n";

    part = new int [nvtxs];

    //int *part = theGraph.New_part(nvtxs);
    //if ( ubvec != 0 ) delete [] ubvec;
    //wopserr << "ParMetis::partition() -6\n";
    ...
    if ( mype != 0 )
    {
        ...
        opserr << "ParMETIS_V3_PartKway reported a cut of " << edgecut << "\n";
        delete [] vwgt;
    }

    //opserr << "ParMetis::partition() -7\n";
...
# ifdef _PDD
    ...
    if ( mype != 0 )
    {
        ...
        if (mype == 1)
        {
            opserr << "Initial Load Imbalance: " << (1.0 * max_wgt * npes) / (1.0 * sum_wgt) << endln;
            opserr << "MAX: " << max_wgt << " MIN: " << min_wgt << " SUM: " << sum_wgt << endln;
        }
    }
    ...
    if ( mype != 0 )
    {
        ...
        opserr << "ParMETIS_V3_AdaptiveRepart reported a cut of " << edgecut << "\n";
    }
