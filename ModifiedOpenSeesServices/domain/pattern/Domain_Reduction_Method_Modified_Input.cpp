    ...
    DRMAccelerationsFile(strdup(AccefName))
{
    ...
    else
    {
        ...
        //  cerr << "- Domain_Reduction_Method_Modified_Input:: Displacement Matrix:" << endln;;
        //  cerr << (*U) << endln;
        //--------------------------------
    ...
    else
    {
        ...
        //  cerr << "- Domain_Reduction_Method_Modified_Input:: Acceleration Matrix:" << endln;;
        //  cerr << (*Udd) << endln;
        //--------------------------------
...
Domain_Reduction_Method_Modified_Input::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    //Returned them back by Babak 08/10/2012:
    //     if ( !LoadComputed )
    //      this->CompPBLoads(); //have to compute the load before send them over!
    ...
    // cerr << "Domain_Reduction_Method_Modified_Input::sendSelf -- failed to send Matrix U\n";
    // return -1;
    //}
    //
    //if (theChannel.sendMatrix(dataTag, commitTag, Udd) < 0) {
    // cerr << "Domain_Reduction_Method_Modified_Input::sendSelf -- failed to send Matrix Udd\n";
    // return -1;
    //}
    //
    //if (theChannel.sendID(dataTag, commitTag, PBowlElements) < 0) {
    // cerr << "Domain_Reduction_Method_Modified_Input::sendSelf -- failed to send PBowlElements\n";
    // return -1;
    //}
        ...
        FEM_ObjectBroker& theBroker)
{
    ...
    {
        cerr << "Domain_Reduction_Method_Modified_Input::recvSelf -- failed to recv ID\n";
        return -1;
    }
    ...
    {
        cerr << "Domain_Reduction_Method_Modified_Input::recvSelf() - ran out of memory constructing";
    ...
    {
        cerr << "Domain_Reduction_Method_Modified_Input::recvSelf -- failed to recv BoundaryNodes\n";
        return -1;
    }
    ...
    {
        cerr << "Domain_Reduction_Method_Modified_Input::recvSelf -- failed to recv ExteriorNodes\n";
        return -1;
    }
    ...
    {
        cerr << "Domain_Reduction_Method_Modified_Input::recvSelf -- failed to recv Matrix PBowlLoads\n";
        return -1;
    }
    ...
    {
        cerr << "Domain_Reduction_Method_Modified_Input::recvSelf -- failed to recv NodeID\n";
        return -1;
    }
    ...
    // cerr << "Domain_Reduction_Method_Modified_Input::recvSelf -- failed to recv Matrix U\n";
    // return -1;
    //}
    //
    ...
    // cerr << "Domain_Reduction_Method_Modified_Input::recvSelf -- failed to recv Matrix Udd\n";
    // return -1;
    //}
    //
    //int numPBE = idData(4);
    ...
    // cerr << "Domain_Reduction_Method_Modified_Input::recvSelf -- failed to recv PBowlElements\n";
    // return -1;
    //}
    ...
    {
        cerr << "Domain_Reduction_Method_Modified_Input::recvSelf -- failed to recv data\n";
        return -1;
    }
    ...
    //cerr << "Domain_Reduction_Method_Modified_Input::recvSelf() - the Domain_Reduction_Method_Modified_Input received: \n";// << *PBowlLoads << endln;
...
Domain_Reduction_Method_Modified_Input::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (theChannel.sendID(myDbTag, commitTag, theData) < 0) {
    g3ErrorHandler->warning("Domain_Reduction_Method_Modified_Input::sendSelf - channel failed to send the initial ID");
    return -1;
  }
  ...
  if (theChannel.sendID(myDbTag, commitTag, theMotionsData) < 0) {
    g3ErrorHandler->warning("Domain_Reduction_Method_Modified_Input::sendSelf - channel failed to send the motions ID");
    return -1;
  }
  ...
  for (int j=0; j<numMotions; j++)
    if (theMotions[j]->sendSelf(commitTag, theChannel) < 0) {
      g3ErrorHandler->warning("Domain_Reduction_Method_Modified_Input::sendSelf - motion no: %d failed in sendSelf", j);
      return -1;
    }
       ...
       FEM_ObjectBroker &theBroker)
{
  ...
  if (theChannel.recvID(myDbTag, commitTag, theData) < 0) {
    g3ErrorHandler->warning("Domain_Reduction_Method_Modified_Input::recvSelf - channel failed to recv the initial ID");
    return -1;
  }
  ...
  if (theChannel.recvID(myDbTag, commitTag, theMotionsData) < 0) {
    g3ErrorHandler->warning("Domain_Reduction_Method_Modified_Input::recvSelf - channel failed to recv the motions ID");
    return -1;
  }
  ...
  delete theMotions[i];
    ...
    if (theMotions == 0) {
      g3ErrorHandler->warning("Domain_Reduction_Method_Modified_Input::recvSelf - out of memory creating motion array of size %d\n", numMotions);
      numMotions = 0;
      return -1;
    }
  ...
  g3ErrorHandler->warning("Domain_Reduction_Method_Modified_Input::recvSelf - motion no: %d failed in recvSelf", i);
  numMotions = 0;
  return -1;
      }
    }
...
Domain_Reduction_Method_Modified_Input::getCopy(void)
{
    ...
    //implemented by Babak to make an appropriate copy ...
    ...
    //   cerr << "Domain_Reduction_Method_Modified_Input::getCopy() - not yet implemented\n";
    //   return 0;
}
...
Domain_Reduction_Method_Modified_Input::getCopy(void)
{
  Domain_Reduction_Method_Modified_Input *theCopy = new Domain_Reduction_Method_Modified_Input(0, 0);
...
// void
// Domain_Reduction_Method_Modified_Input::addPBElements(const ID &PBEle)
// {
//   // create a copy of the vector containg plastic bowl elements
//   PBowlElements = new ID(PBEle);
//   // ensure we did not run out of memory
//   if (PBowlElements == 0 || PBowlElements->Size() == 0) {
//     cerr << "Domain_Reduction_Method_Modified_Input::addPBElements() - ran out of memory constructing";
...
Domain_Reduction_Method_Modified_Input::CompPBLoads()
{
    ...
    //      cerr << "Domain_Reduction_Method_Modified_Input::CompPBLoads() - Boundary node specification not correct..." << endln;
    //      exit(1);
    //   }
    ...
    if ( (BoundaryNodes == 0) && ( (xPlus < xMinus) || (yPlus < yMinus) || (zPlus < zMinus) ) )
    {
        cerr << "Domain_Reduction_Method_Modified_Input::CompPBLoads() - Boundary node specification not correct... node corrdinates signes flipped" << endln;
        exit(1);
    }
    ...
    if (BoundaryNodes == 0 || BoundaryNodes->Size() == 0)
    {
        cerr << "Domain_Reduction_Method_Modified_Input::CompPBLoads() - ran out of memory constructing";
    ...
    for (int i = 0; i < Bowl_elem_nb; i++)
    {
        ...
        //   final=clock()-init;
        //   cout << "Domain_Reduction_Method_Modified_Input::CompPBLoads(): time spent is #" << (double)final / ((double)CLOCKS_PER_SEC) << " seconds \n";
        ...
        //  cerr<<"Domain_Reduction_Method_Modified_Input::CompPBLoads(): Zero out the diagonal block of Boundary nodes is done ..."<<endln;
        ...
        //cerr<<"Domain_Reduction_Method_Modified_Input::CompPBLoads(): Zero out the diagonal block of Exterior nodes is done ..."<<endln;
        ...
        for ( int t = 0; t < thetimeSteps; t++)
        {
            if (i == 2315)
            {
                std::cerr << "Domain_Reduction_Method_Modified_Input::CompPBLoads(): ELE # " << i << " I-*-*-*-*-t: " << t << "\n";
            }

            //------Replaced by Babak to do binary search  7/310/13 ----
            ...
            if (i == 2315)
            {
                std::cerr << "Domain_Reduction_Method_Modified_Input::CompPBLoads(): ELE # " << i << "III- *-*-*-*-t: " << t << "\n";
            }
