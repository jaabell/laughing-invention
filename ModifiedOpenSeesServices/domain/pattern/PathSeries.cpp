      ...
      thePath(0), pathTimeIncr(theTimeIncr), cFactor(theFactor), otherDbTag(0), lastSendCommitTag(-1)
{
    ...
    if (thePath == 0 || thePath->Size() == 0)
    {
        ...
        cerr << " a Vector of size: " <<  theLoadPath.Size() << endln;
...
PathSeries::getDuration()
{
    if (thePath == 0)
    {
        cerr << "WARNING -- PathSeries::getDuration() on empty Vector" << endln;
        return 0.0;
    }
...
PathSeries::getPeakFactor()
{
    if (thePath == 0)
    {
        cerr << "WARNING -- PathSeries::getPeakFactor() on empty Vector" << endln;
        return 0.0;
    }
