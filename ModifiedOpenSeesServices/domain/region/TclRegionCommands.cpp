                 ...
                 TCL_Char** argv, Domain& theDomain)
{
    ...
    {
        cerr << "WARNING region tag? .. - invalid tag " << argv[loc] << endln;
        return TCL_ERROR;
    }
    ...
    while (loc < argc)
    {
        ...
        else if (strcmp(argv[loc], "-eleRange") == 0)
        {
            ...
            {
                cerr << "WARNING region tag? -eleRange start? end? - invalid start " << argv[loc + 1] << endln;
                return TCL_ERROR;
            }

            if (Tcl_GetInt(interp, argv[loc + 2], &end) != TCL_OK)
            {
                cerr << "WARNING region tag? -eleRange start? end? - invalid end " << argv[loc + 2] << endln;
                return TCL_ERROR;
            }
        ...
        else if (strcmp(argv[loc], "-nodeRange") == 0)
        {
            ...
            if (Tcl_GetInt(interp, argv[loc + 2], &end) != TCL_OK)
            {
                cerr << "WARNING region tag? -eleRange start? end? - invalid end " << argv[loc + 1] << endln;
                return TCL_ERROR;
            }
        ...
        else if (strcmp(argv[loc], "-rayleigh") == 0)
        {
            ...
            {
                cerr << "WARNING region tag? .. -rayleigh aM bK bK0 - invalid aM " << argv[loc + 1] << endln;
                return TCL_ERROR;
            }

            if (Tcl_GetDouble(interp, argv[loc + 2], &betaK) != TCL_OK)
            {
                cerr << "WARNING region tag? .. -rayleigh aM bK bK0 - invalid bK " << argv[loc + 2] << endln;
                return TCL_ERROR;
            }

            if (Tcl_GetDouble(interp, argv[loc + 3], &betaK0) != TCL_OK)
            {
                cerr << "WARNING region tag? .. -rayleigh aM bK bK0 - invalid bK0 " << argv[loc + 3] << endln;
                return TCL_ERROR;
            }

            if (Tcl_GetDouble(interp, argv[loc + 4], &betaKc) != TCL_OK)
            {
                cerr << "WARNING region tag? .. -rayleigh aM bK bK0 - invalid bKc " << argv[loc + 4] << endln;
                return TCL_ERROR;
            }
    ...
    {
        cerr << "WARNING could not add to domain - region " << tag << endln;
