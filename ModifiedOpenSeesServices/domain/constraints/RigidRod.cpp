...
RigidRod::RigidRod(Domain& theDomain, int nR, int nC, int startMPtag)
{
    ...
    if (nodeR == 0)
    {
        cerr << "RigidRod::RigidRod - retained Node" <<  nR <<  "not in domain\n";
        return;
    }
    ...
    if (nodeR == 0)
    {
        cerr << "RigidRod::RigidRod - constrained Node" <<  nC <<  "not in domain\n";
        return;
    }
    ...
    if (dimR != dimC)
    {
        cerr << "RigidRod::RigidRod - mismatch in dimension " <<
    ...
    {
        cerr << "RigidRod::RigidRod - mismatch in numDOF " <<
             "between constrained Node " <<  nC <<  " and Retained node " << nR << endln;
        return;
    }
    ...
    if(numDOF < dimR)
    {
        cerr << "RigidRod::RigidRod - numDOF at nodes " << nR << " and " << nC <<
    ...
    if (newC == 0)
    {
        cerr << "RigidRod::RigidRod - for nodes " << nR << " and " << nC << " out of memory\n";
        exit(-1);
    }
    else
    {
        ...
        {
            cerr << "RigidRod::RigidRod - for nodes " << nC << " and " << nR << " could not add to domain\n",
                 delete newC;
        }
    }
}
