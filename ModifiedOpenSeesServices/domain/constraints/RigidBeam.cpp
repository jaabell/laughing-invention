...
RigidBeam::RigidBeam(Domain& theDomain, int nR, int nC, int startMPtag)
{
    ...
    if (nodeR == 0)
    {
        cerr << "RigidBeam::RigidBeam - retained Node" <<  nR <<  "not in domain\n";
        return;
    }
    ...
    if (nodeR == 0)
    {
        cerr << "RigidBeam::RigidBeam - constrained Node" <<  nC <<  "not in domain\n";
        return;
    }
    ...
    if (dimR != dimC)
    {
        cerr << "RigidBeam::RigidBeam - mismatch in dimension "  <<
    ...
    {
        cerr << "RigidBeam::RigidBeam - mismatch in numDOF "  <<
             "between constrained Node " <<  nC <<  " and Retained node" << nR << endln;
        return;
    }
    ...
    if(numDOF < dimR)
    {
        cerr << "RigidBeam::RigidBeam - numDOF at nodes " <<
    ...
    if (dimR != numDOF)
    {
        ...
        {
            cerr << "RigidBeam::RigidBeam -  for nodes " <<
    ...
    if (newC == 0)
    {
        cerr << "RigidBeam::RigidBeam - for nodes " << nC << " and " << nR << ", out of memory\n";
    }
    else
    {
        ...
        {
            cerr << "RigidBeam::RigidBeam - for nodes " << nC << " and " << nR << ", could not add to domain\n";
            delete newC;
        }
    }
}
