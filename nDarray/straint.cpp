...
//##############################################################################
double straintensor::volumetric_strain() const
{
    return( (this->Iinvariant1()) );
}
...
double straintensor::theta()  const             // Chen W.F. "plasticity for
{
    ...
    if ( (fabs(temp6)) <= fabs(temp5 * EPS) )
    {
        return ( 0.000001 );    // slight perturbation because of 1/0 KERU06sep96
    }
    ...
    if ( tempabs1 < 0.001 )
    {
        return ( 0.000001 / 3.0 );    // slight perturbation because of 1/0 KERU06sep96
    }
