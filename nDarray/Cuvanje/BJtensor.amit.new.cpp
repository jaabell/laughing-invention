...
BJtensor BJtensor::operator*( BJtensor& arg)
{
    ...
    ::fprintf(stderr, "\n \n \n \n \n -------------------------------------------\n"); //Amit
    ::fprintf(stderr, "this_indices = %6s ; arg_indices = %6s \n", this_indices, arg_indices); //Amit
...
arg.rank() = %d\n",
    ...
    ::fprintf(stderr, "uncontr_counter=%6d      this_uni_count=%6d        arg_uni_count=%6d \n", uncontr_counter, this_uni_count, arg_uni_count); //Amit
    ...
    int* final_integer_array_for_for_both_indices = new int[10];
    compare_ind( this_indices,
                 arg_indices,
                 this_indices_number,
                 arg_indices_number,
                 final_integer_array_for_for_both_indices);


    ::fprintf(stderr, "\n");

    for (int counter = 0 ; counter < this_indices_number + arg_indices_number ; counter++)
    {
        ::fprintf(stderr, "final_integer_array_for_for_both_indices[%d] = %d \n",
                  counter,
                  final_integer_array_for_for_both_indices[counter]);
        //  store[10][8]=c[counter];
    }
    ...
    /////////////////////////////////////////
    static int store[20][8];     ////CHECK if it indeed initializes to zero for all compilers
    static int how_many_in_that_cathegory[100];
    static int current_rows_filled_up = -1;
    int True = 1;
    /////////////////////////////////////////
    ...
    if  (current_rows_filled_up == -1)
    {
        for( int positions = 0 ;  positions < 8 ; positions++ )
        {
            store[row_counter][positions] = final_integer_array_for_for_both_indices[positions];
        }

        current_rows_filled_up++;
        row_counter++;
    }

    // for all other instances go through this

    else

    {

        while (row_counter <= current_rows_filled_up )


        {
            for( int positions = 0 ;  positions < 8 ; positions++ )
            {
                if (store[row_counter][positions] != final_integer_array_for_for_both_indices[positions] )
                {
                    break;
                }
            ...
            if  (row_counter >= current_rows_filled_up)
            {
                for( int positions = 0 ;  positions < 8 ; positions++ )
                {
                    store[row_counter][positions] = final_integer_array_for_for_both_indices[positions] ;
    ...
    for (int i = 0 ; i < 20 ; i++)
    {
        ::fprintf(stderr, " store [%3d] [0-7] = %1d %1d %1d %1d %1d %1d %1d %1d \n",
                  i, store[i][0],
                  store[i][1],
                  store[i][2],
                  store[i][3],
    ...
    // -    //if ( store[j][i]!=c[i] ) {
    // -
    // -   //     ::fprintf(stderr,"store[%d][%d] = store[10][i] \n",);
    // -       cout<<store[10][i];
    // -
    // -      }
    // -      }
    ...
    // -    //   for(int counter=0; counter < this_indices_number + arg_indices_number; counter++)
    // -     //{
    // -       //if ( store[10][counter] != c[counter] )
    // -     //{
    // -       //store[10][counter] =  c[counter];
    // -
    // -     //}
    // -     //}
    // -
    ...
    // -  //int tempi=0, tempj=0;
    // -  //int i=10,j=8;
    // -  //for (int counter=0; counter<this_indices_number+arg_indices_number && tempi<i;)
    // -  //{
    // -
    // -  //tempi++;
    // -  //for (tempj=0;tempi<i && tempj<j; tempj++)
    // -  //{
    // -
    // -    // if ( store[tempi][tempj]!=c[counter++] ) {
    // -     // cout<<store[tempi][tempj];
    // -
    // -     //}
    // -     //}
    // -     //}
    ...
    // -        for(int counter=0; counter < this_indices_number + arg_indices_number; counter++)
    // -          {
    // -            if ( final_integer_array_for_for_both_indices[counter] != store[10][counter] )
    // -              {
    // -                store[10][counter] =  final_integer_array_for_for_both_indices[counter];
    // -              k++;
    // -              }
    // -          }
    // -      }
    ...
    //::fprintf(stderr,"%10s",results_indices);//Amit
    ...
    //  ::fprintf(stderr,"%10",results_indices);//Amit
                            ...
                            for ( cd[1] = 1 ; cd[1] <= inerr_dims[1] ; cd[1]++ )
                                for ( cd[0] = 1 ; cd[0] <= inerr_dims[0] ; cd[0]++ )
                                {
                                    ...
                                    for ( t = 0 ; t < contr_counter ; t++ )
                                    {
                                        ...
                                        //   ::fprintf(stderr,"this contracted  t=%d  lid[%d]=%d ",
                                        //     t,this_contr[t]-1,lid[this_contr[t]-1]);
                                    }
                                    ...
                                    for ( t = 0 ; t < contr_counter ; t++ )
                                    {
                                        ...
                                        //  ::fprintf(stderr,"arg contracted  t=%d  rid[%d]=%d    ",
                                        //t,arg_contr[t]-1,rid[arg_contr[t]-1]);
                                    }
...
void BJtensor::compare_ind(const char*   this_indices,
                           const char*   arg_indices,
                           int this_indices_number,
                           int arg_indices_number,
                           int* final_integer_array_for_for_both_indices)

{

    int leftcontractedint;
    int rightcontractedint;

    int leftno[4] = {0, 0, 0, 0};
    int rightno[4] = {0, 0, 0, 0};

    //  int c[8]={0,0,0,0,0,0,0,0};


    //int type1[8]={1,2,3,4,1,2,7,8};
    ...
    ::fprintf(stderr, "\n");
    ::fprintf(stderr, "String length for this_indices_number = %d\n", this_indices_number);
    ::fprintf(stderr, "String length for arg_indices_number = %d\n", arg_indices_number);

    //#############################################################################################

    int i = 0;

    for (i = 0 ; i < this_indices_number; i++) //Generate integers for left indices
    {
        leftno[i] = i + 1;
        // cout<<leftno[i]<<i<<endl;
    }

    //#############################################################################################

    for (i = 0 ; i < arg_indices_number ; i++) //Generate integers for right indices
    {
        rightno[i] = i + this_indices_number + 1;
        //cout<<rightno[j]<<j<<endl;
    }
    ...
    for (i = 0 ; i < this_indices_number ; i++)
    {
        for (j = 0 ; j < arg_indices_number  ; j++)
        {
            if (this_indices[i] == arg_indices[j])
            {
                //       cout<<"left contracted indices number= "<<i+1<<endl<<"Right contracted indices number= "<<j+1<<endl<<a[i]<<endl<<b[j]<<endl;
                leftcontractedint = i + 1;
                rightcontractedint = j + this_indices_number + 1;
                rightno[j] = leftcontractedint;      //trying to replace right indices with left contracted members
            }
        }
    }


    //############################### store changed numbers############################
    //  for (i=0 ; i<this_indices_number ; i++)
    //    {
    //     // cout<<leftno[i];
    //    }
    //
    //  for (i=0;i<arg_indices_number  ;i++ )
    //    {
    //     // cout<<rightno[i];
    //    }
    //  cout<<endl;

    //################################## storing 2 arrays in a single array################

    for (i = 0; i < this_indices_number; i++)
    {
        final_integer_array_for_for_both_indices[i] = leftno[i];
    }

    for (i = 0; i < arg_indices_number; i++)
    {
        final_integer_array_for_for_both_indices[i + this_indices_number] = rightno[i];
        // ::fprintf(stderr,"%10",c[i]);//Amit
    }
                             ...
                             int     argright_ind_numb)
{
    ...
    ::fprintf(stderr, "(inside contracted indices) contr_counter = %6d\n", contr_counter); //Amit
                               ...
                               int tens_ind_numb)
{
            ...
            tens_unindices_counter++ )
    {
                ...
                tens_indices_counter++ )
        {
            ...
            //::fprintf(stderr,"CHECK tens_uncontr[%d]=%d",tens_uni_count, tens_uncontr[tens_uni_count]);
            //::fprintf(stderr,"  WITH tens_contr[%d]=%d\n",tens_contr[t_i_c]);
