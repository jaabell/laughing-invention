    ...
    : NDMaterial(tag, ND_TAG_Template3Dep)
{
    ...
    if (theElasticMat == 0)
    {
        cerr << "Template3Dep:: Template3Dep failed to get copy of material\n";
        exit(-1);
    }
    ...
    {
        cerr << "Template3Dep:: Template3Dep failed to construct the template3Dep\n";
        exit(-1);
    }
...
tensor Template3Dep::ElasticComplianceTensor(void) const
{
    ...
    //ZC05/2004         if (Ey == 0)
    //ZC05/2004           {
    //ZC05/2004             cerr << endln << "Ey = 0! Can't give you D!!" << endln;
    //ZC05/2004             exit(1);
    //ZC05/2004           }
    //ZC05/2004
    ...
    //ZC05/2004     else
    //ZC05/2004       {
    //ZC05/2004       cerr << "Template3Dep::ElasticComplianceTensor failed to create elastic compliance tensor. Eflag must be 0-3: " <<  eflag << endln;
    //ZC05/2004       exit(-1);
    //ZC05/2004
    //ZC05/2004       }
    //ZC05/2004
...
tensor Template3Dep::ElasticStiffnessTensor(void) const
{
    ...
    {
        cerr << "Template3Dep::theElasticMat setTrialStrain failed in material with strain ";
        return -1;
    }
    ...
    //ZC05/2004     else
    //ZC05/2004       {
    //ZC05/2004       cerr << "Template3Dep::ElasticStiffnessTensor failed to create elastic compliance tensor. Eflag must be 0-3: " <<  eflag << endln;
    //ZC05/2004       exit(-1);
    //ZC05/2004
    //ZC05/2004       }
    //ZC05/2004    return ret;
...
int Template3Dep::setTrialStrainIncr(const Tensor& v)
{
    ...
    //cerr << "start_stress 0 " << start_stress;
    ...
    //   cerr << " Step Start Stress:" << start_stress << endln;
    //
    //   loop += 1;
    ...
    {
        cerr << "Template3Dep::setTrialStrainIncr, error when getting integrator flag from EPState! \n";
        exit(1);
    }
...
NDMaterial* Template3Dep::getCopy(const char* code)
{
    ...
    {
        cerr << "Template3Dep::getCopy failed to get model: " <<  code << endln;
        exit(-1);
    }
...
EPState Template3Dep::ForwardEulerEPState( const straintensor& strain_increment)
{
    ...
    //cerr << " stress_increment: " << stress_increment << endlnn;

    EPState startEPS( *(getEPS()) );
    stresstensor start_stress = startEPS.getStress();
    start_stress.null_indices();
    //cerr << "===== start_EPS =====: " << startEPS;
    ...
    //cerr << " Elastic_predictor_stress: " << elastic_predictor_stress << endlnn;
    ...
    if ( ( (f_start <= 0) && (f_pred <= 0) ) || (f_start > f_pred) )
    {
        ...
        //cerr <<" strain_increment.Iinvariant1() " << strain_increment.Iinvariant1() << endlnn;
    ...
    if ( f_start <= 0 && f_pred > 0 )
    {
        ...
        //cerr  << "    start_stress: " <<  start_stress << endlnn;
        //cerr  << "    Intersection_stress: " <<  intersection_stress << endlnn;
    ...
    //cerr <<"elpl start eps: " <<   forwardEPS;
    //double f_cross =  this->getYS()->f( &forwardEPS );
    //cerr << " #######  f_cross = " << f_cross << "\n";
    ...
    //cerr << " inside ConstitutiveDriver after setInitD " << forwardEPS;
    ...
    if ( f_pred >= 0 )
    {
        ...
        //cerr << "dF/ds" << dFods << endlnn;
        //cerr << "dQ/ds" << dQods << endlnn;
        ...
        //cerr << "FE : lower " << lower << " hardMod_ " << hardMod_ << endln;
        //Calculating Kp according to Kp = - (df/dq*) * qbar
        //double Kp = material_point.EL->getKp(&forwardEPS, norm_dQods);
        //Kp = 0.0;
        //cerr << endlnn << ">>>>>>>>>   Lower = " << lower << endlnn;
        //lower = lower + Kp;
        //cerr << endlnn << ">>>>>>>>>    Kp = " << Kp << endlnn;

        //cerr << " stress_increment "<< stress_increment << endlnn;
        //cerr << " true_stress_increment "<< true_stress_increment << endlnn;

        temp3 = dFods("ij") * true_stress_increment("ij"); // L_ij * E_ijkl * d e_kl (true ep strain increment)
        temp3.null_indices();
        //cerr << " temp3.trace() -- true_stress_incr " << temp3.trace() << endln;
        //GZ  temp3 = temp1("ij")*strain_incr("ij");
        //GZ  temp3.null_indices();
        //cerr << " temp3.trace() " << temp3.trace() << endlnn;
        Delta_lambda = (temp3.trace()) / lower;

        //cerr << "FE: Delta_lambda " <<  Delta_lambda << endln;
        if (Delta_lambda < 0.0)
        {
            Delta_lambda = 0.0;
        }
        ...
        //cerr << " Delta_lambda " << Delta_lambda << "plastic_stress =   " << plastic_stress << endln;
        //cerr << "plastic_stress =   " << plastic_stress << endlnn;
        //cerr << "plastic_strain =   " << plastic_strain << endlnn;
        //cerr << "plastic_strain I1= " << plastic_strain.Iinvariant1() << endlnn;
        //cerr << "plastic_strain vol " << Delta_lambda * ( forwardEPS.getScalarVar( 2 ) )<< endlnn ;
        //cerr << "  q=" << Delta_lambda * dQods.q_deviatoric()<< endlnn;
        ...
        //cerr << "elastic_strain I1=" << elastic_strain.Iinvariant1() << endlnn;
        //cerr << "elastic_strain " << elastic_strain << endlnn;
        //cerr << "strain increment I1=" << strain_increment.Iinvariant1() << endlnn;
        //cerr << "strain increment    " << strain_increment << endlnn;
        ...
        */
        ...
        //cerr <<" after calculation---Eep.rank()= " << Eep.rank() <<endlnn;
        ...
        //cerr <<"inside constitutive driver: forwardEPS "<< forwardEPS;

        forwardEPS.setEep(Eep);
        //forwardEPS.getEep().printshort(" after set");

        //Before update all the internal vars
        //double f_forward =  //J.Abell : Dec 2012 -> is not used
        getYS()->f( &forwardEPS );
        ...
        //        int err = 0; //J.Abell : Dec 2012 -> is not used
        if ( getELT1() )
        {
        ...
        //cerr << "UpdateAllVars " << forwardEPS<< endlnn;

        //After update all the internal vars
        //f_forward =  //J.Abell : Dec 2012 -> is not used
        getYS()->f( &forwardEPS );
...

{
    ...
    //cerr.precision(5);
    //cerr.width(10);
    //cerr << " strain_increment " << strain_increment << "\n";

    //cerr.precision(5);
    //cerr.width(10);
    //cerr << "start_stress " <<  start_stress;
    ...
    if ( f_pred <= Ftolerance ) //Guanzhou changed
    {
        ...
        //cerr<< "Elastic:  Total strain" << tstrain << endln;
    ...
    if ( f_pred > Ftolerance )
    {
        ...
        //cerr << " ----------Predictor Stress" << EP_PredictorEPS.getStress();
        ...
        //cerr <<  " BE: F_elplpredictor " << Felplpredictor << endln;
        ...
        //Guanzhou out Mar2005 //cerr << "elastic_plastic_predictor_stress" << elastic_plastic_predictor_stress;

        //cerr.precision(5);
        //cerr.width(10);
        //cerr << " " << EP_PredictorEPS.getStress().p_hydrostatic() << " ";

        //cerr.precision(5);
        //cerr.width(10);
        //cerr << EP_PredictorEPS.getStress().q_deviatoric()<< " ";

        //cerr.precision(5);
        //cerr.width(10);
        //cerr << Delta_lambda << endlnn;
        ...
        //..new_S = S + dS;
        //..cerr << "Internal Var : " << new_S << endlnn;
        ...
        while  (( fabs(fabs(Felplpredictor) - fabs(Felplold)) > 1.0e-6 )
            ...
            //out07may97      do
        {
            //cerr << "Iteration " << step_counter << " F " << Felplpredictor << " delta " << Delta_lambda << endln;
            ...
            //cerr << "upper " << upper << " lower " << lower << endln;
            Delta_lambda = Delta_lambda + delta_lambda;

            if ( Delta_lambda < 0.0 )
            {
                Delta_lambda = 0.0;    //Guanzhou
            }
            ...
            //    int err; //J.Abell : Dec 2012 -> is not used
            if ( getELT1() )
            {
                double pl_st_vol = pstrain.Iinvariant1(); //Joey 02-17-03
                // D > 0 compressive -> Iinv > 0  -> de < 0 correct!
                //err = //J.Abell : Dec 2012 -> is not used
                getELT1()->updateEeDm(&EP_PredictorEPS, pl_st_vol, Delta_lambda);
            }
        ...
        if ( step_counter >= MAX_STEP_COUNT  )
        {
            ...
            //Guanzhou
            cerr << "Template3Dep::BackwardEuler(), failed to converge in " << step_counter << "steps!!!" << '\n';
        ...
        //GZ out //cerr<< "Plastic:  Total strain" << tstrain <<endln;
        //GZ out
        ...
        int number_of_subincrements)
{
    ...
    //cerr << "in FESubIncrementation MP " << MP;
    ...
    //cerr << "INCREMENT strain " << strain_increment << endlnn ;
    //cerr << "SUB INCREMENT strain " << elastic_subincremental_strain << endlnn ;
    ...
    for( int steps = 0 ; steps < number_of_subincrements ; steps++ )
    {
        ...
        //cerr.unsetf(ios::showpos);
        //cerr << setw(4);
        //cerr << "Step No. " << steps << "  ";

        //cerr.setPrecision(SCIENTIFIC);
        //cerr.precision(3);
        ...
        //cerr << setw(7);
        //cerr << "p " << back_stress.p_hydrostatic() << "  ";
        //cerr << setw(7);
        //cerr << "q " << back_stress.q_deviatoric() << "  ";
        //cerr << setw(7);
        //cerr << " theta " << back_stress.theta() << "  ";
        //cerr << setw(7);
        //cerr << "alfa1 " << FESI_EPS.getScalarVar(1) << "  ";
        //cerr << setw(7);
        //cerr << "f = " << getYS()->f( &FESI_EPS ) << endlnn;
        ...
        int number_of_subincrements)
{
    ...
    for( int steps = 0 ; steps < number_of_subincrements ; steps++ )
    {
        ...
        {
            //this->setEPS( BESI_EPS );
            cerr << "Template3Dep::BESubIncrementation  failed to converge at " << steps << "th(of "
            ...
            //cerr.precision(3);
            //cerr << setw(7);
            //cerr << " back-stress  p " << back_stress.p_hydrostatic() << "  ";
            //cerr << setw(7);
            //cerr << "q " << back_stress.q_deviatoric() << "  ";
            //cerr << setw(7);
            //cerr << "alfa1 " << BESI_EPS.getScalarVar(1) << "  ";
            //cerr << setw(7);
            //cerr << "f = " << MP->YS->f( &BESI_EPS ) << "  "<< endlnn;
            ...
            //   cerr.width(7);
            //   cerr << "q " << begin_stress.q_deviatoric() << "  ";
            //   cerr.width(7);
            //   cerr << "alfa1 " << old_EPS.getScalarVar(1) << "  ";
            //   //cerr.width(7);
            //   //cerr << "f = " << MP->YS->f( &BESI_EPS ) << "  "<< endlnn;
            //
            //   cerr << "strain increment " << strain_incr << endln;
            //
            //fflush(cerr);
            break;
            //exit(1);
...
//  {
//    if (E == 0) {
//      cerr << endln << "Ey = 0! Can't give you D!!" << endln;
//      exit(1);
//    }
//
//    tensor ret( 4, def_dim_4, 0.0 );
...
int Template3Dep::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    idData(1) = YS->getClassTag();
    idData(2) = PS->getClassTag();

    if(ELS1 != 0)
    {
        idData(3) = ELS1->getClassTag();
    }

    if(ELS2 != 0)
    {
        idData(4) = ELS2->getClassTag();
    }

    if(ELS3 != 0)
    {
        idData(5) = ELS3->getClassTag();
    }

    if(ELS4 != 0)
    {
        idData(6) = ELS4->getClassTag();
    }

    if(ELT1 != 0)
    {
        idData(7) = ELT1->getClassTag();
    }

    if(ELT2 != 0)
    {
        idData(8) = ELT2->getClassTag();
    }

    if(ELT3 != 0)
    {
        idData(9) = ELT3->getClassTag();
    }

    if(ELT4 != 0)
    {
        idData(10) = ELT4->getClassTag();
    }

    //Guanzhou, for elastic part
    idData(11) = (this->getElMat())->getClassTag();
    //idData(12) = (this->getElMat())->getTag();

    //    idData(3) = EPS->getClassTag();

    if (theChannel.sendID(this->getDbTag(), commitTag, idData) < 0)
    {
        cerr << "Template3Dep::sendSelf -- failed to send ID\n";
        return -1;
    }

    //Now call sendSelf on the objects
    //    res += YS->sendSelf(commitTag, theChannel);
    //    res += PS->sendSelf(commitTag, theChannel);
    if (EPS->sendSelf(commitTag, theChannel) < 0)
    {
        cerr << "Template3Dep::sendSelf -- EPS failed to send self\n";
        return -1;
    }

    if(ELS1 != 0)
        if (ELS1->sendSelf(commitTag, theChannel) < 0)
        {
            cerr << "Template3Dep::sendSelf -- failed to send ELS1\n";
            return -1;
        }

    if(ELS2 != 0)
        if (ELS2->sendSelf(commitTag, theChannel) < 0)
        {
            cerr << "Template3Dep::sendSelf -- failed to send ELS2\n";
            return -1;
        }

    if(ELS3 != 0)
        if (ELS3->sendSelf(commitTag, theChannel) < 0)
        {
            cerr << "Template3Dep::sendSelf -- failed to send ELS3\n";
            return -1;
        }

    if(ELS4 != 0)
        if (ELS4->sendSelf(commitTag, theChannel) < 0)
        {
            cerr << "Template3Dep::sendSelf -- failed to send ELS4\n";
            return -1;
        }

    if(ELT1 != 0)
        if (ELT1->sendSelf(commitTag, theChannel) < 0)
        {
            cerr << "Template3Dep::sendSelf -- failed to send ELT1\n";
            return -1;
        }

    if(ELT2 != 0)
        if (ELT2->sendSelf(commitTag, theChannel) < 0)
        {
            cerr << "Template3Dep::sendSelf -- failed to send ELT2\n";
            return -1;
        }

    if(ELT3 != 0)
        if (ELT3->sendSelf(commitTag, theChannel) < 0)
        {
            cerr << "Template3Dep::sendSelf -- failed to send ELT3\n";
            return -1;
        }

    if(ELT4 != 0)
        if (ELT4->sendSelf(commitTag, theChannel) < 0)
        {
            cerr << "Template3Dep::sendSelf -- failed to send ELT4\n";
            return -1;
        }

    if (this->getElMat()->sendSelf(commitTag, theChannel) < 0)
    {
        cerr << "Template3Dep::sendSelf -- failed to send the Elastic part\n";
        return -1;
    }
...
int Template3Dep::recvSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
    ...
    /*FEM_ObjectBroker *theLocalBroker = new FEM_ObjectBroker();
    YS = theLocalBroker->getYieldSurfacePtr(idData(1));
    PS = theLocalBroker->getPotentialSurfacePtr(idData(2));
    if(idData(3) != 0)
        ELS1 = theLocalBroker->getEL_S(idData(3));
    if(idData(4) != 0)
        ELS2 = theLocalBroker->getEL_S(idData(4));
    if(idData(5) != 0)
        ELS3 = theLocalBroker->getEL_S(idData(5));
    if(idData(6) != 0)
        ELS4 = theLocalBroker->getEL_S(idData(6));
    if(idData(7) != 0)
        ELT1 = theLocalBroker->getEL_T(idData(7));
    if(idData(8) != 0)
        ELT2 = theLocalBroker->getEL_T(idData(8));
    if(idData(9) != 0)
            ELT3 = theLocalBroker->getEL_T(idData(9));
    if(idData(10) != 0)
        ELT4 = theLocalBroker->getEL_T(idData(10));*/


    YS = theBroker.getYieldSurfacePtr(idData(1));
    PS = theBroker.getPotentialSurfacePtr(idData(2));

    if(idData(3) != 0)
    {
        ELS1 = theBroker.getEL_S(idData(3));
    }

    if(idData(4) != 0)
    {
        ELS2 = theBroker.getEL_S(idData(4));
    }

    if(idData(5) != 0)
    {
        ELS3 = theBroker.getEL_S(idData(5));
    }

    if(idData(6) != 0)
    {
        ELS4 = theBroker.getEL_S(idData(6));
    }

    if(idData(7) != 0)
    {
        ELT1 = theBroker.getEL_T(idData(7));
    }

    if(idData(8) != 0)
    {
        ELT2 = theBroker.getEL_T(idData(8));
    }

    if(idData(9) != 0)
    {
        ELT3 = theBroker.getEL_T(idData(9));
    }

    if(idData(10) != 0)
    {
        ELT4 = theBroker.getEL_T(idData(10));
    }

    EPS = new EPState();

    //EPS = theBroker.getEPState();
...
//Guanzhou added
int
Template3Dep::getObjectSize()
{
    int size = 0;

    size += 12 * sizeof(int);

    size += EPS->getObjectSize();

    if(ELS1 != 0)
    {
        size += ELS1->getObjectSize();
    }

    if(ELS2 != 0)
    {
        size += ELS2->getObjectSize();
    }

    if(ELS3 != 0)
    {
        size += ELS3->getObjectSize();
    }

    if(ELS4 != 0)
    {
        size += ELS4->getObjectSize();
    }

    if(ELT1 != 0)
    {
        size += ELT1->getObjectSize();
    }

    if(ELT2 != 0)
    {
        size += ELT2->getObjectSize();
    }

    if(ELT3 != 0)
    {
        size += ELT3->getObjectSize();
    }

    if(ELT4 != 0)
    {
        size += ELT4->getObjectSize();
    }

    size += this->getElMat()->getObjectSize();
