...
EvolutionLaw_NL_EijMD::EvolutionLaw_NL_EijMD(
    ...
    double Cfd):// = 100):
    //double ed  = 0.85,
    EvolutionLaw_T(ELT_TAG_NL_EijMD), eo(eod), a(ad), Mc(Mcd), Me(Med), Lambda(Lambdad), ec_ref(ec_refd), p_ref(p_refd),
...
EvolutionLaw_NL_EijMD::EvolutionLaw_NL_EijMD(const EvolutionLaw_NL_EijMD& MDE )
    : EvolutionLaw_T(ELT_TAG_NL_EijMD)
{
...
tensor EvolutionLaw_NL_EijMD::h_t( EPState* EPS, PotentialSurface* PS)
{
    ...
    {
        cerr << "EvolutionLaw_NL_EijMD::h_t  |n_ij| = 0, divide by zero! Program exits.\n";
        exit(-1);
    }
    ...
    {
        //cerr << " p: " << p;
        cerr << "EvolutionLaw_NL_EijMD::h_t   p < 0, Program exits.\n";
        exit(-1);
    }
    ...
    //cerr << "theta0 = " << theta0/3.1416*180;
    //cerr << "  theta = " << theta/3.1416*180 << endln;
    ...
    //  //cerr << "alpha_theta_d " << alpha_theta_d<<" g_WW(theta, c) "<< g_WW(theta, c) << endln;

    double cb = getke_b() / getkc_b();

    if ( xi > 0.0 )
    {
        xi = 0.0;    // < -xi > for alpha_theta_b
    }
    ...
    if (bn < 0)
    {
        bn = 0;    // Added Joey 02-19-03
    }

    double abs_bn = fabs( sqrt(bn) );
    //cerr << ">>>> alpha_c_b " << alpha_c_b;
    //cerr << " alpha_theta_b " << alpha_theta_b;
    //cerr << " alpha " << alpha;
    //cerr << " \nn " << n;

    //double h = getho() * abs_bn /( b_ref - abs_bn );
    double h = getho() * abs_bn / ( b_ref - abs_bn );
    //cerr << " ||| b_ref =" << b_ref << " bn =" << bn << " abs(bn) =" << abs_bn  << " h =" << h << endln;
    //cerr << " " << h << endln;

    //Updating D and F---need to fine tune
    //temp1 = d("ij") * n("ij");
    //double dn = temp1.trace();
    //cerr << "bn =" << bn << "  dn =" << dn << endln;
...
int EvolutionLaw_NL_EijMD::updateEeDm(EPState* EPS, double st_vol, double dLamda)
{
    ...
    //cerr << " ++st_vol = " << st_vol << " de = " << de << " e = " << e << " p = " << p << " ec = " << ec << " xi " << xi;

    //double Do = EPS->getScalarVar(2);
    //cerr << " !!!!! Do = " << Do;
    //if ( dLamda < 0)
    //cerr << " dLamda = " << dLamda << endln;
    ...
    {
        cerr << "EvolutionLaw_L_EijMD::dFods  |n_ij| = 0, divide by zero! Program exits.\n";
        err += 1;
        exit(-1);
    }
    ...
    //
    double theta = r_bar.theta( );
    //cerr << "Update... theta = " << theta/3.1416*180 << endln;
    ...
    //Testing g_WW
    //cerr << "g_A(0, c) " << g_A(0.0, 0.0167) << endln;
    //cerr << "g_A(60, c) " << g_A(1.0472, 0.0167) << endln;
    double alpha_theta_dd = g_A(theta, c) * Mc + g_A(theta, cd) * getkc_d() * xi - m;
    stresstensor alpha_theta_d = n("ij") * alpha_theta_dd * sqrt23rd;
    ////cerr << " cd "<< cd << " c " << c << "alpha_theta_d_norm " << alpha_theta_dd << endln;
    //cerr << " alpha_theta_d " << alpha_theta_d; //<<" g_A(theta, cd) "<< g_A(theta, cd) << " Mc " << Mc << endln;
    //cerr << " alpha = " << alpha << "\n";
    ...
    this->D = newD;
    //cerr << " dn = "  << dn << " ** D = " << newD << endln << endln;
    ////cerr << "alpha_theta_d " << alpha_theta_d<<" g_WW(theta, c) "<< g_WW(theta, c);
