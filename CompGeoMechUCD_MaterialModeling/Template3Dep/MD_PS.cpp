...
MDPotentialSurface::MDPotentialSurface( )
    : PotentialSurface(PS_TAG_MDPotentialSurface)
{ }
...
tensor MDPotentialSurface::dQods(const EPState* EPS) const
{
    ...
    //}
    //else {
    //  cerr->fatal("MDYieldSurface::dQods |n_ij| = 0, divide by zero! Program exits.");
    //  exit(-1);
    //}
    //
...
tensor MDPotentialSurface::d2Qods2(const EPState* EPS) const
{
    ...
    if ( norm >= d_macheps() ){
      ...
      cerr << "MDPotentialSurface::dQods  |n_ij| = 0, divide by zero! Program exits.\n";
      exit(-1);
    }
    */
    ...
    if (p < 0.0)
    {
        cerr << "MDPotentialSurface::d2Qods2  p < 0, Program exits.\n";
        exit(-1);
    }
