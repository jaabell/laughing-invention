...
MDPotentialSurface01::MDPotentialSurface01(double pc )
    : PotentialSurface(PS_TAG_MDPotentialSurface01) //Guanzhou
{
    Pc = pc;
}
...
tensor MDPotentialSurface01::dQods(const EPState* EPS) const
{
    ...
    {
        cerr << "MDPotentialSurface01::dQods  |n_ij| = 0, divide by zero! Program exits.\n";
        exit(-1);
...
tensor MDPotentialSurface01::d2Qods2(const EPState* EPS) const
{
    ...
    if (p < 0.0)
    {
        cerr << "MDPotentialSurface01::d2Qods2  p < 0, Program exits.\n";
        exit(-1);
    }
...
// Boris Jeremic cleaning ostream 09Apr2008
