...
int
EvolutionLaw_NL_Eij::sendSelf(int commitTag, Channel& theChannel)
{
    int res = 0;
    ...
    if (res < 0)
    {
        cerr << "EvolutionLaw_NL_Eij::sendSelf() - failed to send data\n";
    }
...
//Guanzhou added for PDD
int
EvolutionLaw_NL_Eij::receiveSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
    int res = 0;
    static Vector data(2);
    ...
    if (res < 0)
    {
        cerr << "EvolutionLaw_NL_Eij::receiveSelf() - failed to recv data\n";
    }
    else
    {
        ha = data(0);
...
int
EvolutionLaw_NL_Eij::getObjectSize()
{
    int size = 0;
