...
// Concrete3D is another form of NewTemplate3Dep which has all
// the functions embeded here for efficiency.

#include "Concrete3D.h"
#include <math.h>

const  straintensor Concrete3D::ZeroStrain;
const  stresstensor Concrete3D::ZeroStress;
BJtensor Concrete3D::Ee(4, def_dim_4, 0.0);


//================================================================================
Concrete3D::Concrete3D(int tag,
                       double E_in,
                       double v_in,
                       double k_in,
                       double rho_in,
                       ...
                       double tolerance_1,
                       double tolerance_2)
    : NDMaterial(tag, ND_TAG_Concrete3D),
      E(E_in),
      v(v_in),
      k(k_in),
      rho(rho_in),
...
//================================================================================
Concrete3D::~Concrete3D()
{

}

//================================================================================
int Concrete3D::setTrialStrain(const Tensor& v)
{
    return this->setTrialStrainIncr( v - getStrainTensor() );
}

//================================================================================
int Concrete3D::setTrialStrainIncr(const Tensor& strain_increment)
{
...
//================================================================================
const BJtensor& Concrete3D::getTangentTensor(void)
{
    return this->Stiffness;
}

//================================================================================
const stresstensor&  Concrete3D::getStressTensor(void)
{
...
//================================================================================
const straintensor& Concrete3D::getStrainTensor(void)
{
    return this->TrialStrain;
}

//================================================================================
const straintensor& Concrete3D::getPlasticStrainTensor(void)
{
    return this->TrialPlastic_Strain;
}

//================================================================================
int Concrete3D::commitState(void)
{
...
//================================================================================
int Concrete3D::revertToLastCommit(void)
{
...
//================================================================================
int Concrete3D::revertToStart(void)
{
...
//================================================================================
NDMaterial* Concrete3D::getCopy(void)
{
    NDMaterial* tmp = new Concrete3D(this->getTag(),
                                     this->getE(),
                                     this->getv(),
                                     this->getk(),
                                     this->getRho(),
...
//================================================================================
NDMaterial* Concrete3D::getCopy(const char* code)
{
    if (strcmp(code, "ThreeDimensional") == 0)
    {
        Concrete3D* tmp = new Concrete3D( this->getTag(),
                                          this->getE(),
                                          this->getv(),
                                          this->getk(),
                                          this->getRho(),
...
//================================================================================
const char* Concrete3D::getType(void) const
{
    return "ThreeDimensional";
}

//================================================================================
int Concrete3D::sendSelf(int commitTag, Channel& theChannel)
{
    return 0;
}
...
//================================================================================
void Concrete3D::Print(ostream& s, int flag)
{
    s << (*this);
}

//================================================================================
double Concrete3D::getE()
{
    return E;
}

//================================================================================
double Concrete3D::getv()
{
    return v;
}

//================================================================================
double Concrete3D::getk()
{
    return k;
}

//================================================================================
double Concrete3D::getRho(void)
{
    return rho;
}

//================================================================================
double Concrete3D::getInitialConfiningStress(void)
{
...
double Concrete3D::YieldFunctionValue( const stresstensor& Stre) const
{

    // Invariants
    double I1 = Stre.trace();
    double J2 = Stre.Jinvariant2();
    double J3 = Stre.Jinvariant3();

    // Shear yield function Ff(I1)
    double Ff = alpha - lambda * exp(-beta * I1) + theta * I1;
    ...
    // X[kappa] =  L[kappa] + R*Ff(kappa)
    double Xk = Lk + R * (alpha - lambda * exp(-beta * Lk) + theta * Lk);

    // Cap yield function Fc(J1,kappa)
    double Fc = 1 - (J1 - Lk) * ( abs(J1 - Lk) + J1 - Lk  ) / 2 / pow(Xk - Lk, 2);

    // Lode Angle
    double sin3lode = 1.5 * sqrt(3) * J3 / sqrt(pow(J2, 3));
    double lode = asin(sin3lode) / 3;
...
//================================================================================
int Concrete3D::Explicit(const straintensor& strain_incr)
{
...
//================================================================================
stresstensor Concrete3D::yield_surface_cross(const stresstensor& start_stress,
...
//================================================================================
double Concrete3D::zbrentstress(const stresstensor& start_stress,
