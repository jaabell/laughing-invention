        ...
        TCL_Char** argv, TclModelBuilder* theTclBuilder, int eleArgStart)
{
    ...
    {
        std::cerr << "Warning: nDMaterial FiniteDeformationElastic3D - invalid tag " << argv[loc + 1] << "\n";
        exit (-1);
    }
    ...
    if ( (strcmp(argv[loc + 2], "NeoHookean3D") == 0) || (strcmp(argv[loc + 2], "NeoHookeanCompressible3D") == 0) )
    {
        ...
        if (argc < 7)
        {
            std::cerr << "Warning: NeoHookeanCompressible3D -insufficient number of arguments\n";
            exit (-1);
        }

        if (Tcl_GetDouble(interp, argv[loc + 3], &K_in) != TCL_OK)
        {
            std::cerr << "nDMaterial NeoHookeanCompressible3D - invalid K " << argv[loc + 3] << "\n";
            exit (-1);
        }

        if (Tcl_GetDouble(interp, argv[loc + 4], &G_in) != TCL_OK)
        {
            std::cerr << "Warning: nDMaterial NeoHookeanCompressible3D - invalid G " << argv[loc + 4] << "\n";
            exit (-1);
        }

        if (Tcl_GetDouble(interp, argv[loc + 5], &rho_in) != TCL_OK)
        {
            std::cerr << "Warning: nDMaterial NeoHookeanCompressible3D - invalid rho " << argv[loc + 5] << "\n";
            exit (-1);
        }
    ...
    else if ( (strcmp(argv[loc + 2], "DecoupledLog3D") == 0) || (strcmp(argv[loc + 2], "DecoupledLogarithmic3D") == 0) )
    {
        ...
        if (argc < 7)
        {
            std::cerr << "Warning: DecoupledLogarithmic3D -insufficient number of arguments\n";
            exit (-1);
        }

        if (Tcl_GetDouble(interp, argv[loc + 3], &K_in) != TCL_OK)
        {
            std::cerr << "Warning: nDMaterial DecoupledLogarithmic3D - invalid K " << argv[loc + 3] << "\n";
            exit (-1);
        }

        if (Tcl_GetDouble(interp, argv[loc + 4], &G_in) != TCL_OK)
        {
            std::cerr << "Warning: nDMaterial DecoupledLogarithmic3D - invalid G " << argv[loc + 4] << "\n";
            exit (-1);
        }

        if (Tcl_GetDouble(interp, argv[loc + 5], &rho_in) != TCL_OK)
        {
            std::cerr << "Warning: nDMaterial DecoupledLogarithmic3D - invalid rho " << argv[loc + 5] << "\n";
            exit (-1);
        }
        ...
        {
            std::cerr << "Error: nDMaterial DecoupledLogarithmic3D -invalid material model\n";
            exit (-1);
        }
    ...
    else if ( (strcmp(argv[loc + 2], "DecoupledNeoHookean3D") == 0) || (strcmp(argv[loc + 2], "DecoupledNH3D") == 0) )
    {
        ...
        if (argc < 7)
        {
            std::cerr << "Warning: DecoupledNeoHookean3D -insufficient number of arguments\n";
            exit (-1);
        }

        if (Tcl_GetDouble(interp, argv[loc + 3], &K_in) != TCL_OK)
        {
            std::cerr << "Warning: nDMaterial DecoupledNeoHookean3D - invalid K " << argv[loc + 3] << "\n";
            exit (-1);
        }

        if (Tcl_GetDouble(interp, argv[loc + 4], &G_in) != TCL_OK)
        {
            std::cerr << "Warning: nDMaterial DecoupledNeoHookean3D - invalid G " << argv[loc + 4] << "\n";
            exit (-1);
        }

        if (Tcl_GetDouble(interp, argv[loc + 5], &rho_in) != TCL_OK)
        {
            std::cerr << "Warning: nDMaterial DecoupledNeoHookean3D - invalid rho " << argv[loc + 5] << "\n";
            exit (-1);
        }
        ...
        {
            std::cerr << "Error: nDMaterial DecoupledNeoHookean3D -invalid material model\n";
            exit (-1);
        }
    ...
    else if ( (strcmp(argv[loc + 2], "DecoupledMooneyRivlinSimo3D") == 0) || (strcmp(argv[loc + 2], "DecoupledMRS3D") == 0) )
    {
        ...
        {
            std::cerr << "Warning: nDMaterial DecoupledNeoHookean3D - invalid c1 " << argv[loc + 3] << "\n";
            exit (-1);
        }

        if (Tcl_GetDouble(interp, argv[loc + 4], &c2_in) != TCL_OK)
        {
            std::cerr << "Warning: nDMaterial DecoupledNeoHookean3D - invalid c2 " << argv[loc + 4] << "\n";
            exit (-1);
        }

        if (Tcl_GetDouble(interp, argv[loc + 5], &K_in) != TCL_OK)
        {
            std::cerr << "Warning: nDMaterial DecoupledNeoHookean3D - invalid K " << argv[loc + 5] << "\n";
            exit (-1);
        }

        if (Tcl_GetDouble(interp, argv[loc + 6], &rho_in) != TCL_OK)
        {
            std::cerr << "Warning: nDMaterial DecoupledNeoHookean3D - invalid rho " << argv[loc + 6] << "\n";
            exit (-1);
        }
        ...
        {
            std::cerr << "Error: nDMaterial DecoupledMooneyRivlinSimo3D -invalid material model\n";
            exit (-1);
        }
    ...
    else if ( (strcmp(argv[loc + 2], "DecoupledOgdenSimo3D") == 0) || (strcmp(argv[loc + 2], "DecoupledOS3D") == 0) )
    {
        ...
        if (argc > 2 * N_in + 5)
        {
            ...
            {
                std::cerr << "Warning: invalid vector parameter number for Ogden Strain Energy Function\n";
                exit (-1);
            }
            ...
            for (int i = 0; i < N_in; i++)
            {
                ...
                {
                    std::cerr << "Warning: invalid parameter for Ogden Strain Energy Function\n";
                    exit (-1);
                }
            ...
            {
                std::cerr << "Warning: invalid Bulk Modulus number for Ogden Strain Energy Function\n";
                exit (-1);
            }

            if (Tcl_GetDouble(interp, argv[loc + 2 * N_in + 5], &rho_in) != TCL_OK)
            {
                std::cerr << "Warning: nDMaterial DecoupledSimoPister3D - invalid rho\n";
                exit (-1);
            }
            ...
            {
                std::cerr << "Error: nDMaterial DecoupledOgdenSimo3D -invalid material model\n";
                exit (-1);
            }
    ...
    else if ( (strcmp(argv[loc + 2], "DecoupledMooneyRivlin3D") == 0) || (strcmp(argv[loc + 2], "DecoupledMR3D") == 0) )
    {
        ...
        {
            std::cerr << "Error: nDMaterial DecoupledMooneyRivlin3D -invalid material model\n";
            exit (-1);
        }
    ...
    else if ( (strcmp(argv[loc + 2], "DecoupledOgden3D") == 0) )
    {
        ...
        if (argc > 2 * N_in + 4)
        {
            ...
            {
                std::cerr << "Warning: nDMaterial Ogden - invalid rho\n";
                exit (-1);
            }
            ...
            {
                std::cerr << "Error: nDMaterial DecoupledOgden3D -invalid material model\n";
                exit (-1);
            }
    ...
    else if ( (strcmp(argv[loc + 2], "DecoupledSimoPister3D") == 0) || (strcmp(argv[loc + 2], "DecoupledSP3D") == 0) )
    {
        ...
        if (argc < 6)
        {
            std::cerr << "Warning: DecoupledSimoPister3D -insufficient number of arguments\n";
            exit (-1);
        }

        if (Tcl_GetDouble(interp, argv[loc + 3], &K_in) != TCL_OK)
        {
            std::cerr << "Warning: nDMaterial DecoupledSimoPister3D - invalid K " << argv[loc + 3] << "\n";
            exit (-1);
        }

        if (Tcl_GetDouble(interp, argv[loc + 4], &rho_in) != TCL_OK)
        {
            std::cerr << "Warning: nDMaterial DecoupledSimoPister3D - invalid rho " << argv[loc + 4] << "\n";
            exit (-1);
        }
        ...
        {
            std::cerr << "Error: nDMaterial DecoupledSimoPister3D -invalid material model\n";
            exit (-1);
        }
    ...
    {
        std::cerr << "Error: nDMaterial - unknown FiniteDeformationElastic3D material model\n";
        exit (-1);
    }
