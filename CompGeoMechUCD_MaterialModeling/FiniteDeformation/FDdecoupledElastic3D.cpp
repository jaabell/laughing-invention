    ...
    : FiniteDeformationElastic3D(tag, classTag, rho_in)
{
    ...
    {
std:
        cerr << "FDdecoupledElastic3D:: FDdecoupledElastic3D failed to construct the W Energy\n";
        exit(-1);
    }
}
...
const stresstensor& FDdecoupledElastic3D::getPK1StressTensor(void)
{
    if ( FromForC == 0 )
    {
        ...
        FDdecoupledElastic3D::static_FDE_stress = F("kJ") * SPKStress("IJ");
    }

    if ( FromForC == 1 )
    {
std:
        cerr << "FDdecoupledElastic3D: unknown deformation gradient - cannot compute PK1 stress" << "\n";
        exit (-1);
    }
...
const stresstensor& FDdecoupledElastic3D::getCauchyStressTensor(void)
{
    if ( FromForC == 0 )
    {
        ...
        stresstensor tensor2 = tensor2("iK") * F("mK");
        double J = fabs(F.determinant());
        FDdecoupledElastic3D::static_FDE_stress = tensor2 * (1.0 / J);
    }

    if ( FromForC == 1 )
    {
std:
        cerr << "FDdecoupledElastic3D: unknown deformation gradient - cannot compute Cauchy stress" << "\n";
        exit (-1);
    }
...
{
std:
    cerr << "FDdecoupledElastic3D::getCopy(const char *) - not yet implemented\n";
...
int FDdecoupledElastic3D::sendSelf (int commitTag, Channel& theChannel)
{
    ...
    {
std:
        cerr << "FDdecoupledElastic3D::sendSelf -- could not send Vector\n";
        return res;
    }
...
//----------------------------------------------------------------------------------
void FDdecoupledElastic3D::Print (ostream& s, int flag)
{
...
int FDdecoupledElastic3D::ComputeTrials()
{
    ...
    {
std:
        cerr << "FDdecoupledElastic3D::getCaseIndex -- unknown case! \n";
        exit (-1);
    }
