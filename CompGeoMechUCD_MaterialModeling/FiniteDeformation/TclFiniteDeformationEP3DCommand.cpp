        ...
        TCL_Char** argv, TclModelBuilder* theTclBuilder, int eleArgStart)
{
    ...
    {
        std::cerr << "Warning: nDMaterial FDEP3D - invalid tag " << argv[loc] << "\n";
        exit (-1);
    }

    if (Tcl_GetInt(interp, argv[loc++], &tagElasticFD) != TCL_OK)
    {
        std::cerr << "Warning: nDMaterial FDEP3D - invalid elastic material tag " << argv[loc] << "\n";
        exit (-1);
    }

    if (tagElasticFD == tag)
    {
        std::cerr << "Error: nDMaterial FDEP3D, elastic matTag is the same with FDEP3D matTag" << argv[loc] << "\n";
        exit (-1);
    }
    ...
    if (tagElasticFD == 0)
    {
        std::cerr << "WARNING: nD FD elastic material does not exist\n";
        std::cerr << "nD FD material: " << tagElasticFD;
    ...
    while (loc < argc)
    {
        ...
        if ((strcmp(argv[loc], "-fdYield") == 0) || (strcmp(argv[loc], "-fdY") == 0))
        {
            ...
            if (fdY == 0)
            {
                std::cerr << "Warning: nDMaterial FDEP3D - could not create a fdYield from" << argv[loc + 1] << "\n";
                exit (-1);
            }
        }
        ...
        else if ((strcmp(argv[loc], "-fdFlow") == 0) || (strcmp(argv[loc], "-fdF") == 0))
        {
            ...
            if (fdF == 0)
            {
                std::cerr << "Warning: nDMaterial FDEP3D - could not create a fdFlow from" << argv[loc + 1] << "\n";
                exit (-1);
            }
        }
        ...
        else if ((strcmp(argv[loc], "-fdEvolution_S") == 0) || (strcmp(argv[loc], "-fdES") == 0))
        {
            ...
            if (fdES == 0)
            {
                std::cerr << "Warning: nDMaterial FDEP3D - could not create a fdES from" << argv[loc + 1] << "\n";
                exit (-1);
            }
        }
        ...
        else if ((strcmp(argv[loc], "-fdEvolution_T") == 0) || (strcmp(argv[loc], "-fdET") == 0))
        {
            ...
            if (fdET == 0)
            {
                std::cerr << "Warning: nDMaterial FDEP3D - could not create a fdET from" << argv[loc + 1] << "\n";
                exit (-1);
            }
        }
        ...
        {
            std::cerr << "Warning: nDMaterial FDEP3D - don't understand %s\n";
            exit (-1);
        }
    ...
    {
        std::cerr << "Warning: invalid args used to create a FiniteDeformationEP3D material\n";
    }
...
fdYield* EvaluatefdYield(ClientData clientData, Tcl_Interp* interp, TCL_Char* tclString)
{
    ...
    if ((strcmp(argv[0], "-VM") == 0) || (strcmp(argv[0], "-vM") == 0) || (strcmp(argv[0], "-J2") == 0))
    {
        ...
        if (argc >= 2)
        {
            ...
            {
                std::cerr << "Warning: nDMaterial FDEP3D - invalid Y0 " << argv[1] << "\n";
                exit (-1);
            }
        }
        ...
        if (argc >= 2 + 9)
        {
            ...
            for (j = 0; j < 9; j++)
            {
                if (Tcl_GetDouble(interp, argv[2 + j], &av[j]) != TCL_OK)
                {
                    std::cerr << "Warning:  FDEP3D - invalid input " << argv[2 + j] << endln;
                    exit (1);
                }
            }
    ...
    else if ((strcmp(argv[0], "-DP") == 0) || (strcmp(argv[0], "-dp") == 0) )
    {
        ...
        if (argc >= 2)
        {
            if (Tcl_GetDouble(interp, argv[1], &m0) != TCL_OK)
            {
                std::cerr << "Warning: nDMaterial FDEP3D - invalid m0 " << argv[1] << "\n";
                exit (-1);
            }
        }
        ...
        fdY = new fdYieldDP(m0, a0);
    }

    else
    {
        std::cerr << "Warning: invalid fd yield function: " << argv[0] << "\n";
        exit (-1);
    }
...
fdFlow* EvaluatefdFlow(ClientData clientData, Tcl_Interp* interp, TCL_Char* tclString)
{
    ...
    if ((strcmp(argv[0], "-VM") == 0) || (strcmp(argv[0], "-vM") == 0) || (strcmp(argv[0], "-J2") == 0))
    {
        ...
        fdF = new fdFlowVM(Y0, a0);
    }

    // 2. Druke-Prager fd Flow Surface
    //
    else if ((strcmp(argv[0], "-DP") == 0) || (strcmp(argv[0], "-dp") == 0) )
    {
        double m0 = 0.0;
        ...
        fdF = new fdFlowDP(m0, a0);
    }

    else
    {
        std::cerr << "Warning: invalid fd flow rule: " << argv[0] << "\n";
        exit (-1);
    }
...
fdEvolution_S* EvaluatefdEvolution_S(ClientData clientData, Tcl_Interp* interp, TCL_Char* tclString)
{
    ...
    if ((strcmp(argv[0], "-LS") == 0) || (strcmp(argv[0], "-LinearSaturated") == 0))
    {
        ...
        if (argc >= 2)
        {
            ...
            {
                std::cerr << "Warning: nDMaterial FDEP3D - invalid H_linear " << argv[1] << "\n";
                cleanup(argv);
                exit (-1);
            }
        }
        ...
        if (argc >= 4)
        {
            ...
            {
                std::cerr << "Warning: nDMaterial FDEP3D - invalid q_saturated " << argv[2] << "\n";
                cleanup(argv);
                exit (-1);
            }
            ...
            {
                std::cerr << "Warning: nDMaterial FDEP3D - invalid beta " << argv[3] << "\n";
                cleanup(argv);
                exit (-1);
            }
        }
