...
//#                    Nov2006, removed the space lines caused by known reason
//===============================================================================
    ...
    : NDMaterial(tag, ND_TAG_FiniteDeformationEP3D)
{
    ...
    {
        std::cerr << "FiniteDeformationEP3D:: FiniteDeformationEP3D failed to construct the fdYield\n";
        exit (-1);
    }
    ...
    {
        std::cerr << "FiniteDeformationEP3D:: FiniteDeformationEP3D failed to construct the fdFlow\n";
        exit (-1);
    }
...
//----------------------------------------------------------------------
void FiniteDeformationEP3D::Print(ostream& s, int flag)
{
    s << (*this);
}
...
const stresstensor& FiniteDeformationEP3D::getPK1StressTensor(void)
{
    ...
    FiniteDeformationEP3D::static_stress = F("kJ") * SPKStress("IJ");

    return FiniteDeformationEP3D::static_stress;
}
...
const stresstensor& FiniteDeformationEP3D::getCauchyStressTensor(void)
{
    ...
    FiniteDeformationEP3D::static_stress = tensor2 * (1.0 / J);
...
int FiniteDeformationEP3D::ImplicitAlgorithm()
{
    ...
    straintensor res_Ee;                // residual of intermediate Ee
    straintensor res_eta;               // norm of residual of eta
    ...
    straintensor Fpinv;         // inverse of Fp
    straintensor Ce;            // intermediate C
    straintensor Ee;            // intermediate Ee

    straintensor Fp_n;       // Plastic deformation gradient at time n
    straintensor Fp_ninv;    // Plastic deformation gradient at time n
    straintensor Ee_n;       // Ee at the incremental step n, calculated from Fp_n
    double xi_n = 0.0;       // xi at the incremental step n, known
    straintensor eta_n;      // eta at the incremental step n, known

    stresstensor Mtensor;    // --> dFl/dT
    stresstensor MCtensor;   // --> dFl/dS
    tensor Ltensor(4, def_dim_4, 0.0);          // Tangent tensor in the intermediate configuration
    tensor LATStensor(4, def_dim_4, 0.0);       // Consistent tangent tensor in the intermediate configuration

    double nscalar = 0.0;               // --> dFl/d(xi)
    double Kscalar = 0.0;               // Isotropic hardening modoulus
    straintensor ntensor;               // --> dFl/d(eta)
    tensor Ktensor(4, def_dim_4, 0.0);  // Kinematic hardening modoulus

    stresstensor dyods;      // --> dY/d(stress)
    double dyodq = 0.0;      // --> dY/d(xi)
    stresstensor dyoda ;     // --> dY/d(eta)

    tensor dMods(4, def_dim_4, 0.0);            // --> dM/d(stress), from d2Fl/d(stress)d(stress)
    tensor dMCods(4, def_dim_4, 0.0);           // --> dMs/d(stress)

    double dnsodq = 0.0;                        // --> d2Fl/dqdq
    tensor dntoda(4, def_dim_4, 0.0);           // --> d2Fl/dada
    ...
    tensor A11(4, def_dim_4, 0.0);
    tensor A12(2, def_dim_2, 0.0);
    tensor A13(4, def_dim_4, 0.0);
    tensor A21(2, def_dim_2, 0.0);
    double a22 = 0.0;
    tensor A23(2, def_dim_2, 0.0);
    tensor A31(4, def_dim_4, 0.0);
    tensor A32(2, def_dim_2, 0.0);
    tensor A33(4, def_dim_4, 0.0);

    BJmatrix C99(9, 9, 0.0);
    BJmatrix CCC(19, 19, 0.0);
    BJmatrix TTT(19, 19, 0.0);
    ...
    tensor LM(4, def_dim_4, 0.0);      // For Mandel Tangent Stiffness
    stresstensor  B_Mandel;            // For Mandel stress
    ...
    if ( yieldfun > (fdy->getTolerance()) )   // start of plastic part
    {
        ...
        do  // beginning of do - while
        {
            ...
            for (i = 1; i <= 9; i++)
            {
                for (j = 1; j <= 9; j++)
                {
                    C99.val(i, j) = TTT.cval(i, j);
                }
            }

            A11 = C99.BJmatrix2BJtensor_2();

            A12.val(1, 1) = TTT.cval(1, 10);
            A12.val(1, 2) = TTT.cval(2, 10);
            A12.val(1, 3) = TTT.cval(3, 10);
            A12.val(2, 1) = TTT.cval(4, 10);
            A12.val(2, 2) = TTT.cval(5, 10);
            A12.val(2, 3) = TTT.cval(6, 10);
            A12.val(3, 1) = TTT.cval(7, 10);
            A12.val(3, 2) = TTT.cval(8, 10);
            A12.val(3, 3) = TTT.cval(9, 10);

            A21.val(1, 1) = TTT.cval(10, 1);
            A21.val(1, 2) = TTT.cval(10, 2);
            A21.val(1, 3) = TTT.cval(10, 3);
            A21.val(2, 1) = TTT.cval(10, 4);
            A21.val(2, 2) = TTT.cval(10, 5);
            A21.val(2, 3) = TTT.cval(10, 6);
            A21.val(3, 1) = TTT.cval(10, 7);
            A21.val(3, 2) = TTT.cval(10, 8);
            A21.val(3, 3) = TTT.cval(10, 9);
            ...
            for (i = 1; i <= 9; i++)
            {
                for (j = 1; j <= 9; j++)
                {
                    C99.val(j, i) = TTT.cval(10 + i, j);
                }
            }

            A31 = C99.BJmatrix2BJtensor_2();

            A32.val(1, 1) = TTT.cval(11, 10);
            A32.val(1, 2) = TTT.cval(12, 10);
            A32.val(1, 3) = TTT.cval(13, 10);
            A32.val(2, 1) = TTT.cval(14, 10);
            A32.val(2, 2) = TTT.cval(15, 10);
            A32.val(2, 3) = TTT.cval(16, 10);
            A32.val(3, 1) = TTT.cval(17, 10);
            A32.val(3, 2) = TTT.cval(18, 10);
            A32.val(3, 3) = TTT.cval(19, 10);

            A23.val(1, 1) = TTT.cval(10, 11);
            A23.val(1, 2) = TTT.cval(10, 12);
            A23.val(1, 3) = TTT.cval(10, 13);
            A23.val(2, 1) = TTT.cval(10, 14);
            A23.val(2, 2) = TTT.cval(10, 15);
            A23.val(2, 3) = TTT.cval(10, 16);
            A23.val(3, 1) = TTT.cval(10, 17);
            A23.val(3, 2) = TTT.cval(10, 18);
            A23.val(3, 3) = TTT.cval(10, 19);

            for (i = 1; i <= 9; i++)
            {
                for (j = 1; j <= 9; j++)
                {
                    C99.val(i, j) = TTT.cval(10 + i, 10 + j);
                }
            }
            ...
            {
                std::cerr << "Warnning: Iteration More than " << Max_Iter;
        ...
        // Begin...
        tensor tensor1 = fdf->d2Fodsds(B_Mandel, *fdeps);
        tensorTemp6 = tensor1("ijkl")  * LM("klmn");
        tensorTemp6.null_indices();
        stresstensor tensor2 = fdf->d2Fodsdq(B_Mandel, *fdeps);
        tensorTemp7 = tensor2 * Kscalar;

        if ( fdEvolutionT )
        {
            tensor tensor3 = fdf->d2Fodada(B_Mandel, *fdeps);
            tensorTemp8 = tensor3("ijkl") * Ktensor("klmn");
            tensorTemp8.null_indices();
        }
