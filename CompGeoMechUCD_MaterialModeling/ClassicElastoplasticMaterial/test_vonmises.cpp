...
int main (void)
{
	...
	int tag = 1;
	double k0 = 150;
	double H_alpha = 150 * 10;
	double H_k = 0;//10;
	double E = 1000;
	double nu = 0.25;
	double rho = 0.0;
	VonMisesLinearHardening von(tag , k0 , H_alpha , H_k, E, nu, rho);
	...
	for (int i = 0; i < 5; i++)
	{
		von.setTrialStrainIncr(de);
		sigma = von.getStressTensor();
		epsilon = von.getStrainTensor();
		von.commitState();
	...
	fstream fout("out_von.txt", ios::out);
