...
int main(){
	...
	double kinematic_hardening_AF_ha=100;   
	...
	auto DP_NonAssoc_AF= new DruckerPragerNonAssociateArmstrongFrederick(
		material_tag,
	...
	auto initial_strain = DP_NonAssoc_AF -> getStrainTensor();
	cout<<"The initial strain state: "<<endl;
	cout<<initial_strain<<endl;

	auto initial_stress = DP_NonAssoc_AF -> getStressTensor();
	...
	DP_NonAssoc_AF->setTrialStrainIncr(myinputStrain);
	DP_NonAssoc_AF->commitState();
	print_this_step(++mystep,DP_NonAssoc_AF);
