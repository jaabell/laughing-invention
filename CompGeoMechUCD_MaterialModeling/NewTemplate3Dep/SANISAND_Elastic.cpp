...
int SANISAND_Elastic::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    {
        cerr << "SANISAND_Elastic::sendSelf -- failed to send ID\n";
        return -1;
    }

    if (theChannel.sendnDarray(dataTag, commitTag, this->Stress) < 0)
    {
        cerr << "SANISAND_Elastic::sendSelf() -  failed to send nDarray Stress\n";
        return -1;
    }

    if (theChannel.sendnDarray(dataTag, commitTag, this->Strain) < 0)
    {
        cerr << "SANISAND_Elastic::sendSelf() -  failed to send nDarray Strain\n";
        return -1;
    }
