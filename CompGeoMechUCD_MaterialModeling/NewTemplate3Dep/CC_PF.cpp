...
double CC_PF::getM(const MaterialParameter& MaterialParameter_in) const
{
    ...
    {
        cerr << "Warning!! CC_PF: Invalid Input (M). " << endln;
        exit (1);
    }
}
...
double CC_PF::getP0(const MaterialParameter& MaterialParameter_in) const
{
    ...
    {
        cerr << "Warning!! CC_PF: Invalid Input (po). " << endln;
        exit (1);
    }
}
...
int CC_PF::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    {
        cerr << "CC_PF::sendSelf -- failed to send ID\n";
        return -1;
    }
...
//Guanzhou added for parallel
int CC_PF::receiveSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
    int dataTag = this->getDbTag();
    ...
    {
        cerr << "CC_PF::receiveSelf -- failed to recv ID\n";
        return -1;
    }
