...
////////////////////////////////////////////////////////////////
PressureDependent_Elastic::~PressureDependent_Elastic()
{
...
const BJtensor& PressureDependent_Elastic::getElasticStiffness (const MaterialParameter& MaterialParameter_in) const
{
    ...
    if (v >= 0.5 || v < -1.0)
    {
        cerr << "Warning!! PressureDependent_Elastic: Invalid possoin's ratio. " << endln;
        exit (1);
    }
    ...
    if ( p_ref <= 0.0)
    {
        cerr << "Warning!! PressureDependent_Elastic: Invalid reference pressure. " << endln;
        exit (1);
    }
...
double PressureDependent_Elastic::getE0(const MaterialParameter& MaterialParameter_in) const
{
    ...
    {
        cerr << "PressureDependent_Elastic: Invalid Input. " << endln;
        exit (1);
    }
    else
    {
...
int PressureDependent_Elastic::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    {
        cerr << "PressureDependent_Elastic::sendSelf -- failed to send ID\n";
        return -1;
    }

    if (theChannel.sendnDarray(dataTag, commitTag, this->Stress) < 0)
    {
        cerr << "PressureDependent_Elastic::sendSelf() -  failed to send nDarray Stress\n";
        return -1;
    }

    if (theChannel.sendnDarray(dataTag, commitTag, this->Strain) < 0)
    {
        cerr << "PressureDependent_Elastic::sendSelf() -  failed to send nDarray Strain\n";
        return -1;
    }
