...
double Linear_Eij::getLinearFactor(const MaterialParameter& material_parameter) const
{
    ...
    {
        cerr << "Linear_Eij: Invalid Input. " << endln;
        exit (1);
    }
}
...
int Linear_Eij::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    {
        cerr << "Linear_Eij::sendSelf -- failed to send ID\n";
        return -1;
    }
...
//Guanzhou added for parallel
int Linear_Eij::receiveSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
    int dataTag = this->getDbTag();
    ...
    {
        cerr << "Linear_Eij::receiveSelf -- failed to recv ID\n";
        return -1;
    }
