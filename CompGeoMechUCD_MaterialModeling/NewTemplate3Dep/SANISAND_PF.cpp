...
const stresstensor& SANISAND_PF::getalpha(const MaterialParameter& MaterialParameter_in) const
{
    ...
    {
        cerr << "SANISAND_PF: Invalid Input. " << endln;
        exit (1);
    }
}
...
double SANISAND_PF::getParameters(const MaterialParameter& MaterialParameter_in, int parIndex, int which) const
{
    ...
    {
        cerr << "SANISAND_PF: Invalid Input. " << parIndex << " and " << which << endln;
        exit (1);
    }
}
...
double SANISAND_PF::getec(double e_r, double lambda, double xi, double Pat, double p_c) const
{
    ...
    {
        cerr << "Warning: SANISAND_PF - 'p_c/Pat' less than zero! " << endln;
    }
...
int SANISAND_PF::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    idData(1) = e_r_which;
    idData(14) = index_e_r;
    idData(2) = lambda_which;
    idData(15) = index_lambda;
    idData(3) = xi_which;
    idData(16) = index_xi;
    idData(4) = Pat_which;
    idData(17) = index_Pat;
    idData(5) = alpha_cc_which;
    idData(18) = index_alpha_cc;
    idData(6) = c_which;
    idData(19) = index_c;
    idData(7) = A0_which;
    idData(20) = index_A0;
    ...
    {
        cerr << "SANISAND_PF::sendSelf -- failed to send ID\n";
        return -1;
    }
...
//Guanzhou added for parallel
int SANISAND_PF::receiveSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
    int dataTag = this->getDbTag();
    ...
    {
        cerr << "SANISAND_PF::receiveSelf -- failed to recv ID\n";
        return -1;
    }
    ...
    e_r_which      = idData(1) ;
    index_e_r      = idData(14);
    lambda_which   = idData(2) ;
    index_lambda   = idData(15);
    xi_which       = idData(3) ;
    index_xi       = idData(16);
    Pat_which      = idData(4) ;
    index_Pat      = idData(17);
    alpha_cc_which = idData(5) ;
    index_alpha_cc = idData(18);
    c_which        = idData(6) ;
    index_c        = idData(19);
    A0_which       = idData(7) ;
    index_A0       = idData(20);
