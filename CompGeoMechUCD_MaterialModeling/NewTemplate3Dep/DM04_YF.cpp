        ...
        int which) const
{
    ...
    //}
    //else {
    // cerr << "DM04_YF: Invalid Input. " << endln;
    // exit (1);
    //}
...
double DM04_YF::getm(const MaterialParameter& MaterialParameter_in) const
{
    ...
    if ( m_which == 0 && index_m <= MaterialParameter_in.getNum_Material_Constant() && index_m > 0 )
    {
        ...
        if (m <= 0.0)
        {
            cerr << "DM04_YF: Invalid Input, m <= 0.0. " << endln;
            exit (1);
        }
    ...
    {
        cerr << "Warning!! DM04_YF: Invalid Input. " << endln;
        exit (1);
    }
}
...
int DM04_YF::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    {
        cerr << "DM04_YF::sendSelf -- failed to send ID\n";
        return -1;
    }
...
//Guanzhou added for parallel
int DM04_YF::receiveSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
    int dataTag = this->getDbTag();
    ...
    {
        cerr << "DM04_YF::receiveSelf -- failed to recv ID\n";
        return -1;
    }
