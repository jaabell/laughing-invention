        ...
        const MaterialParameter& MaterialParameter_in)  const
{
    ...
    // Nima Tafazzoli added form (II) lecture notes
    // Febuary 2010
...
double DP_PF::getm(const MaterialParameter& MaterialParameter_in) const
{
    ...
    if ( m_which == 0)
    {
        ...
        {
            cerr << "DP_PF: Invalid Input. " << endln;
            exit (1);
        }
    }
    else if ( m_which == 1)
...
int DP_PF::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    {
        cerr << "DP_PF::sendSelf -- failed to send ID\n";
        return -1;
    }
...
//Guanzhou added for parallel
int DP_PF::receiveSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
    int dataTag = this->getDbTag();
