...
int define_convergence_test_RelativeNormunbalance_for_analysis(double theTol, int maxIter, int PrintFlag)
{
	theConvergenceTest = new CTestRelativeNormUnbalance(theTol, maxIter, PrintFlag);

	if (theConvergenceTest == NULL)
	{
		cerr << "Error: (define_convergence_test_RelativeNormunbalance_for_analysis) memory for convergence test can not be allocated!" << endl;
		return -1;
	}
