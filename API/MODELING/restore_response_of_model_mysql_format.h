...
int restore_response_of_model_mysql_format(int Node_Number, int DOF_Number, int Step_Number,
        ...
        string socket)
{
    ...
    if (theDatabase == NULL)
    {
        cerr << "Error: (restore_state_of_model_mysql_format) memory allocation problem!" << endl;
        return -1;
    }


    double displacement(theDatabase->restoreNodalDisplacement(Node_Number, DOF_Number, Step_Number));

    cout.flush() << "\nNode Number: " << Node_Number << " DOF Number: " << DOF_Number << ", Step Number: " << Step_Number << ", Displacement: " << displacement << "\n";
