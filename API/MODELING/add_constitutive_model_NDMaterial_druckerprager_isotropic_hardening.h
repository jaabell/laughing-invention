...
// Defining  3D Drucker-Prager Isotropic Hardening material model
//!
//! @param[in] MaterialNumber number/tag of the nD material to be used
//! @param[in] AlgorithType Explicit (=0) or Implicit (=1)
...
//!
//! \details Material Modeling, Drucker-Prager Isotropic Hardening .
//!
//! \details This DSL command is used to define a Drucker-Prager Isotropic Hardening  material model
//!
...
int add_constitutive_model_NDMaterial_druckerprager_isotropic_hardening(int MaterialNumber,
        int Algorithm,
        double rho,
        double E,
        double v,
        ...
        double tolerance_2)
{
    ...
    if (theMaterial == NULL)
    {
        cerr << "Error: (add_constitutive_model_NDMaterial_druckerprager_isotropic_hardening) memory allocation problem!" << endl;
        return -1;
    }

    if( theDomain.addNDMaterial(*theMaterial) != 0 )
    {
        cerr << "Error: (add_constitutive_model_NDMaterial_druckerprager_isotropic_hardening) Material " << MaterialNumber << " could not be added to the domain " << endl;
        return -1;
    }
