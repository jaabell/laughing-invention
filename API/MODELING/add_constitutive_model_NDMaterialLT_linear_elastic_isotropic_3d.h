...
int add_constitutive_model_NDMaterialLT_linear_elastic_isotropic_3d(int MaterialNumber,
        double ElasticModulus,
        double nu,
        double rho)
{
    NDMaterialLT *theMaterial = 0;
    theMaterial = new ElasticIsotropic3DLT(MaterialNumber, ElasticModulus, nu, rho);

    if (theMaterial == NULL)
    {
        cerr << "Error: (add_constitutive_model_NDMaterial_linear_elastic_isotropic_3d_LT) memory allocation problem!" << endl;
        return -1;
    }

    if ( theDomain.addNDMaterialLT(*theMaterial) != 0 )
    {
        cerr << "Error: (add_constitutive_model_NDMaterial_linear_elastic_isotropic_3d_LT) Material " << MaterialNumber << " could not be added to the domain " << endl;
        return -1;
    }
