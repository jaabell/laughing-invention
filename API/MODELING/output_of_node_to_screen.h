...
//! \n
//! This documentation is the API for Domain Specific Language (DSL) of print node which gives some information about node.
//! \n
//! \n
...
//! Inputs:
//! - NodeNumber: tag number of the node
...
int output_of_node_to_screen(int NodeNumber)
{
    Node *theNode = 0;
    theNode = theDomain.getNode(NodeNumber);

    if (theNode == NULL)
    {
        cerr << "Error: (output_of_node_to_screen) node could not be retrieved from the domain" << endl;
        return -1;
    }
