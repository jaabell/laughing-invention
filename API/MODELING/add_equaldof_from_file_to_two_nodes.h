...
int add_equaldof_from_file_to_two_nodes(string inputequaldofnodes)
{

    const char* nodesequaldoffromfile = inputequaldofnodes.c_str();

    ifstream inputequaldof;

    if(! inputequaldof)
    {
        cerr << "Error: (add_equaldof_from_file_to_two_nodes) Can not open " << inputequaldofnodes << "!" << endl;
        return -1;
    }
