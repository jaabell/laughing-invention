...
int add_element_brick_20node_variable_number_of_gauss_points(int ElementNumber,
        ...
        int MaterialNumber)
{
    ...
    if (theElement == NULL)
    {
        cerr << "Error: (add_element_brick_20node_variable_number_of_gauss_points) memory allocation problem for theElement!" << endl;
        return -1;
    }

    if (theDomain.addElement(theElement) == false)
    {
        cerr << "WARNING: (add_element_brick_20node_variable_number_of_gauss_points) could not add element to the domain\n";
