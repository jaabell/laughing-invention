...
//! \n
//! <div class="contents">
//! <h1>DSL of Constitutive Testing for Constant "P" Triaxial Test</h1>
...
//! \n
//! This documentation is the API for Domain Specific Language (DSL) of constitutive testing for constant "P" Triaxial test.
//! \n
//! \n
...
int simulate_constitutive_testing_for_constant_p_triaxial(double increment_size,
        double strain_cut,
        ...
        int MaterialNumber)
{
    ...
    cout << "Test condition is: Constant P triaxial loading (strain control) \n";
    ...
    if (FTEP == NULL)
    {
        cerr << "Error: (simulate_constitutive_testing_for_constant_p_triaxial) material allocation problem!" << endl;
        return -1;
    }
    ...
    do
    {
        ...
        int test_condition = 1; //constant_p_triaxial_loading_strain_control_d_epsilon_11
        ...
        outStress << -tStrain.val(1, 1) <<  "  "  << -tStrain.val(1, 2) << "  " << -tStrain.val(2, 2) << "  " << -tStrain.val(3, 3) << "  " << -volstrain <<  "  " << -devstrain << "  " << pp << "  " << MIT_q << "  " << qq << endln;
        Strain    << -tStrain.val(1, 1) <<  "  "  << -tStrain.val(1, 2) << "  " << -tStrain.val(2, 2) << "  " << -tStrain.val(3, 3) << "  " << -volstrain <<  "  " <<  -devstrain << endln;
        Stress    << -tStress.val(1, 1) <<  "  "  << -tStress.val(2, 2) << "  " << -tStress.val(3, 3) << "  " << pp << "  " << MIT_q << "  " << qq << endln;
